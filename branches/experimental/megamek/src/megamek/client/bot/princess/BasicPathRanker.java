...
public class BasicPathRanker extends PathRanker {
    ...
    protected double checkPathForHazards(MovePath path, Entity movingUnit, IGame game) {
        ...
        try {
            ...
            owner.log(getClass(), METHOD_NAME, LogLevel.DEBUG, logMsg);
        }
    }
        ...
        for (int hazard : hazards) {
            switch (hazard) {
                case Terrains.FIRE:
                    ...
                    break;
                case Terrains.MAGMA:
                    hazardValue += calcMagmaHazad(hex, endHex, movingUnit, jumpLanding, step, logMsg);
                    break;
                case Terrains.ICE:
    ...
    private double calcBuildingHazard(MoveStep step, Entity movingUnit, MovePath movePath, IBoard board,
        ...
        // Jumping onto building is handled in PathRanker.validatePaths.
        ...
        logMsg.append("\n\t\tChance to fail piloting roll: ").append(LOG_DECIMAL.format(odds));

        // Hazard is based on potential damage taken.
        double dmg = board.getBuildingAt(step.getPosition()).getCurrentCF(step.getPosition()) / 10;
    ...
    private double calcIceHazard(Entity movingUnit, IHex hex, MoveStep step, boolean jumpLanding,
        ...
        // Hazard is based on chance ot break through to the water underneath.
        double breakthroughMod = jumpLanding ? 0.5 : 0.1667;
        logMsg.append("\n\t\tChance to break through ice: ").append(LOG_DECIMAL.format(breakthroughMod));
    ...
    private double calcWaterHazard(Entity movingUnit, IHex hex, MoveStep step, StringBuilder logMsg) {
        ...
        for (int i = 0; i < movingUnit.locations(); i++) {
            if (Mech.LOC_CLEG == i && !(movingUnit instanceof TripodMech)) {
                continue;
            }
            ...
            if (Mech.LOC_RLEG == i || Mech.LOC_LLEG == i) {
                submergedLocations.add(i);
                continue;
            }

            if (movingUnit instanceof TripodMech && Mech.LOC_CLEG == i) {
                submergedLocations.add(i);
                continue;
            }

            if ((movingUnit instanceof QuadMech) && (Mech.LOC_RARM == i || Mech.LOC_LARM == i)) {
                submergedLocations.add(i);
            }
        }
        ...
        for (int loc : submergedLocations) {
            ...
            // If there are any engine crits in a breached side torso, water is deadly.
            if ((Mech.LOC_LT == loc || Mech.LOC_RT == loc) &&
                (movingUnit.getEngine().getSideTorsoCriticalSlots().length > 0)) {
    ...
    private double calcFireHazard(Entity movingUnit, boolean endHex, StringBuilder logMsg) {
        ...
        // Tanks risk critical hits by moving into fire.
        if (movingUnit instanceof Tank) {
            logMsg.append("Possible crit on tank (25).");
            return 25;
        }

        // Protomechs risk location destruction by moving into fire.
        if (movingUnit instanceof Protomech) {
            logMsg.append("Possible location destruction (50).");
            return 50;
        }

        // Infantry and BA risk total destruction by moving into fire.
        ...
        if (movingUnit.getHeatCapacity() != 999) {
    ...
    private double calcMagmaHazad(IHex hex, boolean endHex, Entity movingUnit, boolean jumpLanding, MoveStep step,
                                  StringBuilder logMsg) {
        logMsg.append("\n\tCalculating magma hazard:  ");

        // Hovers are unaffected.
        if (EntityMovementMode.HOVER.equals(movingUnit.getMovementMode())) {
        ...
        if (magmaLevel == 2) {
            ...
            double breakThroughMod = jumpLanding ? 0.5 : 0.1667;
            logMsg.append("\n\t\tChance to break through crust = ").append(LOG_DECIMAL.format(breakThroughMod));

            // Factor in the chance to break through.
            double lavaHazrd = calcLavaHazard(endHex, movingUnit, step, logMsg) * breakThroughMod;
            logMsg.append("\nt\t\t\tLavaHazard (").append(LOG_DECIMAL.format(lavaHazrd)).append(").");
            hazardValue += lavaHazrd;
    ...
    private double calcLavaHazard(boolean endHex, Entity movingUnit, MoveStep step, StringBuilder logMsg) {
        ...
        // Non-mech units auto-destroyed by lava.
