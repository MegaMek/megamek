...
public class CityBuilder {
    ...
    private void buildHubCity(int maxX, int maxY,int roads){
        ...
        for ( int dir = 0; dir < roads; dir++){
            ...
            while (coords.x >= -1 && coords.x <= maxX && coords.y >= -1 && coords.y <= maxY ){
                ...
                if(coords == null || (cityPlan.contains(coords) && x!=midX && y!=midY)) {
                    break;
                }
                cityPlan.add(coords);
    ...
    private void buildMetroCity(int maxX, int maxY){
        ...
        for ( int hex=0; hex < 6; hex++ )
            cityPlan.add(new Coords(Coords.xInDir(midX,midY,hex),Coords.yInDir(midX,midY,hex)));
    ...
    private void addBridge(IHex hex, int exits, int altitude, int cf) {
        ...
        hex.addTerrain(tf.createTerrain(Terrains.BRIDGE, getBuildingTypeByCF(cf), true, (exits & 63)));
    ...
    private Coords tryToBuildBridge(Coords start, int direction) {
        ...
        if(end != null) {
            ...
            Compute.randomInt(1 + mapSettings.getCityMaxCF() - mapSettings.getCityMinCF());
