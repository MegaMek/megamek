...
public class AmmoTypeTest {
    ...
    public void testIsAmmoValid() {
        ...
        TestCase.assertTrue(AmmoType.isAmmoValid(mockAmmoAC5, mockAC5));
        TestCase.assertTrue(AmmoType.isAmmoValid(mockAmmoSrm4, mockSRM4));
        TestCase.assertTrue(AmmoType.isAmmoValid(mockAmmoInferno4, mockSRM4));

        // Test an empty ammo bin.
        TestCase.assertFalse(AmmoType.isAmmoValid(mockAmmoAC5Empty, mockAC5));

        // Test null ammo.
        TestCase.assertFalse(AmmoType.isAmmoValid((Mounted)null, mockAC5));
        TestCase.assertFalse(AmmoType.isAmmoValid((AmmoType)null, mockAC5));

        // Test incompatible ammo.
        TestCase.assertFalse(AmmoType.isAmmoValid(mockAmmoAC5, mockSRM4));
        TestCase.assertFalse(AmmoType.isAmmoValid(mockAmmoSrm4, mockPPC));

        // Test wrong rack size.
        TestCase.assertFalse(AmmoType.isAmmoValid(mockAmmoAC10, mockAC5));
        TestCase.assertFalse(AmmoType.isAmmoValid(mockAmmoSrm6, mockSRM4));

        // Test something that's not ammo.
        TestCase.assertFalse(AmmoType.isAmmoValid(mockNotAmmo, mockAC5));
    }
}
