...
/**
 * This lass represents Land Air Mechs.
 * It is to be considered experimental, and not yet ready for release.
 * @author dirk
 *
 */
public class LandAirMech extends BipedMech {
    ...
    //conversion modes: all conversions take a turn and the mech uses an intermediate mode in them.
    /**
     * this is a placeholder for generic conversions.
     * All conversion modes must be greater than this.
     */
    public static final int MODE_CONVERT =                     0x100;
    
    public static final int MODE_CONVERT_MECH_TO_AIRMECH =     0x112;
    public static final int MODE_CONVERT_MECH_TO_AIRCRAFT =    0x114; //illegal conversion
    
    public static final int MODE_CONVERT_AIRMECH_TO_MECH =     0x121;
    public static final int MODE_CONVERT_AIRMECH_TO_AIRCRAFT = 0x124;
    
    public static final int MODE_CONVERT_AIRCRAFT_TO_MECH =    0x141; //illegal conversion
    ...
    /**
     * Returns the elevation of the LAM, correct in Mech mode, but needs fixing in other modes.
     */
     ...
     * assuming passed elevation.
     * 
     * What are the rules regarding airmechs in water?
     * I'll assume that airmechs can't go underwater, and that fighters follow the rules for VTOLS
     */
    ...
    public void newRound(int roundNumber) {
        if(mode >= MODE_CONVERT) { //if we are undergoing a conversiuon
            switch(mode) {
                ...
                case MODE_CONVERT_AIRCRAFT_TO_MECH:
                    //I am not sure if changing back while in the air to mech form is allowed.
                    //It is almost always a bad idea however. I'll assume the mech crashes prone.
                    mode = MODE_MECH;
                    if(getElevation() > 0){ //the mech was up in the air, now it comes crashing down.
        ...
        startMode = mode; //set the mode we start the turn in. Can never be on of the conversion modes.
    ...
    public boolean isEligibleForFiring() {
        ...
        if(mode == MODE_CONVERT_AIRMECH_TO_AIRCRAFT || mode == MODE_CONVERT_AIRCRAFT_TO_AIRMECH)
            return false;
