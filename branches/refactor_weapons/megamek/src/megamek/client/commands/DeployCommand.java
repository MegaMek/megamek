...
public class DeployCommand extends ClientCommand {
    ...
    public String run(String[] args) {
        if(args.length == 1) {
            ...
            return "Unit " + id + " deployed to " + coord.toFriendlyString() + ". (this is assuming it worked. No error checking done.)";
        }
    ...
    public String legalDeploymentZone()
    {
        ...
        switch (nDir) {
        case 0 : // Any
            return deep +"Deploy nearly anywhere. MinX: " + (minx+1) + " MinY: " + (miny+1) + " MaxX: " + (maxx+1) + " MaxY: " + (maxy+1);
        case 1 : // NW
        ...
        case 10: // Centre
            return deep + "Deploy in the center. MinX: " + (Math.max(minx, width / 3)+1) + " MinY: " + (Math.max(miny, height / 3)+1) + " MaxX: " + (Math.min(maxx, 2*width / 3)+1) + " MaxY: " + (Math.min(miny, 2*height / 3)+1);
        default : // ummm. . 
