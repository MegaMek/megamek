...
public class WeaponHandler implements AttackHandler {
    ...
    boolean bGlancing = false;
    public boolean bMissed;
    int nRange;
    int nDamPerHit;
    boolean throughFront;
    boolean announcedEntityFiring = false;
    ...
    /**
     * @return a <code>boolean</code> value indicating wether or not 
     *         this attack needs further calculating, like a missed shot
    ...
    protected int calcDamagePerHit() {
        if (bGlancing) {
            return (int)Math.floor(wtype.getDamage()/2.0);
        }
    ...
    protected void handleIgnitionDamage(Vector vPhaseReport,
        ...
        if (tn != TargetRoll.IMPOSSIBLE) {
            ...
            Report.addNewline(vPhaseReport);
            server.tryIgniteHex( target.getPosition(), subjectId, false, tn, true );
        }
    }

    protected void handleClearDamage(Vector vPhaseReport, Building bldg,
        ...
        // Buildings can't be accidentally ignited.
        if (bldg != null && server.tryIgniteHex(target.getPosition(), subjectId, false, 9)) {
            return;
        }
    ...
    protected void handleBuildingDamage(Vector vPhaseReport,
    ...
    public WeaponHandler(ToHitData t, WeaponAttackAction w, IGame g, Server s) {
        ...
        if (target instanceof Mech) {
            throughFront = Compute.isThroughFrontHex(game, waa.getEntityId(), (Entity)target); 
        } else {
            throughFront = true;
        }
        roll = Compute.d6(2);
     ...
     * @param bMissed      - a <code>boolean</code> value indicating wether the attack
     *                 missed or hit   
     * @return a <code>boolean</code> value indicating wether further processin
     *         is needed or not
     */
