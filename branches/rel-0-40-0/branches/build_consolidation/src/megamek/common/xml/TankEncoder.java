...
public class TankEncoder {
    ...
    public static void encode(Entity entity, Writer out) throws IOException {
        ...
        if (tank.isTurretLocked(Tank.LOC_TURRET_2)) {
            ...
            value = tank.getFacing();
            out.write(String.valueOf(value));
            out.write("\" /><turret2Facing value=\"");
            value = tank.getDualTurretFacing();
            out.write(String.valueOf(value));
            out.write("\" /><turret2Locked value=\"true");
        }
        out.write("\" />");
    }
    ...
    public static Entity decode(ParsedXML node, IGame game) {
        ...
        while (children.hasMoreElements()) {
            ...
            // Did we find the turret's secondaryFacing node?
            else if (childName.equals("turret2Facing")) {

                // Get the Tank's turret's facing.
                attrStr = child.getAttribute("value");
                if (null == attrStr) {
                    throw new IllegalStateException(
                            "Couldn't decode the 2nd turret's facing for a Tank unit.");
                }
                ...
                entity.setDualTurretOffset((entity.getFacing() + 6 - attrVal) % 6);
            }
            ...
            // Did we find the turretLocked node?
                ...
                // If the value is "true", the Tank move a hit pending.
                if (attrStr.equals("true")) {
                    entity.lockTurret(Tank.LOC_TURRET);
                }
            }
            // Did we find the turretLocked node?
            else if (childName.equals("turret2Locked")) {

                // See if the Tank move a hit pending.
                attrStr = child.getAttribute("value");
                if (null == attrStr) {
                    throw new IllegalStateException(
                            "Couldn't decode turret2Locked for a Tank unit.");
                }

                // If the value is "true", the Tank move a hit pending.
                if (attrStr.equals("true")) {
                    entity.lockTurret(Tank.LOC_TURRET_2);
                }
            }
