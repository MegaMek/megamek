...
public class AddBotCommand extends ClientCommand {
    ...
    public AddBotCommand(Client client) {
        super(client, "replacePlayer", "Replaces a player who is a ghost with a bot. Usage #replacePlayer name, to replace the player named name. they must be a ghost.");
    }
    ...
    public String run(String[] args) {
        ...
        Player target = null;
        for (Enumeration<Player> i = client.game.getPlayers(); i.hasMoreElements();) {
        ...
        if(target.isGhost()) {
            BotClient c = new TestBot(target.getName(), client.getHost(), client.getPort());
