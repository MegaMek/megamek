...
/**
 * @author dirk
 * This command exists to print tile information to the chat window, it's primarily intended for vissually impaired users.
 */
    ...
    public String run(String[] args) {
        try {
            ...
            do {        
                ...
                if(hex != null) {
                    str = "Details for hex (" + (coord.x+1) + ", " + (coord.y+1) + ") : " + hex.toString();
                    
                    //if we are not playing in double blind mode also list the units in this tile.
                    if(!client.game.getOptions().booleanOption("double_blind")) {
                        Enumeration<Entity> entList = client.game.getEntities(coord);
                        if(entList.hasMoreElements()) {
                            str = str + "; Contains entities: " + entList.nextElement().getId();
                            while(entList.hasMoreElements()) {
                                str = str + ", " + entList.nextElement().getId();
                            }
                        }
                    }
                   ...
                   report = report + "Hex (" + (coord.x+1) + ", " + (coord.y+1) + ") is not on the board.\n";
                }
