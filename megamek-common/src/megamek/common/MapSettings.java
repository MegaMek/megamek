...
public class MapSettings implements Serializable {
    ...
    public void load(InputStream is) {
        ...
        Element rss = root.getRootElement();
        List<Element> rootChildren = rss.getChildren();
        Element enivoronmentNode = rootChildren.get(0);

        if (enivoronmentNode.getName().equals("ENVIRONMENT")) { //$NON-NLS-1$
        	List<Element> children = enivoronmentNode.getChildren();

            for(Element child : children) {
                try {
                    parseEnvironmentNode(child);
                } catch (Exception ex) {
    ...
    private void parseEnvironmentNode(Element node) {
        List<Element> values = node.getChildren();
        if (values.isEmpty()) {
            return;
        }
        Element value = values.get(0);
        String param = value.getValue();
        if (null == param) {
            return;
        }
    ...
    void loadStringParameter(Element node, String param, String value) {
        if (node.getName().equals(param)) {
            value = node.getValue();
        }
    }

    void loadIntParameter(Element node, String param, Integer value) {
        if (node.getName().equals(param)) {
