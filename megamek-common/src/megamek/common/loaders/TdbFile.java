...
public class TdbFile implements IMechLoader {
    ...
    public TdbFile(InputStream is) throws EntityLoadingException {
        ...
        // science teacher once told me. -- Anon
        
        // And yet, here we do it... --Kurios
        isRearMounted = new boolean[256];
        isSplit = new boolean[256];

        critData = new String[8][12][2];
        parseNode( root.getChildren().get(0));
    }

    private void parseNode(Element node) throws EntityLoadingException {
    ...
    private void parseCreatorNode(Element node) throws EntityLoadingException {
        if (!(node.getCType() == CType.Element)) {
            // We only want to parse element nodes, text nodes
        ...
        if (node.getName().equals(NAME)) {
            creatorName = children.get(0).getValue();
        } else if (node.getName().equals(VERSION)) {
            creatorVersion = children.get(0).getValue();
        } else if (children != null) {
            // Use recursion to process all the children
            for (Element child : children){
                parseNode(child);
    ...
    private void parseBasicsNode(Element node) throws EntityLoadingException {
        if (!(node.getCType() == CType.Element)) {
            // We only want to parse element nodes, text nodes
            // are directly parsed below.
            return;
        ...
        if (node.getName().equals(NAME)) {
            ...
            model = children.get(0).getValue();
        } else if (node.getName().equals(OMNI)) {
            isOmni = children.get(0).getValue().equals(TRUE_LOWER);
        } else if (node.getName().equals(VARIANT)) {
            variant = children.get(0).getValue();
        } else if (node.getName().equals(TECHNOLOGY)) {
            techBase = children.get(0).getValue();
            rulesLevel = node.getAttributeValue(LEVEL);
        } else if (node.getName().equals(TONNAGE)) {
            tonnage = children.get(0).getValue();
        } else if (node.getName().equals(TYPE)) {
            chassisConfig = children.get(0).getValue();
            ...
            // a LAM (Land Air Mech).
            LAMTonnage = node.getAttributeValue(TONNAGE);
            ...
            jumpMP = children.get(0).getValue();
        } else if (node.getName().equals(HEAT_SINKS)) {
            if (children.get(0).getValue().indexOf(
                    DOUBLE) != -1) {
                dblSinks = true;
            } else {
                dblSinks = false;
            }
            heatSinks = node.getAttributeValue(COUNT);
        } else if (node.getName().equals(ARMOR)) {
            armorType = children.get(0).getValue();
        } else if (node.getName().equals(ENGINE)) {
            engineType = children.get(0).getValue();
            engineRating = Integer.parseInt(node.getAttributeValue(RATING));
        } else if (node.getName().equals(STRUCTURE)) {
            structureType = children.get(0).getValue();
        } else if (node.getName().equals(GYRO)) {
            gyroType = children.get(0).getValue();
        } else if (node.getName().equals(COCKPIT)) {
            cockpitType = children.get(0).getValue();
        } else if (node.getName().equals(TARGSYS)) {
            targSysStr = children.get(0).getValue().trim();
    ...
    private void parseMountedNode(Element node) throws EntityLoadingException {
        if (!(node.getCType() == CType.Element)) {
            // We only want to parse element nodes, text nodes
            // are directly parsed below.
            return;
        ...
        if (node.getName().equals(MOUNTED_ITEM)) {
            if (node.getAttribute(REAR_MOUNTED).equals(TRUE)) {
                isRearMounted[Integer.parseInt(node.getAttributeValue(ITEM_INDEX))] = true;
            } else {
                isRearMounted[Integer.parseInt(node.getAttributeValue(ITEM_INDEX))] = false;
            }
            if (node.getAttribute(IS_SPREAD).equals(TRUE)) {
                isSplit[Integer.parseInt(node.getAttributeValue(ITEM_INDEX))] = true;
            } else {
                isSplit[Integer.parseInt(node.getAttributeValue(ITEM_INDEX))] = false;
            }
        } else if (children != null) {
            // Use recursion to process all the children
        	for (Element child : children){
    ...
    private void parseCritNode(Element node) throws EntityLoadingException {
        if (!(node.getCType() == CType.Element)) {
            // We only want to parse element nodes, text nodes
            // are directly parsed below.
            return;
        ...
        if (node.getName().equals(LOCATION)) {
            ...
            int armor = -1;
            int rearArmor = -1;
            if (node.getAttributeValue(ARMOR) != null) {
                armor = Integer.parseInt(node.getAttributeValue(ARMOR));
            }
            if (node.getAttributeValue(REAR_ARMOR) != null) {
                rearArmor = Integer.parseInt(node.getAttributeValue(REAR_ARMOR));
            }
            if (node.getAttributeValue(NAME).equals("LA")
                    || node.getAttributeValue(NAME).equals("FLL")) {
                loc = Mech.LOC_LARM;
                larmArmor = armor;
            } else if (node.getAttributeValue(NAME).equals("RA")
                    || node.getAttributeValue(NAME).equals("FRL")) {
                loc = Mech.LOC_RARM;
                rarmArmor = armor;
            } else if (node.getAttributeValue(NAME).equals("LT")) {
                loc = Mech.LOC_LT;
                ltArmor = armor;
                ltrArmor = rearArmor;
            } else if (node.getAttributeValue(NAME).equals("RT")) {
                loc = Mech.LOC_RT;
                rtArmor = armor;
                rtrArmor = rearArmor;
            } else if (node.getAttributeValue(NAME).equals("CT")) {
                loc = Mech.LOC_CT;
                ctArmor = armor;
                ctrArmor = rearArmor;
            } else if (node.getAttributeValue(NAME).equals("H")) {
                loc = Mech.LOC_HEAD;
                headArmor = armor;
            } else if (node.getAttributeValue(NAME).equals("LL")
                    || node.getAttributeValue(NAME).equals("RLL")) {
                loc = Mech.LOC_LLEG;
                llegArmor = armor;
            } else if (node.getAttributeValue(NAME).equals("RL")
                    || node.getAttributeValue(NAME).equals("RRL")) {
                loc = Mech.LOC_RLEG;
                rlegArmor = armor;
            }
            ...
            for(Element child : children) {
                Element critSlotNode =  child;
                critData[loc][i][0] = (critSlotNode.getChildren().get(0).getText());
                if (clanTC && critData[loc][i][0].equals("Targeting Computer")) {
                    critData[loc][i][0] = "(C) " + critData[loc][i][0];
                }
                critData[loc][i++][1] = critSlotNode.getAttributeValue(ITEM_INDEX);
            }
        } else if (children != null) {
            // Use recursion to process all the children
        	for (Element child : children){
