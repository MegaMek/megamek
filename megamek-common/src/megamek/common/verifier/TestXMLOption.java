...
public class TestXMLOption implements TestEntityOption {
    ...
    private static float getContentAsFloat(Element node) {
        if (!node.getChildren().isEmpty()) {
    ...
    private static boolean getContentAsBoolean(Element node) {
        if (!node.getChildren().isEmpty()) {
            return Boolean.getBoolean(node.getChildren().get(0).getValue());
        }
    ...
    private static int getContentAsInteger(Element node) {
        if (!node.getChildren().isEmpty()) {
    ...
    public void readXMLOptions(Element node) {
    ...
    private void readCeilWeight(Element node) {
        for (Element child : node.getChildren()) {
