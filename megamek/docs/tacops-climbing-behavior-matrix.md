...
Player must use jump, route around, or stay put for cliffs/tall buildings. Climb mode is purely the legacy
building/bridge toggle.
...
Tooltip on hover explains the active effect (e.g., *"At Edge — Climb Mode toggle has no effect for descent —
Dangle/Drop/Cancel dialog appears either way."*).
...
Climb cost per level: **2 MP** with two functional climbing arms, **3 MP** with one
*(`ClimbingHelper.MP_COST_TWO_HANDS` / `MP_COST_ONE_HAND`)*.

A "functional climbing arm" requires all four actuators intact (shoulder, upper arm, lower arm, hand), free hand (no
carried object), and **no club/hatchet/sword** mounted in that arm. A Mek variant with a sword loses one arm's
climbability.
...
Drop from a dangle position reduces both modifiers by `DANGLE_LEVELS_PER_TURN` (= 2), reflecting the lower effective
drop height.
