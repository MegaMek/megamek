...
Equipment von Level 1 (3025), sowie beinahe alle Level 1 Regeln sind
enthalten. Wenn das alles fertig ist, wird die Arbeit an den Level 2 Regeln
und dem Equipment beginnen.
...
und den rest muss sich mit diesem Server verbinden. Die verbindenden Spieler
können von überall her mit der TCP/IP Verbindung zum Server connecten,
inklusive des selben Computer auf dem der Server läuft.(Schaut in Hotseat, unten.)

Um einen Server zu starten, klickt man auf den "Host a New Game..." Button
im Main Menü.  Bestimmt euren Namen und klickt dann auf "Okay". Wenn dann
der Lobby Bildschirm auftaucht, können die anderen Spieler connecten, und
das Spiel kann beginnen. Die anderen Spieler müssen die IP Adresse des
Servers kennen um sich mit ihm zu verbinden. Solltet ihr eure derzeitige IP
Adresse nicht kennen, gibt es da ein paar Internet Homepages, wie zum
Beispiel www.whatismyip.com wo sie euch gezeigt wird. Oder unter Windows,
ins Startemenü gehen, und dann bei "AUSFÜHREN" WINIPCFG" eingeben.
...
wollen, und in der Spalte "Server Address" im Verbindungs Fernster gibt
an, sofern es dort nicht bereits steht, "localhost" ein. Jeder Spieler muss 
dann zu seinem Fenster/Spiel von MEGAMEK wechseln um seinen Zug zu machen.
...
man benötigt um den Zug durch zu führen. GRÜNE Farbe zeigt den GEHEN-Bereich
an, GELB den LAUFEN-Bereich und GRAU den Bereich in den man Springen kann.
ROTE Bereich zeigen an, das man seine Reichweite überschritten hat, oder
...
und dann das gewünschte Feld oder die gewünschte Richtung. Man kann nicht aufstehen und
springen im selben Zug.

Aufladen und TOD-VON-OBEN (D.F.A.) sind angriffe die während der Bewegungsphase
hemacht werden müssen. Man Klickt auf den "Charge" oder "D.F.A." Knopf und klickt
dann auf die Einheit die man angreifen will. Wenn der Angriff durch geführt werden kann,
wird es sofort ausgeführt, und der Zug ist beendet. Das Ergebnis wird dann bei den
physischen Anngriffen aufgeführt. Sollte MEGAMEK nicht denn gewünschten Pfad benutzen, 
kann man selber den Pfad auswehlen, indem man einen eigenen Pfad in der nähe des Ziels
...
so werden die Ergebnisse in einem Bericht gezeigt.


WAFFEN-FEUER PHASE:

Der Knopf am rechten unteren Rand erlaubt euch, eure Waffen abzufeuern,
eine andere Einheit auszuwählen, oder um euren derzeitigen Schuss zu
bestätigen. Nochmal, ihr müsst nicht mit der als erstes gewählten Einheit
feuern.

Um auf eine andere Einheit zu zielen, klickt man sich auf dem Spielfeld an.
Die Zielinformationen über die grade gewählte Waffe werden dann im MECHINFORMATIONSFELD
angezeigt. Um die gewählte Waffe aufs Ziel abzufeuer, drückt man den "FIRE" Knopf.
Um zu einer anderen Waffe zu wechseln, klickt auf den Namen der Waffe in der Liste im
MECHINFORMATIONSFELD. So kann man sein Feuer auf verschiedene Ziele verteilen, indem
man diese Schritte wiederholt. Wenn alle Waffen auf ihre Ziele ausgerichtet sind, bzw
nicht abgefeuert werden können, drückt man den "DONE" Knopf.
...
Blickrichtung klickt. Die gewählte Einheit wird nun versuchen den Torso in die
...
(PUNCH/KICK), so werden hier die Ergebnisse gezeigt. Zusätlich
zu den Resultaten der Hitze.

Nach dieser Phase beginnt das Ziehen von vorn.
...
- Du kannst nur am Anfang deines Zuges aufstehen. Sollteste du während
deines Zuges hin fallen, liegst du für den Rest der Runde auf dem Boden.
...
muss, Derzeit wählt MEGAMEK einfach willkürlich ein Hex-Feld aus. Das soll
in der zukunft noch geändert werden, um die Verteidiger zu unterstützen.
...
bestimmt derzeit diese Feld noch selbst für sie.
...
MEGAMEK STARTEN
---------------
Java Programme laufen in ihrer eigenen Umgebung, die man Virtual Machine
oder kurz VM nennt. Diese Java Vms sind auf den meisten Systemen bereits
vorhanden.

Windows Benutzer: Um MEGAMEK zu starten, müssen sie MegaMek.exe amklicken.

Andere grafische OS: Viele andere grafische OS, solche wie MacOS und OSX,
geben dir sie Möglichkeit die *.jar Dateien direkt anzuklicken um sie zu
staretn. Sollte dies nicht Funktionieren, versucht MEGAMEK über die
Befehlszeile zu starten.

MEGAMEK über die Befehlsleiste starten: Um dies mit Sun Java, oder einem
anderen Program zu tun, geht in das Verzeichnis, in dem die *.jar Dateien
enthalten sind und gebt dann ein:

java -jar MegaMek.jar

Sollte eure Java VM *.jar Dateinen nicht unterstützt, die the fertigen Java
.class Dateien sind im "classes" Verzeichnis enthalten. Ihr müsst dann denn
Class Pfad zu diesem Verzeichnis ändern, und dann Megamek.class starten.
Um dies mit Sun Java zu tun:

java -cp classes megamek/MegaMek



INSTALLIEREN ODER UPDATEN SEINER JAVA RUNTIMES
----------------------------------------------
Sollte dein System kein Java VM besitzen, oder du ein Update deines Java VMs
brauchst, hast du hier ein paat Links dazu.
...
Java Updates sind in den OS Updates für 10.1 und höhere Versionen enthalten.
Es gibt sonst keinen Weg dein Java VM upzudaten.
...
die contact Page auf der MEGAMEK Webseite, oder schickt eine E-Mail an
bmazur@sev.org
