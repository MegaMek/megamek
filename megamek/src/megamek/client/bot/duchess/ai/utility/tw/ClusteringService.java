...
public class ClusteringService {
    ...
    private final Map<Integer, Coords> elementToCentroid = new HashMap<>();
            ...
            for (Entity elem : teamElements) {
                if (!visited.contains(elem.getId())) {
                    ...
                    for (List<Entity> subClusterElements : splitClusters) {
                        ...
                        for (Entity scElem : subClusterElements) {
                            elementToCentroid.put(scElem.getId(), new Coords((int) Math.round(cx), (int) Math.round(cy)));
    ...
    public Coords getClusterMidpoint(Entity e) {
        return elementToCentroid.getOrDefault(e.getId(), null);
    }
