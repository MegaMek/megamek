...
public class BasicPathRanker extends PathRanker {
    ...
    protected double calculateFacingMod(Entity movingUnit, Game game, final MovePath path, @Nullable Coords closestPosition) {
        // If the unit is withdrawing then it should be facing the edge it is moving towards
        BehaviorType behaviorType = getOwner().getUnitBehaviorTracker().getBehaviorType(movingUnit, getOwner());
        if (behaviorType == BehaviorType.ForcedWithdrawal && getOwner().getBehaviorSettings().isIgnoreDamageOutput()) {
            closestPosition = switch (getOwner().getHomeEdge(movingUnit)) {
                case NORTH -> new Coords(path.getFinalCoords().getX(), 0);
                case EAST -> new Coords(game.getBoard().getWidth() - 1, path.getFinalCoords().getY());
                case SOUTH -> new Coords(path.getFinalCoords().getX(), game.getBoard().getHeight() - 1);
                case WEST -> new Coords(0, path.getFinalCoords().getY());
                default -> game.getBoard().getCenter();
            };
        }
