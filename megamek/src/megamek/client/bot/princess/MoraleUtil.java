 ...
 * @version: %Id%
 */
public class MoraleUtil implements IMoraleUtil {

    private static final DecimalFormat DEC_FORMAT = new DecimalFormat("0.00");

    private final Set<Integer> brokenUnits = ConcurrentHashMap.newKeySet();
    ...
    public boolean isUnitBroken(int unitId) {
        return brokenUnits.contains(unitId);
    }

    @Override
    public void checkMorale(boolean forcedWithdrawal, int bravery, int selfPreservation, IPlayer player, IGame game) {
        ...
        try {
            ...
            for (Entity unit : game.getPlayerEntities(player, true)) {
                ...
                boolean rally = brokenUnits.contains(unitId);
            ...
            logger.log(getClass(), "checkMorale(boolean, int, int, IPlayer, IGame)", LogLevel.INFO, logMsg);
        }
    }
    ...
    private int calcBvRatioMod(IPlayer player, IGame game, StringBuilder logMsg) {
        ...
        for (Entity entity : allEntities) {
            ...
            } else if (!brokenUnits.contains(entity.getId())) {
