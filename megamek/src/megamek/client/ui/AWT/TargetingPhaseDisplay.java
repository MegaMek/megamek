...
public class TargetingPhaseDisplay extends StatusBarPhaseDisplay implements
    ...
    public TargetingPhaseDisplay(ClientGUI clientgui, boolean offboard) {
        ...
        this.phase = offboard ? IGame.Phase.PHASE_OFFBOARD : IGame.Phase.PHASE_TARGETING;
        shiftheld = false;
        ...
        butFlipArms = new Button(Messages
        ...
        butFireMode = new Button(Messages
        ...
        butSearchlight = new Button(Messages
    ...
    /**
     * Have the panel register itself as a listener wherever it's needed. <p/>
     * According to
    ...
    private void beginMyTurn() {
        ...
        if (client.game.getTurn() instanceof GameTurn.TriggerAPPodTurn
            ...
            ready();
                ...
                && null != ce()) {
            disableButtons();
            TriggerBPodDialog dialog = new TriggerBPodDialog(clientgui.getFrame(), ce(), ((GameTurn.TriggerBPodTurn)client.game.getTurn()).getAttackType());
            dialog.setVisible(true);
            attacks.removeAllElements();
            Enumeration<EntityAction> actions = dialog.getActions();
            while (actions.hasMoreElements()) {
     ...
     * button. We'll sort it by range to us.
     */
