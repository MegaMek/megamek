...
public class MovementDisplay extends ActionPhaseDisplay {
     ...
     */
    private enum BridgeSelectionStage {
        NONE,
        SECTION,
        DIRECTION
    }
     ...
     */
    private void currentMoveHoldingShift(Coords target,
                                         int finalBoardId) {
        shiftHeld = true;
    ...
    private void cancel() {
        ...
        if ((currentEntity != null) && currentEntity.isDeployed()) {
            computeMovementEnvelope(currentEntity);
     ...
     */
    private boolean infantryGroundPostureAvailable(Entity unit,
    ...
    private void addStepsToMovePath(boolean noCost,
                                    boolean isManeuver,
                                    int maneuverType,
                                    MoveStepType... moveSteps) {
        for (MoveStepType moveStep : moveSteps) {
            cmd.addStep(moveStep, noCost, isManeuver, maneuverType);
        }
        updateMove();
            ...
            if (ClimbingHelper.isAtEdge(currentEntity, stepPos, game)) {
                ...
                if (game.getOptions().booleanOption(
                    ...
                    descentOptions.add(new ClimbingChoiceDialog.ClimbingOption(levelDiff,
                                                                               ...
                                                                               Messages.getString(
                                                                                       "MovementDisplay.ClimbingDialog.dropOption",
                                                                                       ClimbingHelper.DROP_MP_COST),
                ...
                descentOptions.add(new ClimbingChoiceDialog.ClimbingOption(0, 0,
                                                                           Messages.getString(
                                                                                   "MovementDisplay.ClimbingDialog.walkAsCalculatedOption"),
                ...
                descentOptions.add(new ClimbingChoiceDialog.ClimbingOption(0, 0,
                                                                           Messages.getString(
                                                                                   "MovementDisplay.ClimbingDialog.cancelEdgeOption"),
    ...
    private boolean checkNags() {
        ...
        if (needNagForOther()) {
            if ((currentlySelectedEntity != null) && (cmd != null) && currentlySelectedEntity.isAero()) {
    ...
    private void updateRecklessButton() {
        ...
        if (currentEntity instanceof ProtoMek) {
            ...
            setRecklessEnabled((cmd == null) || (pathZeroOrDeploy()));
        }
    }
    ...
    private void updateManeuverButton() {
        ...
        setManeuverEnabled((cmd == null) || !cmd.contains(MoveStepType.MANEUVER));
    }
    ...
    private void updateLoadButton() {
        ...
        if ((currentEntity.getWalkMP() <= 0 && !currentEntity.isAerospace()) || (
        ...
        Vector<Entity> candidates = new Vector<>();
    ...
    private void updateMountButton() {
        ...
        final List<Entity> mountableUnits = isFinalPositionOnBoard()
              ? Compute.getMountableUnits(movingEntity, pos, finalBoardId(),
                    elev + game.getBoard(movingEntity).getHex(pos).getLevel(),
                    game) : List.of();
        // Compile against the actual carrier: a building's flight deck can stow units that require cranes elsewhere.
        final MovePath path = cmd != null ? cmd : new MovePath(game, movingEntity);
        final boolean canMount = !movingEntity.isAirborne() && mountableUnits.stream().anyMatch(carrier ->
              path.clone().addStep(MoveStepType.MOUNT, carrier).getLastStepMovementType()
                    != EntityMovementType.MOVE_ILLEGAL);
    ...
    private @Nullable Entity getMountedUnit() {
        ...
        MovePath path = cmd != null ? cmd : new MovePath(game, currentEntity);
        List<Entity> mountableUnits = Compute.getMountableUnits(currentEntity, pos, finalBoardId(), elev, game).stream()
              .filter(carrier -> path.clone().addStep(MoveStepType.MOUNT, carrier).getLastStepMovementType()
                    != EntityMovementType.MOVE_ILLEGAL)
              .toList();
        ...
        if (mountableUnits.isEmpty()) {
            ...
            // Only one choice.
            choice = mountableUnits.getFirst();
        }
    ...
    private Entity getTowedUnit() {
        ...
        if (hitchChoices.size() > 1) {
            ...
            choice.setTargetBay(hitchChoices.getFirst().getNumber());
            choice.setTowedBy(hitchChoices.getFirst().getId());
        }
    ...
    private Entity getDisconnectedUnit() {
        ...
        if (currentEntity.getAllTowedUnits().isEmpty()) {
            ...
            // Only one choice.
            choice = towedUnits.getFirst();
            towedUnits.removeFirst();
        }
    ...
    private List<Coords> unloadPositions(Entity unloaded) {
        ...
        if (currentEntity instanceof MobileStructure mobile) {
            var pose = new megamek.common.moves.MobileStructureLinkage.Pose(cmd.getFinalCoords(), cmd.getFinalFacing(),
                  cmd.getFinalElevation());
            return MobileStructureCargoRules.exits(mobile, unloaded, pose).stream()
    ...
    private @Nullable Coords getUnloadPosition(Entity unloaded) {
        return chooseUnloadPosition(currentEntity(), unloadPositions(unloaded));
    }
    ...
    private void computeSimpleMovementEnvelope(Entity suggestion) {
        ...
        // This guards against a null position. Go to suggestion if it is null.
        if (entity.getPosition() == null) {
            entity = suggestion;
        }
        // If the suggestion position is null, exit.
        if (suggestion.getPosition() == null) {
            return;
        }
    ...
    public synchronized void actionPerformed(ActionEvent ev) {
        ...
        if (actionCmd.equals(MoveCommand.MOVE_FORWARD_INI.getCmd())) {
            ...
            switch (entity) {
                case QuadVee ignored -> adjustConvertSteps(EntityMovementMode.QUAD);
                case TripodMek ignored -> adjustConvertSteps(EntityMovementMode.TRIPOD);
                case BipedMek ignored -> adjustConvertSteps(EntityMovementMode.BIPED);
                default -> {
                }
            }
            ...
            if ((mf != null) && clientgui.doYesNoDialog(title, body)) {
            ...
            if (options.size() == 1) {
                addStepToMovePath(MoveStepType.BRACE, options.getFirst());
            ...
            if (waitingForCarrier != null) {
                ...
                ready();
                  ...
                  && megamek.common.units.BuildingFlightDeckRules.onDeck(currentEntity()) == null) {
        ...
        if (actionCmd.equals(MoveCommand.MOVE_RAISE_ELEVATION.getCmd())) {
            ...
            if (currentEntity().isAero() && (((IAero) currentEntity()).hasRoomForHorizontalTakeOff()) != null) {
                clientgui.addToast(ToastLevel.ERROR,
    ...
    private void processPickupCargoCommand() {
        ...
        if (displayedOptions.size() == 1) {
            Integer pickupLocation = getPickupLocation(displayedOptions.getFirst());

            if (pickupLocation != null) {
                Map<Integer, Integer> data = new HashMap<>();
                // we pick the only eligible object out of all the objects on the ground
                data.put(MoveStep.CARGO_PICKUP_KEY, options.indexOf(displayedOptions.getFirst()));
     ...
     */
    private boolean addRepairPlansForGap(List<BridgeBuildPlan> plans,
                                         Board board,
