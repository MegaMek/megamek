...
public class FiringDisplay extends AttackPhaseDisplay implements ItemListener, ListSelectionListener {
    ...
    public void selectEntity(int en) {
        ...
        if (clientgui.getClient().getGame().getEntity(en) != null) {
            ...
            setFindClubEnabled(FindClubAction.canMekFindClub(clientgui.getClient().getGame(), en));
    ...
    protected void changeMode(boolean forward) {
        ...
        if ((ce() instanceof BattleArmor)
                ...
                && m.getType().hasFlag(EquipmentFlag.F_BA_INDIVIDUAL)
    ...
    private void updateStrafingTargets() {
        ...
        for (Coords c : strafingCoords) {
            for (Entity t : game.getEntitiesVector(c)) {
                ...
                toHitBuff.append("\n");
                if (m.getType().hasFlag(EquipmentFlag.F_AUTO_TARGET)
                        || (toHit.getValue() == TargetRoll.IMPOSSIBLE)) {
                    setFireEnabled(false);
                }
            }
    ...
    void fire() {
        ...
        for (Targetable t : targets) {
            ...
            if (mounted.getType().hasFlag(EquipmentFlag.F_SPACE_BOMB)) {
                waa.setBombPayloads(getBombPayloads(true, -1));
            } else if (mounted.getType().hasFlag(EquipmentFlag.F_DIVE_BOMB)) {
                waa.setBombPayloads(getBombPayloads(false, -1));
            } else if (mounted.getType().hasFlag(EquipmentFlag.F_ALT_BOMB)) {
        ...
        if (!mounted.getType().hasFlag(EquipmentFlag.F_VGL) && (nextMounted != null)
                && nextMounted.getType().hasFlag(EquipmentFlag.F_VGL)) {
    ...
    public void target(Targetable t) {
        ...
        if ((weapon != null) && weapon.getType().hasFlag(EquipmentFlag.F_VGL)) {
    ...
    public void updateTarget() {
        ...
        if (isStrafing && weaponId != -1) {
            ...
            // update ranges.
            if (m.getType().hasFlag(EquipmentFlag.F_CWS)) {
            ...
            if (m.isUsedThisRound()) {
                ...
                setFireEnabled(false);
            } else if ((m.getType().hasFlag(EquipmentFlag.F_AUTO_TARGET)
            ...
            if (isStrafing) {
                ...
                if (shiftheld) {
                    ...
                    if ((targ instanceof Entity) && Compute.isGroundToAir(ce(), targ)) {
                        ...
                        if (!alreadyShotAt) {
                            entTarg.setPlayerPickedPassThrough(currentEntity, evtCoords);
                        }
                    }
                    target(targ);
                }
    ...
    protected void updateSearchlight() {
        setSearchlightEnabled(
                (ce() != null)
                        ...
                        && SearchlightAttackAction.isPossible(clientgui.getClient().getGame(), currentEntity, target,
                                null)
