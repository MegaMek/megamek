...
public class MovementDisplay extends ActionPhaseDisplay {
    ...
    public enum MoveCommand implements PhaseCommand {
        ...
        // Convert command for a single button, which can cycle through modes because
        // MovePath state is available
         ...
         *
         * @param f          The unit flag to specify what unit type the commands are for.
    ...
    private MegaMekButton getBtn(MoveCommand moveCommand) {
        return buttons.get(moveCommand);
    }

    private boolean isEnabled(MoveCommand moveCommand) {
        return buttons.get(moveCommand).isEnabled();
    }
     ...
     * if configured.
     *
     * @param entity Current {@link Entity}
     */
    ...
    private void updateButtons() {
        ...
        setJumpEnabled(!isAero &&
                             !ce.isImmobileForJump() &&
                             !ce.isProne()
                             // Conventional infantry also uses jump MP for VTOL and UMU MP
                             &&
                             (hasJumpMP() && (!ce.isConventionalInfantry() || ce.getMovementMode().isJumpInfantry())) &&
        ...
        getBtn(MoveCommand.MOVE_CLIMB_MODE).setEnabled(Stream.of(EntityMovementMode.HYDROFOIL,
                    ...
                    EntityMovementMode.QUAD_SWIM)
                                                             .noneMatch(entityMovementMode -> (ce.getMovementMode() ==
                                                                                                     entityMovementMode)));
        updateTurnButton();
        ...
        if (isInfantry && ce.hasWorkingMisc(MiscType.F_TRENCH_CAPABLE)) {
            ...
            getBtn(MoveCommand.MOVE_FORTIFY).setEnabled(isTank && ce.hasWorkingMisc(MiscType.F_TRENCH_CAPABLE));
        }
        ...
        getBtn(MoveCommand.MOVE_CALL_SUPPORT).setEnabled(isInfantry &&
                                                               ce.hasAbility(OptionsConstants.INFANTRY_URBAN_GUERRILLA) &&
                                                               ((Infantry) ce).getCanCallSupport());

        getBtn(MoveCommand.MOVE_SHAKE_OFF).setEnabled((ce instanceof Tank) && (ce.getSwarmAttackerId() != Entity.NONE));
        ...
        // If a Small Craft / DropShip that has unloaded units, then only allowed to
        // unload more
    ...
    private void updateFleeButton() {
        ...
        boolean fleeStart = !hasLastStep && ce().canFlee(ce().getPosition());
    ...
    private boolean checkNags() {
        ...
        if (needNagForNoAction()) {
            if ((ce() != null) && (cmd.length() == 0) && !ce().isAirborne()) {
        ...
        if (needNagForNoUnJamRAC()) {
            if ((ce() != null) && ce().canUnjamRAC() && !isUnJammingRAC) {
                // confirm this action
        ...
        if (needNagForMASC()) {
            if ((ce() != null) && cmd.hasActiveMASC() && !(ce() instanceof VTOL)) {
        ...
        if (needNagForMASC()) {
            if ((ce() != null) && !(ce() instanceof VTOL) && cmd.hasActiveSupercharger()) {
                String title = Messages.getString("MovementDisplay.areYouSure");
                String body = Messages.getString("MovementDisplay.ConfirmSuperchargerRoll",
                      ce().getSuperchargerTarget());
                if (checkNagForMASC(title, body)) {
                    return true;
                }
            }
        ...
        if (needNagForPSR()) {
            ...
            if ((ce() != null) && verticalTakeoffOrTakeoff) {
        ...
        // they will do so if they're not hovering, landing, or changing altitude
        // voluntarily.
        if (needNagForPSR()) {
            if ((ce() != null) &&
        ...
        if (needNagForOther()) {
            if ((ce() != null) && (null != cmd) && ce().isAero()) {
                ...
                if (null != cmd.getLastStep()) {
                    ...
                    unusedVelocity = (((IAero) ce()).getCurrentVelocity() > 0) && (ce().delta_distance == 0);
                }
        ...
        if (needNagForOther()) {
            if (ce() != null && (ce() instanceof Infantry) && ((Infantry) ce()).hasMicrolite()) {
                ...
                int terrainLevelBuilding = ce().getGame()
                                                 .getBoard()
                                                 .getHex(cmd.getFinalCoords())
                                                 .terrainLevel(Terrains.BLDG_ELEV);
                int terrainLevelBridge = ce().getGame()
                                               .getBoard()
                                               .getHex(cmd.getFinalCoords())
    ...
    public synchronized void ready() {
        ...
        if (ce().isAirborne() || ce().isSpaceborne()) {
            ...
            // Aerospace might need to have additional move steps tacked on
    ...
    public synchronized void hexMoused(BoardViewEvent b) {
        ...
        boolean noPath = (ce != null && ce.isAero()) && clientgui.getClient().getGame().useVectorMove();
        ...
        Coords currPosition = cmd != null ? cmd.getFinalCoords() : ce != null ? ce.getPosition() : null;

        if ((b.getType() == BoardViewEvent.BOARD_HEX_DRAGGED) && !noPath) {
            if (!b.getCoords().equals(currPosition) || shiftHeld || (gear == MovementDisplay.GEAR_TURN)) {
            ...
            if (gear == MovementDisplay.GEAR_RAM) {
                ...
                if ((target == null) || target.equals(ce) || !target.isAero()) {
                ...
                if (ce != null && ce.isAero() && clientgui.getClient().getGame().useVectorMove()) {
                    cmd.clipToPossible();
                }
                ...
                clear();
                return;
            } else if (ce != null && gear == MovementDisplay.GEAR_CHARGE) {
                // check if target is valid
                final Targetable target = chooseTarget(b.getCoords());
                if ((target == null) || target.equals(ce)) {
                    clientgui.doAlertDialog(Messages.getString(ce.isAirborneVTOLorWIGE() ?
                ...
                if (toHit.getValue() != TargetRoll.IMPOSSIBLE) {
                    ...
                    if (ce.isAirborneVTOLorWIGE()) {
                        ...
                        toDefender = ChargeAttackAction.getDamageFor(ce,
                              clientgui.getClient()
                                    .getGame()
                                    .getOptions()
    ...
    private synchronized void updateProneButtons() {
        ...
        if (cmd.getFinalProne()) {
            ...
            if (!(ce instanceof Tank) &&
                      !(ce instanceof QuadVee && ce.getConversionMode() == QuadVee.CONV_MODE_VEHICLE)) {
    ...
    private synchronized void updateElevationButtons() {
        ...
        // WiGEs (and LAMs and glider ProtoMeks) cannot go up if they've used ground
        // movement.
        if (ce.getMovementMode().isWiGE() &&
    ...
    private synchronized void updateLandButtons() {
        ...
        if ((null != cmd) && (cmd.length() > 0)) {
            // According to the link below, you can move in the air and then land, but I have personal message to
            // Welshman right now asking that this be changed because it creates all kinds of rules problems, the
            // number one being the ability to use spheroid dropships to perform instant-Death From Above attacks that
            // cannot be defended against So we are going to disallow it
            // http://www.classicbattletech.com/forums/index.php?topic=54112.0
            setLandEnabled(false);
            return;
        }
    ...
    private synchronized void updateSpeedButtons() {
        ...
        int velocityNext = a.getNextVelocity();
        if (null != last) {
            currentVelocity = last.getVelocity();
            velocityNext = last.getVelocityN();
        }
        ...
        if (!clientgui.getClient().getGame().getBoard().inSpace() && (velocityNext == (4 * ce.getWalkMP()))) {
            setAccNEnabled(false);
        }
    ...
    private void updateConvertModeButton() {
        // Issue #5280 NPE - make sure the move path is valid and the last step isn't
        // null.
    ...
    private void updateDropCargoButton() {
        ...
        // there has to be an entity, objects are on the ground,
        // the entity can pick them up
        if ((ce == null) || ce.getCarriedObjects().isEmpty()) {
            setDropCargoEnabled(false);
            return;
        }
    ...
    private Entity getTowedUnit() {
        ...
        final class HitchChoice {
            ...
            @Override
            public String toString() {
                String entityName = (entity != null) ? entity.getShortName() : "NULL Entity";
                // the string should tell the user if the hitch is mounted front or rear
                if (getHitch().getRearMounted()) {
                    return String.format("%s Trailer Hitch #[%d] (rear)", entityName, getNumber());
                }
                return String.format("%s Trailer Hitch #[%d] (front)", entityName, getNumber());
            }
        }
        ...
        // really, there shouldn't be but one per entity. "Towing" front and rear with
        // the
     ...
     *
     * @return {@link Coords} where an {@link EjectedCrew} can be placed
     */
    private Coords getEjectPosition(Entity abandoned) {
        ...
        for (Coords coords : ring) {
            if (selected.equals(coords.toString())) {
                choice = coords;
                break;
            }
        }
    ...
    private static int getSpace(SmallCraft craft, Entity currentEntity) {
        ...
        // Passengers don't actually 'load' into bays to consume space, so update what's
        // available for anyone already aboard
        space -= (int) Math.floor((craft.getTotalOtherCrew() + craft.getTotalPassengers()) * 0.1);
    ...
    private void dumpBombs() {
        ...
        BombPayloadDialog dumpBombsDialog = new BombPayloadDialog(clientgui.frame,
    ...
    /**
     * Computes all the possible moves for an Entity in a particular gear. The Entity can either be a suggested Entity
     * or the currently selected one. If there is a selected entity (which implies it's the current players turn), then
     * the current gear is used (which is set by the user). If there is no selected entity, then the current gear is
     * invalid, and it defaults to GEAR_LAND (standard "walk forward").
     *
     * @param suggestion The suggested Entity to use to compute the movement envelope. If used, the gear will be set to
     *                   GEAR_LAND. This takes precedence over the currently selected unit.
     */
    public void computeMovementEnvelope(Entity suggestion) {
        // do nothing if deactivated in the settings
        if (!GUIP.getMoveEnvelope()) {
            // Issue #5700 : Move envelope doesn't clear when turning off move envelopes
        ...
        // Create a pathfinder to find possible moves; if aerodyne, use a custom Aero
        // pathfinder.
    ...
    /**
     * Computes possible moves for entities of Aero units. This is similar to the `computeMovementEnvelope()` method;
     * however, it uses the `MovePath` final velocity to temporarily set units velocity to draw the move envelope. This
     * method always sets the original entity velocity back to it's orginial.
     *
    ...
    public void computeModifierEnvelope() {
        ...
        if (gear == GEAR_JUMP) {
            ...
            maxMP = ce().getWalkMP();
                         ...
                         !(ce() instanceof QuadVee) &&
                         ce().getMovementMode() == EntityMovementMode.TRACKED) {
            // A non-QuadVee `Mek that is using tracked movement (or converting to it) is
            // limited to walking
            maxMP = ce().getWalkMP();
        } else {
            maxMP = ce().getRunMP();
    ...
    public synchronized void actionPerformed(ActionEvent ev) {
        ...
        if (actionCmd.equals(MoveCommand.MOVE_FORWARD_INI.getCmd())) {
            ...
            // Check for Mine-sweeping Engineers
        ...
        if (actionCmd.equals(MoveCommand.MOVE_RAISE_ELEVATION.getCmd())) {
            ...
            if ((ms != null) &&
                ...
                // on, climb step off. This affects how the StepSprite gets rendered, so it's clearer to keep a
                // climb step once one has been added
            ...
            ManeuverChoiceDialog choiceDialog = new ManeuverChoiceDialog(clientgui.frame,
    ...
    /**
     * Add enough <code>MoveStepType.CONVERT_MODE</code> steps to get to the requested mode, or clear the path if the
     * unit is in the requested mode at the beginning of the turn.
     *
     * @param endMode The mode to convert to
     */
    private void adjustConvertSteps(EntityMovementMode endMode) {
        // Since conversion is not allowed in water, we shouldn't have to deal with the
        // possibility of swim modes.
     ...
     * Bills</a>, the "minimum move" rule allow stranded units to dismount at the start of the turn.
     */
    ...
    public void unitSelected(BoardViewEvent b) {
        ...
        if (clientgui.getClient().isMyTurn()) {
            GameTurn currentTurn = clientgui.getClient().getGame().getTurn();

            if (currentTurn != null && currentTurn.isValidEntity(e, clientgui.getClient().getGame())) {
