...
public class TargetingPhaseDisplay extends StatusBarPhaseDisplay implements
    ...
    public void updateTarget() {
        ...
        if ((target != null) && (weaponId != -1)) {
            ...
            ToHitData toHit = WeaponAttackAction.toHit(clientgui.getClient().getGame(), cen, target,
                    weaponId, Entity.LOC_NONE, AimingMode.NONE, false);
            ...
            if (m.isUsedThisRound()) {
                clientgui.getUnitDisplay().wPan.wToHitR.setText(Messages.getString("TargetingPhaseDisplay.alreadyFired"));
                setFireEnabled(false);
            } else if (m.isInBearingsOnlyMode() && distance < RangeType.RANGE_BEARINGS_ONLY_MINIMUM) {
                clientgui.getUnitDisplay().wPan.wToHitR.setText(Messages.getString("TargetingPhaseDisplay.bearingsOnlyMinRange"));
                setFireEnabled(false);
            } else if ((m.getType().hasFlag(WeaponType.F_AUTO_TARGET) && !m.curMode().equals(Weapon.MODE_AMS_MANUAL))) {
                clientgui.getUnitDisplay().wPan.wToHitR.setText(Messages.getString("TargetingPhaseDisplay.autoFiringWeapon"));
                setFireEnabled(false);
