...
class MovingEntitySprite extends Sprite {
    ...
    public void drawOnto(Graphics g, int x, int y, ImageObserver observer) {
        ...
        if (bv.useIsometric() && (entity.isAirborne() || entity.isAirborneVTOLorWIGE())) {
            Image shadow = bv.createShadowMask(bv.tileManager.imageFor(entity, facing, -1));
            shadow = bv.getScaledImage(shadow, true);

            g.drawImage(shadow, x, y + (int) (bv.DROP_SHADOW_DISTANCE * bv.scale), observer);
        } else if (elevation > 0) {
            Image shadow = bv.createShadowMask(bv.tileManager.imageFor(entity, facing, -1));
            shadow = bv.getScaledImage(shadow, true);

            g.drawImage(shadow, x, y + (int) (elevation * BoardView.HEX_ELEV * bv.scale), observer);
        }
        // submerged?
        if (bv.useIsometric() && ((elevation + entity.getHeight()) < 0)) {
            Graphics2D g2 = (Graphics2D) g;
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.35f));
            g2.drawImage(image, x, y - (int) (elevation * BoardView.HEX_ELEV * bv.scale), observer);
