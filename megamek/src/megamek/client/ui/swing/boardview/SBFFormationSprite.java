...
//    private final int secondaryPos;
    ...
    public SBFFormationSprite(BoardView boardView, SBFFormation formation) {
        super(boardView);
...
//        Coords position = formation.getPosition();
...
//                case ABBREV:
//                    return (formation instanceof Mech) ? formation.getModel() : abbreviateUnitName(standardLabelName());
//                case CHASSIS:
//                    return reduceVehicleName(formation);
//                case NICKNAME:
...
//        String name = formation.getShortName();
...
//        }
//
//        int face = (formation.isCommander() && !onlyDetectedBySensors()) ? Font.ITALIC : Font.PLAIN;
...
//        int indexEntity = bv.game.getEntitiesVector(position, true).indexOf(formation);
...
//        getBounds();
        ...
        graph.drawImage(bv.getScaledImage(formation.getIcon(), true), 0, 0, this);
...
//                boolean shouldBeTranslucent = (trackThisEntitiesVisibilityInfo(formation)
//                        && !formation.isVisibleToEnemy()) || formation.isHidden();
//                if ((shouldBeTranslucent && translucentHiddenUnits)
//                        || (formation.relHeight() < 0)) {
//                    graph.setComposite(AlphaComposite.getInstance(
//                            AlphaComposite.SRC_OVER, 0.5f));
//                }
//
//                // draw the 'fuel leak' decal where appropriate
//                boolean drawFuelLeak = EntityWreckHelper.displayFuelLeak(formation);
//
//                if (drawFuelLeak) {
//                    Image fuelLeak = bv.getScaledImage(bv.tileManager.bottomLayerFuelLeakMarkerFor(formation), true);
//                    if (null != fuelLeak) {
...
//                boolean drawMotiveWreckage = EntityWreckHelper.displayMotiveDamage(formation);
//
//                if (drawMotiveWreckage) {
//                    Image motiveWreckage = bv.getScaledImage(bv.tileManager.bottomLayerMotiveMarkerFor(formation), true);
//                    if (null != motiveWreckage) {
//                        graph.drawImage(motiveWreckage, 0, 0, this);
//                    }
//                }
//
//                graph.drawImage(bv.getScaledImage(bv.tileManager.imageFor(formation, secondaryPos), true),
//                        0, 0, this);
...
//        boolean isTank = (formation instanceof Tank);
//        boolean isInfantry = (formation instanceof Infantry);
//        boolean isAero = formation.isAero();
//        boolean isGunEmplacement = formation instanceof GunEmplacement;
//        boolean isSquadron = formation instanceof FighterSquadron;
//
//        if ((isAero && ((IAero) formation).isSpheroid() && !board.inSpace()) && (secondaryPos == 1)) {
//            graph.setColor(Color.WHITE);
//            graph.draw(bv.facingPolys[formation.getFacing()]);
//        }
...
//            int crewStunned = 0;
//            if (formation instanceof Tank) {
//                turretLocked = !((Tank) formation).hasNoTurret() && !formation.canChangeSecondaryFacing();
//                crewStunned = ((Tank) formation).getStunnedTurns();
//            }
//
//            // draw elevation/altitude if non-zero
//            if (formation.isAirborne()) {
//                if (!board.inSpace()) {
//                    stStr.add(new Status(Color.CYAN, "A", SMALL));
//                    stStr.add(new Status(Color.CYAN, Integer.toString(formation.getAltitude()), SMALL));
//                }
//            } else if (formation.getElevation() != 0) {
//                stStr.add(new Status(Color.CYAN, Integer.toString(formation.getElevation()), SMALL));
//            }
//
//            // Shutdown
//            if (formation.isManualShutdown()) {
//                stStr.add(new Status(GUIP.getCautionColor(), "SHUTDOWN"));
//            } else if (formation.isShutDown()) {
//                stStr.add(new Status(GUIP.getWarningColor(), "SHUTDOWN"));
//            }
//
//            // Prone, Hulldown, Stuck, Immobile, Jammed
//            if (formation.isProne()) {
//                stStr.add(new Status(GUIP.getCautionColor(), "PRONE"));
//            }
//
//            if (!formation.getHiddenActivationPhase().isUnknown()) {
//                stStr.add(new Status(GUIP.getPrecautionColor(), "ACTIVATING"));
//            }
//
//            if (formation.isHidden()) {
//                stStr.add(new Status(GUIP.getPrecautionColor(), "HIDDEN"));
//            }
//
//            if (formation.isGyroDestroyed()) {
//                stStr.add(new Status(GUIP.getWarningColor(), "NO_GYRO"));
//            }
//
//            if (formation.isHullDown()) {
//                stStr.add(new Status(GUIP.getPrecautionColor(), "HULLDOWN"));
//            }
//
//            if (formation.isStuck()) {
//                stStr.add(new Status(GUIP.getCautionColor(), "STUCK"));
//            }
//
//            if (!isGunEmplacement && formation.isImmobile()) {
//                stStr.add(new Status(GUIP.getWarningColor(), "IMMOBILE"));
//            }
//
//            if (formation.isBracing()) {
...
//            // Infantry
//            if (isInfantry) {
//                Infantry inf = ((Infantry) formation);
...
//                        boolean isLocalTeam = formation.getOwner().getTeam() == bv.clientgui.getClient().getLocalPlayer().getTeam();
//                        boolean isLocalPlayer = formation.getOwner().equals(bv.clientgui.getClient().getLocalPlayer());
//                        if (isLocalPlayer) {
...
//                        }
//                    } else {
//                        graph.setColor(formation.getOwner().getColour().getColour(false));
//                    }
...
//                if (!formation.isDone() && !onlyDetectedBySensors()) {
...
//            // draw facing
//            graph.setColor(Color.white);
//            if ((formation.getFacing() != -1)
//                    && !(isInfantry && !((Infantry) formation).hasFieldWeapon()
//                            && !((Infantry) formation).isTakingCover())
//                    && !(isAero && ((IAero) formation).isSpheroid() && !board.inSpace())) {
...
//                    graph.fill(bv.facingPolys[formation.getFacing()]);
//                    graph.setColor(Color.LIGHT_GRAY);
//                    graph.draw(bv.facingPolys[formation.getFacing()]);
//                    graph.setTransform(tr);
//                }
//
//                if (!formation.isDone() && bv.game.getPhase().isMovement()) {
//                    graph.setColor(GUIP.getWarningColor());
//                    graph.fill(bv.facingPolys[formation.getFacing()]);
//                    graph.setColor(Color.WHITE);
//                    graph.draw(bv.facingPolys[formation.getFacing()]);
...
//            int secFacing = formation.getFacing();
//            if (!((formation instanceof Mech) || (formation instanceof Protomech))
//                    || (formation instanceof QuadVee)) {
//                secFacing = formation.getSecondaryFacing();
//            } else if (formation.getArmsFlipped()) {
//                secFacing = (formation.getFacing() + 3) % 6;
//            }
//            // draw secondary facing arrow if necessary
//            if ((secFacing != -1) && (secFacing != formation.getFacing())) {
//                graph.setColor(Color.GREEN);
//                graph.draw(bv.facingPolys[secFacing]);
//            }
//            if (formation.isAero() && this.bv.game.useVectorMove()) {
//                for (int head : formation.getHeading()) {
//                    graph.setColor(Color.GREEN);
...
//
//            percentRemaining = formation.getArmorRemainingPercent();
...
//                percentRemaining = formation.getInternalRemainingPercent();
//                barLength = (int) (STATUS_BAR_LENGTH * percentRemaining);
//
//                graph.setColor(Color.darkGray);
...
//            if ((pipOption != 0) && !isGunEmplacement && !formation.isAero()
//                    && ((formation.isDone() && bv.game.getPhase().isMovement())
//                    || bv.game.getPhase().isFiring())) {
//                int tmm = Compute.getTargetMovementModifier(bv.game, formation.getId()).getValue();
//                Color tmmColor = (pipOption == 1) ? Color.WHITE : GUIP.getColorForMovement(formation.moved);
//                graph.setColor(Color.darkGray);
...
//        var hexEntities = bv.game.getEntitiesVector(formation.getPosition());
//        return hexEntities.stream()
//                .filter(e -> hexEntities.indexOf(formation) > hexEntities.indexOf(e))
//                .filter(e -> e.getFacing() == formation.getFacing())
//                .anyMatch(e -> !e.isDone());
//    }

//    private @Nullable Color getDamageColor() {
//        switch (formation.getDamageLevel()) {
...
//        return EntityVisibilityUtils.onlyDetectedBySensors(bv.getLocalPlayer(), formation);
//    }
...
//            return "C0C0C0";
//        } else {
//            return formation.getOwner().getColour().getHexString();
//        }
//    }
