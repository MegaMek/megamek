...
public class WeaponPanel extends PicMap implements ListSelectionListener, ActionListener, IPreferenceChangeListener {
    ...
    private class WeaponListMouseAdapter extends MouseInputAdapter {
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            if (currentIndex != dragSourceIndex) {
                ...
                WeaponMounted weaponMounted = srcModel.getWeaponAt(dragSourceIndex);
                srcModel.swapIdx(dragSourceIndex, dragTargetIndex);
                dragSourceIndex = currentIndex;
                Entity ent = weaponMounted.getEntity();
    ...
    /**
     * ListModel implementation that supports keeping track of a list of Mounted instantiations, how to display them in
     * the JList, and an ability to sort the Mounted given WeaponComparators.
     *
     * @author arlith
     */
    class WeaponListModel extends AbstractListModel<String> {
         ...
         *
         * @param weaponMounted Add the {@link WeaponMounted} to the list.
         */
        ...
        /**
         * @param weaponId Given the equipment (weapon) id...
         *
         * @return return the index in the (possibly sorted) list of Mounted.
         */
         ...
         */
        public void swapIdx(int index1, int index2) {
            // Bounds checking
            if ((index1 >= weapons.size()) || (index2 >= weapons.size()) || (index1 < 0) || (index2 < 0)) {
                return;
            }
            WeaponMounted m1 = weapons.get(index1);
            weapons.set(index1, weapons.get(index2));
            weapons.set(index2, m1);
         ...
         * Mounted description, as well as additional information like location, whether the Mounted is
        ...
        public String getElementAt(int index) {
            ...
            if ((weaponType.getAmmoType() != AmmoType.T_NA) &&
                ...
                wn.append(')');
                             ...
                             (entity.isSupportVehicle() && (weaponType.getAmmoType() == AmmoType.T_INFANTRY))) {
                int shotsLeft = 0;
                int totalShots = 0;
                ...
                wn.append(" (").append(shotsLeft).append("/").append(totalShots).append(")");
            }
            ...
            if ((game != null) && game.getOptions().booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_CALLED_SHOTS)) {
                wn.append(' ');
         ...
         *
         * @param comparator Custom <code>Comparator</code> for {@link WeaponMounted}
         */
    ...
    private final JPanel panelLower;
    ...
    Color[] backgroundColors = { Color.gray, Color.darkGray };
    int gridY;
    ...
    private void addSubdisplay(JPanel parent, JComponent child, int minHeight, int fill) {
        ...
        child.setBackground(backgroundColors[(gridY++) % backgroundColors.length]);
        ...
        parent.add(child, GBC.eol().gridy(gridY++).insets(10, 1, 10, 1).weighty(1).fill(fill));
    }

    private void createWeaponList(JPanel parent) {
        wSortOrder = new JLabel(Messages.getString("MekDisplay.WeaponSortOrder.label"), SwingConstants.LEFT);
        ...
        pWeaponOrder.add(wSortOrder, GBC.std().insets(15, 1, 1, 1).gridy(panelGridY).gridx(0));
        ...
        pAmmo.add(wBayWeapon, GBC.std().insets(15, 1, 1, 1).gridy(panelGridY).gridx(0));
        pAmmo.add(m_chBayWeapon,
              GBC.std().fill(GridBagConstraints.HORIZONTAL).insets(15, 1, 15, 1).gridy(panelGridY).gridx(1));
        panelGridY++;

        pAmmo.add(wAmmo, GBC.std().insets(15, 9, 1, 1).gridy(panelGridY).gridx(0));
    ...
    private void createWeaponDisplay(JPanel parent) {
        ...
        pCurrentWeapon.add(wDamageTrooperL,
              GBC.std()
                    .fill(GridBagConstraints.NONE)
                    .anchor(GridBagConstraints.WEST)
                    .insets(15, 9, 1, 1)
        ...
        pCurrentWeapon.add(wDamageTrooperR,
              GBC.std()
                    .fill(GridBagConstraints.NONE)
                    .anchor(GridBagConstraints.WEST)
                    .insets(15, 1, 1, 1)
    ...
    private void setBackGround() {
        ...
        Image tile = getToolkit().getImage(new MegaMekFile(Configuration.widgetsDir(),
        ...
        tile = getToolkit().getImage(new MegaMekFile(Configuration.widgetsDir(), udSpec.getTopLine()).toString());
        PMUtil.setImage(tile, this);
        addBgDrawer(new BackGroundDrawer(tile, b));
        ...
        tile = getToolkit().getImage(new MegaMekFile(Configuration.widgetsDir(), udSpec.getTopLeftCorner()).toString());
        PMUtil.setImage(tile, this);
        addBgDrawer(new BackGroundDrawer(tile, b));

        b = BackGroundDrawer.NO_TILING | BackGroundDrawer.VALIGN_BOTTOM | BackGroundDrawer.HALIGN_LEFT;
        tile = getToolkit().getImage(new MegaMekFile(Configuration.widgetsDir(),
    ...
    public void displayMek(Entity en) {
        ...
        int currentHeatBuildup = (en.heat // heat from last round
                                        + en.getEngineCritHeat() // heat engine critical slots will add
        ...
        for (int i = 0; i < entity.getWeaponList().size(); i++) {
            ...
            if (mounted.isUsedThisRound() && (game.getPhase() == mounted.usedInPhase()) && game.getPhase().isFiring()) {
        ...
        String stringHeatOverCapacity = "";
        if (heatOverCapacity > 0) {
            heatText += "*"; // overheat indication
            String msg_over = Messages.getString("MekDisplay.over");
            stringHeatOverCapacity = " " + heatOverCapacity + " " + msg_over;
        }

        String heatMessage = heatText + " (" + heatCapacityStr + ')' + stringHeatOverCapacity;
    ...
    public int selectFirstWeapon() {
        ...
        if (entity.getWeaponList().isEmpty() || (entity.usesWeaponBays() && entity.getWeaponBayList().isEmpty())) {
            return -1;
        }
        ...
        for (int i = 0; i < this.weaponList.getModel().getSize(); i++) {
            ...
            if (entity.isWeaponValidForPhase(selectedWeapon)) {
                ...
                return entity.getEquipmentNum(selectedWeapon);
            }
        }
    ...
    public int getNextWeaponListIdx() {
        ...
        do {
            ...
            selectedWeapon = ((WeaponListModel) weaponList.getModel()).getWeaponAt(selected);
        } while (!hasLooped && !entity.isWeaponValidForPhase(selectedWeapon));

        if ((selected >= 0) && (selected < entity.getWeaponList().size()) && !hasLooped) {
            return selected;
        } else {
            return -1;
        }
    ...
    private void displaySelected() {
        ...
        WeaponMounted mounted = ((WeaponListModel) weaponList.getModel()).getWeaponAt(weaponList.getSelectedIndex());
        ...
        if ((weaponType instanceof InfantryWeapon infantryWeapon) && !weaponType.hasFlag(WeaponType.F_TAG)) {
            wDamageTrooperL.setVisible(true);
            wDamageTrooperR.setVisible(true);
            if (entity.isConventionalInfantry()) {
                wDamageTrooperR.setText(Double.toString((double) Math.round(((Infantry) entity).getDamagePerTrooper() *
                                                                                  1000) / 1000));
            } else {
                wDamageTrooperR.setText(Double.toString(infantryWeapon.getInfantryDamage()));
            }
            ...
            int range = infantryWeapon.getInfantryRange();
        ...
        if (weaponType.getDamage() == WeaponType.DAMAGE_BY_CLUSTERTABLE) {
            ...
            wDamR.setText(damage.toString());
                         ...
                         unitDisplay.getClientGUI()
                               .getClient()
                               .getGame()
                               .getOptions()
                               .booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_ENERGY_WEAPONS)) {
            if (mounted.hasChargedCapacitor() != 0) {
                if (mounted.hasChargedCapacitor() == 1) {
                    wDamR.setText(Integer.toString(Compute.dialDownDamage(mounted, weaponType) + 5));
                }
                if (mounted.hasChargedCapacitor() == 2) {
                    wDamR.setText(Integer.toString(Compute.dialDownDamage(mounted, weaponType) + 10));
                }
            } else {
                wDamR.setText(Integer.toString(Compute.dialDownDamage(mounted, weaponType)));
            }
        } else {
        ...
        if (weaponType.getAmmoType() == AmmoType.T_NA) {
            ...
            int count = 0;
            vAmmo = new ArrayList<>();
            for (AmmoMounted current = mounted.getLinkedAmmo();
                  current != null;
            ...
            for (AmmoMounted ammo : fullAmmoList) {
                ...
                if (ammo.isAmmoUsable() &&
                          sameLocationIfLargeAero &&
                          rightBay &&
                          canSwitchToAmmo &&
    ...
    private void updateRangeDisplayForAmmo(AmmoMounted mAmmo) {
        ...
        if (ammoType.getAmmoType() == AmmoType.T_ATM) {
        ...
        } else if ((ammoType.getAmmoType() == AmmoType.T_LRM) &&
            ...
            wLongR.setText("11 - 15");
            wExtR.setText("16 - 20");
        } else if ((ammoType.getAmmoType() == AmmoType.T_SRM) &&
                         (ammoType.getMunitionType().contains(AmmoType.Munitions.M_DEAD_FIRE))) {
            wMinR.setText("---");
            wShortR.setText("1 - 2");
            wMedR.setText("3 - 4");
            wLongR.setText("5 - 6");
            wExtR.setText("7 - 8");
        } else if (ammoType.getAmmoType() == AmmoType.T_ARROW_IV) {
    ...
    private void updateAttackValues(WeaponMounted weapon, AmmoMounted ammo) {
        ...
        if (null != ammo) {
            AmmoType ammoType = ammo.getType();
            double[] changes = changeAttackValues(ammoType, avShort, avMed, avLong, avExt, maxRange);
    ...
    private double[] changeAttackValues(AmmoType ammoType, double avShort, double avMed, double avLong, double avExt,
        ...
        if (AmmoType.T_ATM == ammoType.getAmmoType()) {
            ...
            if (ammoType.getMunitionType().contains(AmmoType.Munitions.M_ARTEMIS_CAPABLE)) {
                if ((ammoType.getAmmoType() == AmmoType.T_LRM) || (ammoType.getAmmoType() == AmmoType.T_LRM_IMP)) {
                    int bonus = (int) Math.ceil(ammoType.getRackSize() / 5.0);
                    avShort = avShort + bonus;
                    avMed = avMed + bonus;
                    avLong = avLong + bonus;
                }
                if ((ammoType.getAmmoType() == AmmoType.T_SRM) || (ammoType.getAmmoType() == AmmoType.T_SRM_IMP)) {
                    avShort = avShort + 2;
                }
            }
        } else if (ammoType.getAmmoType() == AmmoType.T_AC_LBX) {
        ...
        } else if (ammoType.getAmmoType() == AmmoType.T_AR10) {
        ...
        } else if (ammoType.getAmmoType() == AmmoType.T_KILLER_WHALE) {
        ...
        } else if (ammoType.getAmmoType() == AmmoType.T_WHITE_SHARK) {
    ...
    private void compileWeaponBay(WeaponMounted weapon, AmmoMounted mAmmo, boolean isCapital) {
        ...
        for (WeaponMounted m : bayWeapons) {
            if (!m.isBreached() &&
                ...
                if (null != mAmmo) {
                    double[] changes = changeAttackValues(mAmmo.getType(), mAVShort, mAVMed, mAVLong, mAVExt, mMaxR);
                    mAVShort = changes[0];
                    mAVMed = changes[1];
                    mAVLong = changes[2];
                    mAVExt = changes[3];
        ...
        avShort = multiShot * avShort;
        avMed = multiShot * avMed;
        avLong = multiShot * avLong;
        avExt = multiShot * avExt;
    ...
    public void valueChanged(ListSelectionEvent event) {
        ...
        if (event.getSource().equals(weaponList)) {
            ...
            if (currPanel instanceof FiringDisplay firingDisplay) {
                ...
                if (weaponList.getSelectedIndex() != -1) {
                    mounted = weaponModel.getWeaponAt(weaponList.getSelectedIndex());
                }
                WeaponMounted prevMounted = null;
                if ((event.getLastIndex() != -1) && (event.getLastIndex() < weaponModel.getSize())) {
                    prevMounted = weaponModel.getWeaponAt(event.getLastIndex());
                }
                // Some weapons have a specific target, which gets handled in the target method
                if (mounted != null && mounted.getType().hasFlag(WeaponType.F_VGL)) {
                    // Store previous target, if it's a weapon that doesn't have a forced target
                    if ((prevMounted != null) && !prevMounted.getType().hasFlag(WeaponType.F_VGL)) {
                    ...
                    if (prevTarget != null) {
                        ...
                        unitDisplay.getClientGUI().getBoardView().select(prevTarget.getPosition());
                        prevTarget = null;
                    } else {
            ...
            // Tell the <Phase>Display to update the firing arc info when a weapon has been de-selected
            if (weaponList.getSelectedIndex() == -1) {
    ...
    public void actionPerformed(ActionEvent ev) {
        ...
        if (ev.getSource().equals(m_chAmmo) && (m_chAmmo.getSelectedIndex() != -1) && (clientgui != null)) {
            int n = weaponList.getSelectedIndex();
            if (n == -1) {
                return;
            }

            // We can update display values without changing the selected unit's ammo; this allows displaying
            // selected unit's ammo-based ranges without owning it
            WeaponMounted mountedWeapon = ((WeaponListModel) weaponList.getModel()).getWeaponAt(n);
            AmmoMounted oldAmmo = mountedWeapon.getLinkedAmmo();
            ...
            if (!clientgui.getClient().getLocalPlayer().equals(entity.getOwner())) {
                return;
            }

            // if this is a weapon bay, then this is not what we want
            boolean isBay = false;
            if (mountedWeapon.getType() instanceof BayWeapon) {
                isBay = true;
                ...
                WeaponMounted selectedWeapon = mountedWeapon.getBayWeapon(n);
                // cycle through all weapons of the same type and load with this ammo
                if (selectedWeapon != null) {
                    for (WeaponMounted bayWeapon : mountedWeapon.getBayWeapons()) {
                        // FIXME: Consider new AmmoType::equals / BombType::equals
                        if (bayWeapon.getType().equals(selectedWeapon.getType())) {
                            entity.loadWeapon(bayWeapon, mAmmo);
                            // Alert the server of the update.
                            clientgui.getClient()
                                  .sendAmmoChange(entity.getId(),
                                        entity.getEquipmentNum(bayWeapon),
                ...
                entity.loadWeapon(mountedWeapon, mAmmo);
                // Alert the server of the update.
                clientgui.getClient()
                      .sendAmmoChange(entity.getId(),
                            entity.getEquipmentNum(mountedWeapon),
                            entity.getEquipmentNum(mAmmo),
                            0);
            }

            // Refresh for hot load change
            if ((((oldAmmo == null) || !oldAmmo.isHotLoaded()) && mAmmo.isHotLoaded()) ||
                      ((oldAmmo != null) && oldAmmo.isHotLoaded() && !mAmmo.isHotLoaded())) {
                displayMek(entity);
                weaponList.setSelectedIndex(n);
                weaponList.ensureIndexIsVisible(n);
                displaySelected();
            ...
            if (entity.isAirborne() || entity.usesWeaponBays()) {
                WeaponType weaponType = mountedWeapon.getType();
                if (isBay) {
                    compileWeaponBay(mountedWeapon, mAmmo, weaponType.isCapital());
                } else {
                    // otherwise I need to replace range display with
                    // standard ranges and attack values
                    updateAttackValues(mountedWeapon, mAmmo);
                }
            }
            ...
            displaySelected();
        } else if (ev.getSource().equals(m_chBayWeapon) && (m_chBayWeapon.getItemCount() > 0)) {
            int n = weaponList.getSelectedIndex();
            if (n == -1) {
                return;
            }
