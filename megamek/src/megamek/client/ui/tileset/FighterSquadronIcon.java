...
/**
 * This is a specialized version of EntityImage for FighterSquadrons that assembles its icon from the icons of the
 * fighters that make up the squadron.
 */
public class FighterSquadronIcon extends EntityImage {
    ...
    /**
     * The positionHash is used to determine the specific arrangement of the fighter icons. It should remain the same
     * for any specific squadron through saving/loading and not change from damage. It can change when fighters are
     * removed as the arrangement of icons changes then anyway. Everything else is a matter of design (which squadrons
     * should use the same arrangement?)
     */
    private final int positionHash;

    public FighterSquadronIcon(Image base, Image wreck, Camouflage camouflage, Entity entity, int secondaryPos,
        ...
        String hashString = squadron.getSubEntities()
              .stream()
              .map(Entity::getChassis)
              .sorted()
    ...
    /**
     * @return The pixel position in the hex of the fighter of the given index (from 0 to n-1 where n is the number of
     *       fighters in the squadron) for the given total count (n) of fighters in the squadron
     */
