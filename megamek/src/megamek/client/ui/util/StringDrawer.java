 ...
 * <p>
 * Use: Construct a new StringDrawer with the target String as parameter. The constructor and most other methods return
 * the StringDrawer so further settings can be chained to the constructor call, such as {@link #at(int, int)} setting
 * the position, or {@link #color(Color)} setting the fill color of the drawn text. The chain can be finished by using
...
public class StringDrawer {
    ...
    /**
     * Sets the coordinates to draw the text at. The exact placement depends on other settings such as given by
     * {@link #center()}, {@link #centerX()}, or {@link #rightAlign()}.
     *
    ...
    /**
     * Sets the StringDrawer to center the text on the coordinate given by {@link #at(int, int)}. This is equivalent to
     * calling both {@link #centerX()} and {@link #centerY()}. Note that vertical centering works in a special fashion
     * explained in {@link #centerY()}.
     *
     * @return The StringDrawer itself
     *
     * @see #absoluteCenter()
    ...
    /**
     * Sets the StringDrawer to center the text horizontally on the x coordinate given by {@link #at(int, int)}. Note
     * that StringDrawer uses GlyphVectors to draw the Strings. As a result, leading or trailing spaces of the text will
     * be ignored for centering purposes.
     *
     * @return The StringDrawer itself
     */
    ...
    /**
     * Sets the StringDrawer to center the text vertically on the y coordinate given by {@link #at(int, int)}.
     * <p>
     * Note: Vertical text centering does *not* use the actual text to perform centering as this would make it very
     * difficult to keep a common baseline for multiple consecutive texts when these texts contain characters with
     * varying ascent and descent, such as "A1" and "jg". Instead, vertical centering is done using a placeholder text
     * of capital letters. Therefore, vertical centering will center the text approximately at half the ascent (roughly
     * half the height of capital letters).
     *
     * @return The StringDrawer itself
     *
     * @see #absoluteCenterY()
     ...
     * Sets the StringDrawer to center the text vertically on the y coordinate given by {@link #at(int, int)}.
     * <p>
     * Note: In contrast to {@link #centerY()}, this centers the text based on its own vertical size. This is useful
     * when e.g. centering a single letter or symbol and when no alignment with other text is required.
     *
     * @return The StringDrawer itself
     *
     * @see #centerY()
    ...
    /**
     * Sets the StringDrawer to center the text on the coordinate given by {@link #at(int, int)}. This is equivalent to
     * calling both {@link #centerX()} and {@link #absoluteCenterY()}. Note that vertical centering works based on the
     * text's own vertical size as explained in {@link #absoluteCenterY()}.
     *
     * @return The StringDrawer itself
     *
     * @see #center()
    ...
    /**
     * Sets the StringDrawer to right-align the text horizontally at the coordinate given by {@link #at(int, int)}.
     *
     * @return The StringDrawer itself
     */
    ...
    /**
     * Sets the StringDrawer to left-align the text horizontally at the coordinate given by {@link #at(int, int)}. This
     * is the default setting. This method can be used to override other alignment settings given by a previous
    ...
    /**
     * Sets the StringDrawer to use the given fontSize when drawing the String. When drawing the string, a new Font
     * object will be created using this fontSize and either the font given through {@link #font(Font)} or, when no font
     * is given, the current font. Note that when this method is used, and a font is also specified, the fontSize given
     * here takes precedence even when {@link #font(Font)} is called later. Also note that {@link #maxWidth(int)} may
     * scale down the resulting font size even when this method is used (in other words, this method sets the maximum
     * possible font size). Values for fontSize &lt;= 0 are ignored.
     *
     * @return The StringDrawer itself
     */
    ...
    /**
     * Sets the StringDrawer to give the text an outline of the given color and line width. Note that the outline is
     * drawn first so its width may appear smaller than expected.
     *
    ...
    /**
     * Sets the StringDrawer to give the text a second outline of the given color and line width. Note that the second
     * outline is drawn first (before {@link #outline(Color, float)}) so its width may appear smaller than expected and
     * a width value greater than that of the first outline should be used. Apart from the order of drawing this outline
     * and {@link #outline(Color, float)} are equivalent.
     *
     * @param color The color to draw the outline in
     * @param width The brush stroke width to use
     *
    ...
    /**
     * Sets the StringDrawer to scale the text horizontally by the given scaleX value. Values bigger than 1 stretch the
     * text, values smaller than 1 compress it. Note that the maxWidth value is not scaled so scaleX &lt; 1 will make
     * the text fit maxWidth easier while scaleX &gt; 1 will make it more easily reach maxWidth.
     *
    ...
    /**
     * Draws the String with applied settings to the given Graphics g (obtained from a BufferedImage, JComponent, SVG
     * context or other source). Note that when the text is empty or null, no action at all is taken. The settings of
     * the Graphics context (brush, color, transform, font) are preserved. The returned Rectangle gives the size of the
     * drawn string including leading and trailing white space. Note that when no font is actively set, the used font
     * depends on what is currently set in the Graphics object. The same is true for the text color.
     *
    ...
    public static class StringDrawerConfig {
        ...
        /**
         * Sets the Configuration to scale the text horizontally by the given scaleX value. Values bigger than 1 stretch
         * the text, values smaller than 1 compress it.
         *
         * @param scaleX the horizontal stretch or compression factor to apply to the text
         *
