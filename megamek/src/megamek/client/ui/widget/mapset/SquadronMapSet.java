...
public class SquadronMapSet implements DisplayMapSet {
    ...
    private void setAreas() {
        for (int i = 0; i < max_size; i++) {
            ...
            engineCritImage[i] = jComponent.createImage(3 * (squareSize + 1), squareSize + 1);
            engineCritArea[i] = new PMPicArea(engineCritImage[i]);
            fcsCritImage[i] = jComponent.createImage(3 * (squareSize + 1), squareSize + 1);
            fcsCritArea[i] = new PMPicArea(fcsCritImage[i]);
            sensorCritImage[i] = jComponent.createImage(3 * (squareSize + 1), squareSize + 1);
            sensorCritArea[i] = new PMPicArea(sensorCritImage[i]);
            pilotCritImage[i] = jComponent.createImage(6 * (squareSize + 1), squareSize + 1);
