...
public abstract class Entity extends TurnOrdered implements Transporter,
    ...
    public boolean isClanArmor() {
        ...
        return ((getArmorTechLevel() == TechConstants.T_CLAN_TW)
                ...
                || (getArmorTechLevel() == TechConstants.T_CLAN_EXPERIMENTAL) || (getArmorTechLevel() == TechConstants.T_CLAN_UNOFFICIAL));
    }
    ...
    public boolean isElevationValid(int assumedElevation, IHex hex) {
        ...
        if (getMovementMode() == IEntityMovementMode.VTOL) {
            ...
            if (hex.containsTerrain(Terrains.WATER)
                    && (altitude < hex.surface()) && !(this instanceof Mech)
     ...
     */
    public Mounted addEquipment(EquipmentType etype, int loc,
            boolean rearMounted, boolean bodyMounted)
            throws LocationFullException {
        Mounted mounted = new Mounted(this, etype);
        mounted.setBodyMounted(bodyMounted);
        addEquipment(mounted, loc, rearMounted);
    ...
    public boolean hasActiveECCM() {
        ...
        if ((game.getOptions().booleanOption("tacops_eccm") || game
                .getOptions().booleanOption("stratops_ecm"))
                && !isShutDown()) {
            for (Mounted m : getMisc()) {
                EquipmentType type = m.getType();
                // TacOps p. 100 Angle ECM can have 1 ECM and 1 ECCM at the same
        ...
        if (!isShutDown()) {
            for (Mounted m : getMisc()) {
                ...
                if ((type instanceof MiscType) && type.hasFlag(MiscType.F_ECM)
                        && !m.isInoperable()) {
                    if (BattleArmor.SINGLE_HEX_ECM.equals(type
                            .getInternalName())) {
                        return 0;
                    }
    ...
    public boolean hasC3S() {
        ...
        for (Mounted m : getEquipment()) {
            if ((m.getType() instanceof MiscType)
                    && m.getType().hasFlag(MiscType.F_C3S) && !m.isInoperable()) {
                return true;
            }
        }
        return false;
    ...
    public PilotingRollData getBasePilotingRoll(int moveType) {
        ...
        if (game.getOptions().booleanOption("tacops_fatigue")
                && crew.isPilotingFatigued(game.getRoundCount())) {
    ...
    public PilotingRollData addConditionBonuses(PilotingRollData roll,
        ...
        if ((moveType == IEntityMovementType.MOVE_RUN)
                ...
                || (moveType == IEntityMovementType.MOVE_OVER_THRUST)) {
            int lightPenalty = conditions.getLightPilotPenalty();
            if (lightPenalty > 0) {
                roll
                        .addModifier(lightPenalty, conditions
                                .getLightCurrentName());
            }
        }
    ...
    public PilotingRollData checkGetUp(MoveStep step) {
        ...
        if ((step == null)
                || ((step.getType() != MovePath.STEP_GET_UP) && (step.getType() != MovePath.STEP_CAREFUL_STAND))) {
    ...
    public PilotingRollData checkRecklessMove(MoveStep step, IHex curHex,
            Coords lastPos, Coords curPos, int lastElev) {
        PilotingRollData roll = getBasePilotingRoll(step.getParent()
                .getLastStepMovementType());
        ...
        // costs
                ...
                && ((curHex.movementCost(step.getParent()
                        .getLastStepMovementType()) > 0) || (lastElev != curHex
                        .getElevation()))) {
            roll.append(new PilotingRollData(getId(), 0, "moving recklessly"));
            // ice conditions
    ...
    public PilotingRollData rollMovementInBuilding(Building bldg, int distance,
            String why, int overallMoveType) {
        PilotingRollData roll = getBasePilotingRoll(overallMoveType);
        ...
        if (!unit.isEnemyOf(this)) {
            ...
            while (iter.hasMoreElements()) {
                ...
                if (next.canLoad(unit)
                        && (unit.getElevation() == getElevation())) {
                    return true;
                }
            }
        }
    ...
    public void deployOffBoard() {
        ...
        case IOffBoardDirections.NORTH:
            setPosition(new Coords(game.getBoard().getWidth() / 2
                    + game.getBoard().getWidth() % 2, -getOffBoardDistance()));
            setFacing(3);
            setDeployed(true);
            break;
        case IOffBoardDirections.SOUTH:
        ...
        case IOffBoardDirections.EAST:
            setPosition(new Coords(game.getBoard().getWidth()
                    + getOffBoardDistance(), game.getBoard().getHeight() / 2
    ...
    public void illuminateTarget(HexTarget target) {
        if (hasSpotlight && spotlightIsActive && (target != null)) {
            illuminated = true;
            ArrayList<Coords> in = Coords.intervening(getPosition(), target
                    .getPosition());
            for (Coords c : in) {
        ...
        if (fa % 30 == 0) {
            ...
            if ((srcHex != null) && (curHex != null)) {
                ...
                ArrayList<Coords> in = Coords.intervening(ai.attackPos,
                        ai.targetPos, true);
                leftBetter = LosEffects.dividedLeftBetter(in, game, ai, Compute
                        .isInBuilding(game, this), new LosEffects());
            }
        }
    ...
    public boolean isFlying() {
        ...
        if (getMovementMode() == IEntityMovementMode.VTOL) {
            if ((game.getBoard().getHex(getPosition()).terrainLevel(
                    Terrains.BLDG_ELEV) >= getElevation())
                    || (game.getBoard().getHex(getPosition()).terrainLevel(
                            Terrains.BRIDGE_ELEV) >= getElevation())) {
                return false;
            }
    ...
    public void applySantaAnna() {
        ...
        for (int i = 0; i < ammo.size(); i++) {
            try {
                Mounted newmount = addEquipment(EquipmentType.get(name
                        .elementAt(i)), locations.elementAt(i), false, ammo
                        .elementAt(i));
    ...
    public boolean isSurfaceNaval() {
        ...
        return (getElevation() == 0)
                && ((getMovementMode() == IEntityMovementMode.NAVAL)
                        || (getMovementMode() == IEntityMovementMode.HYDROFOIL) || (getMovementMode() == IEntityMovementMode.SUBMARINE));
    }
