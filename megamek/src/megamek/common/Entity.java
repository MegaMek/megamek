...
public abstract class Entity extends TurnOrdered
    ...
    /**
     * True when the entity has an un-destroyed searchlight that is neither a Quirk searchlight nor a mounted (0.5t /
     * 1slot) searchlight.
     */
    ...
    public Player getOwner() {
        ...
        // Server and other central classes already used {@link Game#getPlayer()} with {@link Entity#getOwnerId()}
        // instead of {@link Entity#getowner()} and it is noted that getOwner is not reliable. The entity owner object
        // would have to be replaced whenever a player is updated which does not happen. The player ID on the other
        // hand stays the same and the game object is not usually replaced. I expect entity.game to be up to date much
        // more than owner. Unfortunately, entities freshly created may not have the game set. Therefore, fall back
        // to the old version when game == null or the player is no longer in the game
    ...
    public boolean isElevationValid(int assumedElevation, Hex hex) {
        ...
        if (getMovementMode() == EntityMovementMode.VTOL) {
            ...
            if (hex.containsTerrain(Terrains.BUILDING)) {
                ...
                // Meks, ProtoMeks and infantry can occupy any floor in the building
