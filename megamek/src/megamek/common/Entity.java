...
public abstract class Entity extends TurnOrdered implements Transporter,
        ...
        if ((getMovementMode() == EntityMovementMode.SUBMARINE)
            ...
            if (next.containsTerrain(Terrains.BUILDING)
                ...
                if (((assumedElevation == bldcur) && (climbMode || isJumpingNow ) && (this instanceof Mech))
                        || (retVal > bldnex)) {
                    retVal = bldnex;
                    System.err.println(" Entity "+ getDisplayName() +"1 retval=" + retVal);
    ...
    public boolean isCapitalFighter(boolean lounge) {
        ...
        if (!lounge && isFighter() && game.getOptions().booleanOption("single_no_cap") && !isPartOfFighterSquadron()) {
            return false;
        }
    ...
    public boolean hasStealth() {
        ...
        if ((getArmorType(1) == EquipmentType.T_ARMOR_STEALTH)
                && !hasPatchworkArmor()) {
            return true;
        }
        return false;
