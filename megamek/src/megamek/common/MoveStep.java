...
public class MoveStep implements Serializable {
     ...
     *            currently checked.
     * @param target
     *            - the <code>Targetable</code> that is the target of this step.
     *            For example, the enemy being charged.
     * @param pos
     *            = the <code>Coords</code> for the target position.
     */
     ...
     *            currently checked.
     * @param mineToLay
     *            - the <code>int</code> that is the id of the mine that should
     ...
     *
     * @param isEnd
     *            the <code>boolean</code> flag that specifies that this step's
    ...
    protected void calcMovementCostFor(IGame game, MoveStep prevStep) {
        ...
        final EntityMovementMode moveMode = parent.getEntity()
                .getMovementMode();
        ...
        final boolean isWorkingTripod = (parent.getEntity() instanceof TripodMech) && (((Mech)parent.getEntity()).countBadLegs() < 1);
        ...
        if (isWorkingTripod && ((type == MoveStepType.TURN_LEFT) || ((type == MoveStepType.TURN_RIGHT)
                && ((prevStep.type == MoveStepType.TURN_LEFT) || (prevStep.type == MoveStepType.TURN_RIGHT))))) {
            mp = 0;
            return;
        }
