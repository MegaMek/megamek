...
public class ChargeAttackAction extends DisplacementAttackAction {
    ...
    public ToHitData toHit(IGame game, Targetable target, Coords src,
            int elevation, int movement, boolean skid, boolean gotUp) {
        ...
        if ((null != bldg) && (!targIsBuilding) && te != null
        ...
        if (te != null && targHex.terrainLevel(Terrains.WATER) == te.height()
    ...
    public ToHitData toHit(IGame game, MovePath md) {
        ...
        return toHit(game, target, chargeSrc, chargeEl, chargeStep
                .getMovementType(), false, md.contains(MovePath.STEP_GET_UP));
    }
