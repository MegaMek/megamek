 ...
 * static {@link #createCard(ASCardDisplayable)}. The basic way to obtain a card image after creating the card is
 * calling the card's {@link #getCardImage()} method. Other methods can be used to configure the resulting card image.
 * <p>
 * This class is responsible for painting Infantry cards. Subclasses of ASCard represent the unit cards for other
 * AlphaStrike unit types. It is not necessary to pay any attention to this when obtaining the card but for Large
 * Aerospace units the specific class can be used to have additional functionality for the two card faces of these
 * cards.
 */
public class ASCard {
    ...
    /**
     * Creates an ASCard for the given AlphaStrike unit (either an AlphaStrikeElement or a MekSummary) which can be used
    ...
    /**
     * Creates and returns an image of the AlphaStrike element's unit card. The card is drawn in grayscale standard MUL
     * style. The card is scaled with the given scale, values smaller than 1 making the card smaller. This method always
     * fully draws the card so it always reflects the current state of the AlphaStrike element.
     *
    ...
    /**
     * Creates and returns an image of the AlphaStrike element's unit card. The card is drawn in grayscale standard MUL
     * style. The card is scaled to fit the given width. This method always fully draws the card so it always reflects
     * the current state of the AlphaStrike element.
     *
    ...
    /**
     * Creates and returns an image of the AlphaStrike element's unit card. The card is drawn in grayscale standard MUL
     * style. The card will have the standard MUL dimension (1050 x 750 pixels) for all unit types except Large
     * Aerospace units. For those, the two card faces are stacked vertically and the resulting card image has a size of
     * 1050 x 1500. This method always fully draws the card so it always reflects the current state of the AlphaStrike
     * element.
     *
     * @return The unit card image
     */
    ...
    protected void paintBaseInfo(Graphics2D g) {
        ...
        new StringDrawer(element.getASUnitType().toString()).at(107, upperY)
        ...
        new StringDrawer(AlphaStrikeHelper.getMovementAsString(element)).at(505, upperY)
              .useConfig(valueConfig)
              .maxWidth(128)
              .draw(g);
    ...
    /**
     * Draws the card background including the AlphaStrike Stats block. When isFlipSide is true, the point value box and
     * logo are omitted and the copyright notice is positioned differently.
     *
     * @param g          The Graphics2D to draw to
    ...
    protected void paintCardBackground(Graphics2D g, boolean isFlipSide) {
        ...
        new StringDrawer("(C) " + LocalDate.now().getYear() + " The Topps Company. All rights reserved.").at(1014,
                    copyrightY).rotate(-Math.PI / 2)
