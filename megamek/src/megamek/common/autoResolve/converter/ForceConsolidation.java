...
public abstract class ForceConsolidation {
    ...
    public record Container(int uid, String name, String breadcrumb, int teamId, int playerId, List<Integer> entities,
          List<Container> subs) {
    ...
    /**
     * Consolidates forces by redistributing entities and sub forces as needed. It will balance the forces by team,
     * ensuring that each force has a maximum of 20 entities and 4 sub forces.
     *
    ...
    /**
     * Converts the forces into a list of ForceRepresentations. It is an intermediary representation of a force, in a
     * way that makes it very lightweight to manipulate and balance. It only contains the representation of the force
     * top-level, and the list of entities in it.
     *
    ...
    protected List<ForceRepresentation> translateForcesToForceRepresentation(Forces forces,
        ...
        for (Force force : forces.getTopLevelForces()) {
            ...
            forceRepresentations.add(new ForceRepresentation(force.getId(),
                  teamByPlayer.get(force.getOwnerId()),
                  force.getOwnerId(),
                  entityIds,
                  new int[0]));
        }
