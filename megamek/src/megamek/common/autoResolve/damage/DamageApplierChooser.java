...
public class DamageApplierChooser {
    ...
    /**
     * Automatically applies damage to the entity based on the "removal condition" provided. The damage is calculated as
     * being a percentage of the total armor of the unit, then it is transformed in a roll of dices which the average
     * roll is that amount, then the total damage is calculated and applied in clusters of 5 damage. It rolls a minimum
     * of 1 dice of damage. The removal condition is a code that indicates why the entity is being removed from the
     * game. It will decide if the unit or entity must survive based on the type of removal condition. The removal
     * conditions are: * RETREAT: crew must survive, entity must survive, 80% of the total armor is applied as damage *
     * SALVAGEABLE: crew may die, entity must be destroyed, 75% of the total armor is applied as damage * CAPTURED: crew
     * must survive, entity must be destroyed, 33% of the total armor is applied as damage * EJECTED: crew must survive,
     * entity must be destroyed, 33% of the total armor is applied as damage * DEVASTATED: crew may survive, entity must
     * be destroyed, 500% of the total armor is applied as damage * OTHER: crew may die, entity may be destroyed, 33% of
     * the total armor applied as damage The amount of damage applied present right now was decided arbitrarily and can
     * be changed later, maybe even make it follow a config file, client settings, etc.
     *
     * @param entity           the entity to choose the handler for
    ...
    private static int getNumberOfDices(Entity entity, EntityFinalState finalState) {
        ...
        double targetDamage = switch (finalState) {
            case ANY, CREW_MUST_SURVIVE, CREW_AND_ENTITY_MUST_SURVIVE ->
                  totalHealth * (0.1 + Compute.randomFloat() * 0.5);
