...
/**
 * Represents a Transport Bay (TM p.239) for carrying light vehicles (not heavier than 50t) aboard large spacecraft or
 * other units.
 */
public final class LightVehicleBay extends UnitBay {
    ...
    public boolean canLoad(Entity unit) {
        ...
        return (getUnused() >= 1) &&
                     (currentDoors >= loadedThisTurn) &&
                     (unit.getWeight() <= 50) &&
                     ((unit instanceof Tank) || loadableQuadVee);
    }

    @Override
    public String getUnusedString(boolean showRecovery) {
        return "Light Vehicle Bay " +
                     numDoorsString() +
                     " - " +
    ...
    public BayType getBayType() {
        return BayType.VEHICLE_LIGHT;
    }

    @Override
    public BayData getBayData() {
        return BayData.VEHICLE_LIGHT;
    }
