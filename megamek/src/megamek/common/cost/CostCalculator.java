...
public class CostCalculator {
    ...
    static long getWeaponsAndEquipmentCost(Entity entity, CalculationReport costReport, boolean ignoreAmmo) {
        ...
        for (Mounted<?> mounted : entity.getEquipment()) {
            ...
            if ((mounted.getType() instanceof MiscType) && mounted.getType().hasFlag(MiscType.F_BA_MANIPULATOR)) {
                continue;
            }
        ...
        for (String weapon : weaponsNumberMap.keySet()) {
            costReport.addLine(weaponsNumberMap.get(weapon) + " " + weapon,
                  "",
                  numberFormatter.format(weaponsCostMap.get(weapon)));
        }
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                costReport.addLine("CASE", "", numberFormatter.format(itemCost));
            }
        }
        ...
        if (!entity.isLargeCraft()) {
            ...
            if (seatCost > 0) {
                cost += seatCost;
                costReport.addLine("Seating", "", numberFormatter.format(seatCost));
            }
            if (quartersCost > 0) {
                cost += quartersCost;
                costReport.addLine("Quarters", "", numberFormatter.format(quartersCost));
            }
            if (bayCost > 0) {
                cost += bayCost;
                costReport.addLine("Bays", "", numberFormatter.format(bayCost));
            }
        }
        return cost;
    }
    ...
    static void fillInReport(CalculationReport costReport, Entity entity, boolean ignoreAmmo, String[] systemNames,
          int equipIndex, double cost, double[] costs) {
        NumberFormat numberFormatter = NumberFormat.getInstance();
        ...
        for (int l = 0; l < systemNames.length; l++) {
            if (l == equipIndex) {
                ...
                String result = numberFormatter.format(costs[l]);
                if (costs[l] == 0) {
                    result = "N/A";
                } else if (costs[l] < 0) {
                    result = "x " + numberFormatter.format(-costs[l]);
                }
                costReport.addLine(systemNames[l], "", result);
            }
        }
        costReport.addResultLine("Total Cost:", "", numberFormatter.format(cost));
    }
