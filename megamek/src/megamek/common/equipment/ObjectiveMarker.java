...
public class ObjectiveMarker extends GroundObject {
    ...
    /**
     * Tells whether a viewer is allowed to see an objective marker riding the given owner's
     * ground-objects-to-place list (a pre-game assignment from MekHQ or a scenario file). Such markers are part
     * of a side's mission plan: the owner and their teammates see them, a game master sees every side's, and
     * everyone else does not - the server strips them from the player data it sends to clients that may not see
     * them. (Unteamed players are their own side, so two unteamed players never see each other's.)
     *
    ...
    /**
     * @return The board position this marker should be placed at when the game starts, as pre-set by MekHQ or a
     *       scenario file, or {@code null} when the marker has no pre-set position (it is then placed by its
     *       owner during the Victory Setup or Deploy Minefields phase). The position is only meaningful while
     *       the marker rides a player's ground-objects-to-place list; once placed, the ground object map holds
     *       the position.
     */
