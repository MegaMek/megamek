...
public class ColorDeserializer extends StdDeserializer<Color> {
    ...
    public Color deserialize(JsonParser jsonParser, DeserializationContext deserializationContext)
          throws IOException {
        return parseColor(jsonParser.getCodec().readTree(jsonParser));
    }

    /**
     * Parses a Color object from the given JsonNode. The node needs to have entries for red, green and blue, alpha is
     * optional.
     *
    ...
    public static Color parseColor(JsonNode node) {
        ...
        if (node.has(ALPHA)) {
            return new Color(red, green, blue, node.get(ALPHA).asInt());
        } else {
