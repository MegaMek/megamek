 ...
 * necessary to accomplish that path.
 */
public class DestructionAwareDestinationPathfinder extends BoardEdgePathFinder {
    ...
    public BulldozerMovePath findPathToCoords(Entity entity, Set<Coords> destinationCoords, boolean jump,
        ...
        // start off with a jump if we're trying to calc a jump path and the entity does
        // not have jump mp, we're done
        if (jump
        ...
        // if we can't at all get to the coordinates with this entity, don't bother with
        // the rest
        if (closest == null) {
            return null;
        }
    ...
    /**
     * Calculates the closest coordinates to the given entity Coordinates which you have to blow up to get into are
    ...
    protected List<BulldozerMovePath> generateChildNodes(BulldozerMovePath parentPath,
        ...
        // there are six possible children of a move path, defined in
        // AeroPathUtil.getTurns()
        for (List<MoveStepType> turns : AeroPathUtil.getTurns()) {
    ...
    protected void processChild(BulldozerMovePath child, List<BulldozerMovePath> children,
        ...
        // through) it still can't be leveled
        ...
        // legal jump moves are simpler: can't go out of bounds, can't jump too high
        // (unless we can destroy the obstacle)
        ...
        boolean irreversibleJumpDown = (child.isJumping()
              && (Math.abs(mli.elevationChange) > child.getCachedEntityState().getJumpMP())) ||
        ...
        int destHexElevation = calculateUnitElevationInHex(child.getGame()
                    .getBoard(child.getFinalBoardId())
                    .getHex(destinationCoords),
    ...
    /**
     * Utility function that returns true if an attack on the building in the given coordinates will result in damage to
     * friendly units. Computation is cached as it is somewhat expensive to perform for each possible path node.
     */
    private boolean friendlyFireCheck(Entity shooter, Game game, Coords position, boolean includeMobileUnits) {
        ...
        // check if there are any entities in the building that meet the following
        // criteria:
        // - is friendly
    ...
    /**
     * Simplified logic for whether a mek going into the given hex will flood breached legs and effectively immobilize
     * it.
     */
    ...
    private record AStarComparator(Coords destination) implements Comparator<BulldozerMovePath> {
        ...
        public int compare(BulldozerMovePath first, BulldozerMovePath second) {
            ...
            // getFacingDiff returns a number between 0 and 3 inclusive. if the value diff
            // is larger than 3, then it won't make a difference and we skip calculating it
            if (Math.abs(dd) < 4) {
                dd *= 10; // facing diff doesn't matter as much as the other stuff, only use it as a
                // tie-breaker
