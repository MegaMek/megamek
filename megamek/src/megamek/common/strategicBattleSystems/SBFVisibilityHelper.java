...
import static megamek.common.strategicBattleSystems.SBFVisibilityStatus.*;

//TODO: use for hidden formations?

/**
 * This class holds visibility information for an SBF game. In SBF games, visibility is not instantaneous
...
public final class SBFVisibilityHelper implements SBFRuleOptionsUser {
     ...
     * visibility status. The default value if no entry can be found is INVISIBLE when double blind rules are
     ...
     * 
     * @implNote Note that the ID values are not checked; when they don't map to anything, no error will 
    ...
    /**
     * Returns the players to test visibility on; this is the viewing player alone unless the
     * rule that teams share their vision is used, when it is all players of the viewing player's team.
    ...
    private Set<Player> getPlayersToCheck(int viewingPlayer) {
        ...
        if (player == null) {
            LogManager.getLogger().error("Player {} does not exist.", viewingPlayer);
        } else {
            playersToCheck.add(player);
            if (usesTeamVision()) {
                Team team = game.getTeamForPlayer(player);
                if (team == null) {
                    LogManager.getLogger().error("Player {} is not on a team.", viewingPlayer);
                } else {
