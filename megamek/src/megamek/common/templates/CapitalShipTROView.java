...
public class CapitalShipTROView extends AeroTROView {
	...
	protected void initModel(EntityVerifier verifier) {
		...
		int nameWidth = addWeaponBays(ARCS);
		setModelData("formatWeaponBayRow", new FormatTableRowMethod(new int[] { nameWidth, 5, 8, 8, 8, 10, 12},
				new Justification[] { Justification.LEFT, Justification.CENTER, Justification.CENTER,
						Justification.CENTER, Justification.CENTER, Justification.CENTER, Justification.LEFT }));
		addFluff();
		TestAdvancedAerospace testAero = new TestAdvancedAerospace(aero, verifier.aeroOption, null);
		...
		setModelData("sailIntegrity", aero.hasSail()?
				aero.getSailIntegrity() : Messages.getString("TROView.NA"));
		...
		Map<String, Integer> miscCount = aero.getMisc().stream()
	...
	private static final String[][] ARCS = {
			{"Nose"}, {"FRS", "FLS"}, {"RBS", "LBS"}, {"ARS", "ALS"}, {"Aft"}
	};
	...
	private static final int[][] ARMOR_LOCS = {
			{Jumpship.LOC_NOSE}, {Jumpship.LOC_FRS, Jumpship.LOC_FLS},
