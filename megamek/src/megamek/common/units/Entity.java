...
public abstract class Entity extends TurnOrdered
    ...
    public boolean hasActiveEiCockpit() {
        if (!hasAbility(OptionsConstants.UNOFFICIAL_EI_IMPLANT)) {
            return false;
        }
        // When tracking hardware, require the EI cockpit equipment
        if ((game != null) && gameOptions().booleanOption(OptionsConstants.ADVANCED_TRACK_NEURAL_INTERFACE_HARDWARE)) {
            return hasEiCockpit();
        }
        // When not tracking hardware, any unit with EI implant pilot gets benefits
        return true;
    }
