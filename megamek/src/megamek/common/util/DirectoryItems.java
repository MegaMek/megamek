 ...
 * hierarchy, categorized according to their directories.  This collection
 * will include all files inside of JAR and ZIP files that are located in
...
public class DirectoryItems implements Categorized {
    ...
    /**
     * A map of the category names to the sub-categories.
    ...
    */
    ...
    /**
     * Helper function to file away new categories.  It adds one entry
     * in the map for each sub-category in the passed category.
     *
     ...
     *
     * @param   rootDir - the <code>File</code> object for the root directory
     *          of the image files.  All files in this root, or in any sub-
     ...
     *          is null or if it is not a directory, or if a <code>null</code>
     *          is passed for <code>itemFactory</code>.
     */
    public DirectoryItems( File rootDir, String categoryName,
        ...
        for ( int entry = 0; entry < contents.length; entry++ ) {
            ...
            if ( file.isDirectory() ) {
                ...
                this.addCategory
                    ( new DirectoryItems(file, name.toString(), factory) );
            ...
            // Is this entry a ZIP or JAR file?
                ...
                try {
                    this.addCategory
                        ( new ZippedItems(file, name.toString(), factory) );
                }
                catch ( Exception err ) {
            ...
            /* DEBUG : uncomment this section to debug *
                ...
                System.out.println( contents[entry] );
            }
            /* DEBUG : uncomment this section to debug */
