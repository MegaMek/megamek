...
public abstract class TestEntity implements TestEntityOption {
    ...
    public static List<ArmorType> legalArmorsFor(long etype, boolean industrial, boolean primitive,
        ...
        if ((etype & Entity.ETYPE_BATTLEARMOR) != 0) {
            ...
            return TestAdvancedAerospace.legalArmorsFor(techManager, primitive);
                          ...
                          (Entity.ETYPE_FIXED_WING_SUPPORT | Entity.ETYPE_SUPPORT_TANK | Entity.ETYPE_SUPPORT_VTOL)) !=
                         0) {
    ...
    public String printWeightHeatSinks() {
        return StringUtil.makeLength("Heat Sinks: " +
              ...
              getPrintSize() - 5) + TestEntity.makeWeightString(getWeightHeatSinks(), usesKgStandard()) + "\n";
    }

    public String printWeightEngine() {
        return StringUtil.makeLength("Engine: " + ((null != engine) ? engine.getEngineName() : "---"),
              getPrintSize() - 5) + TestEntity.makeWeightString(getWeightEngine(), usesKgStandard()) + "\n";
    }
    ...
    public String printWeightStructure() {
        return StringUtil.makeLength("Structure: " + getEntity().getTotalOInternal() + " " + structure.getShortName(),
              getPrintSize() - 5) + TestEntity.makeWeightString(getWeightStructure(), usesKgStandard()) + "\n";
    }
    ...
    public String printWeightArmor() {
        if (!getEntity().hasPatchworkArmor()) {
            ...
            return StringUtil.makeLength("Armor: " + getTotalOArmor() + " " + "Patchwork", getPrintSize() - 5) +
                         TestEntity.makeWeightString(getWeightArmor(), usesKgStandard()) +
                         "\n";
        }
    ...
    public double getWeightAllocatedArmor() {
        if (!getEntity().hasPatchworkArmor()) {
            return getWeightArmor(ArmorType.forEntity(getEntity()),
                  getEntity().getTotalOArmor(),
            ...
            for (int loc = getEntity().firstArmorIndex(); loc < getEntity().locations(); loc++) {
                ...
                if (getEntity().hasRearArmor(loc) && (getEntity().getOArmor(loc, true) > 0)) {
    ...
    public StringBuffer printMiscEquip(StringBuffer buff, int posLoc, int posWeight) {
        for (Mounted<?> m : getEntity().getMisc()) {
            ...
            buff.append(StringUtil.makeLength(getLocationAbbr(m.getLocation()), getPrintSize() - 5 - 20))
                  .append(TestEntity.makeWeightString(m.getTonnage(), usesKgStandard()));
            buff.append("\n");
        }
        return buff;
    }
    ...
    public StringBuffer printAmmo(StringBuffer buff, int posLoc, int posWeight) {
        for (Mounted<?> m : getEntity().getAmmo()) {
            ...
            buff.append(StringUtil.makeLength(mt.getName(), 20));
            buff.append(" ")
                  .append(StringUtil.makeLength(getLocationAbbr(m.getLocation()), getPrintSize() - 5 - 20))
                  .append(TestEntity.makeWeightString(m.getTonnage(), usesKgStandard()))
                  .append("\n");
        }
        return buff;
    ...
    public int calcMiscCrits(MiscType mt, double size) {
        if (mt.hasFlag(MiscType.F_CLUB) &&
            ...
            return 14;
        } else if (EquipmentType.getArmorTypeName(EquipmentType.T_ARMOR_FERRO_FIBROUS_PROTO)
                         .equals(mt.getInternalName())) {
            return 16;
            ...
            if (isClan() ||
                      mt.getInternalName()
                            .equals("Clan " +
                                          EquipmentType.getStructureTypeName(EquipmentType.T_STRUCTURE_ENDO_STEEL))) {
                return 7;
            }
            return 14;
            ...
            if (isClan() ||
                      mt.getInternalName()
                            .equals("Clan " +
                                          EquipmentType.getStructureTypeName(EquipmentType.T_STRUCTURE_ENDO_COMPOSITE))) {
                return 4;
            }
            return 7;
    ...
    public static int calcHeatNeutralHSRequirement(Entity entity) {
        ...
        for (Mounted<?> m : entity.getMisc()) {
            ...
            if (m.getType().hasFlag(MiscType.F_CLUB) &&
                      m.getType().hasSubType(MiscType.S_SPOT_WELDER) &&
                      entity.hasEngine() &&
                      (entity.getEngine().isFusion() || (entity.getEngine().getEngineType() == Engine.FISSION))) {
                continue;
            }
    ...
    public static double getRawArmorPoints(Entity unit, double armorTons) {
        if (unit.hasETypeFlag(Entity.ETYPE_PROTOMEK)) {
            ...
            return Math.floor(Math.floor(armorTons *
                                               TestAdvancedAerospace.armorPointsPerTon((Jumpship) unit,
                                                     EquipmentType.T_ARMOR_AEROSPACE,
                                                     false)) * 0.66);
        }
    ...
    public boolean hasIllegalTechLevels(StringBuffer buff, int ammoTechLvl) {
         ...
         * at the
         * intro date. We test instead whether the stated tech level is every possible
         * based on the
            ...
            if (nextE instanceof AmmoType) {
                if (eqRulesLevel > ammoRulesLevel) {
                    if (!retVal) {
                        ...
                        buff.append(TechConstants.getLevelDisplayableName(ammoTechLvl));
                        buff.append(", ");
                        buff.append(eTLYear);
                        buff.append("):\n");
                    }
                    ...
                    buff.append(", (");
                    buff.append(TechConstants.getLevelDisplayableName(eqTechLevel));
                    buff.append(")\n");
                }
            } else if (illegal) {
                if (!retVal) {
        ...
        if (cockpit != null) {
            ...
            if (illegal) {
                ...
                buff.append(TechConstants.getLevelDisplayableName(eTechLevel));
                buff.append(", ");
                buff.append(eTLYear);
                buff.append("): ");
                buff.append(cockpitName);
                buff.append(" (");
                buff.append(TechConstants.getLevelDisplayableName(TechConstants.convertFromSimplelevel(eqRulesLevel,
                      cockpit.isClan())));
                buff.append(")\n");
                retVal = true;
            }
        ...
        if (!getEntity().hasPatchworkArmor()) {
            ...
            int eqRulesLevel = getEntity().isMixedTech() ?
                                     ...
                                     Entity.getPatchworkArmorAdvancement()
                                           .findMinimumRulesLevel(getEntity().isClan())
                                           .ordinal();
        ...
        for (String atName : armors) {
            ...
            int eqRulesLevel = getEntity().isMixedTech() ?
                                     at.findMinimumRulesLevel().ordinal() :
    ...
    public boolean hasIllegalEquipmentCombinations(StringBuffer buff) {
        ...
        for (Mounted<?> m : getEntity().getMisc()) {
            ...
            if (m.getType().hasFlag(MiscType.F_SRCS) ||
        ...
        if (hasExternalFuelTank &&
                  (!getEntity().hasEngine() ||
                         ((getEntity().getEngine().getEngineType() != Engine.COMBUSTION_ENGINE) &&
        ...
        // the location index.
        Map<Integer, Long> modArmorByLocation = getEntity().getMisc()
                                                      .stream()
        ...
        for (Integer loc : modArmorByLocation.keySet()) {
            if (modArmorByLocation.get(loc) > 1) {
                buff.append("Only one modular armor slot may be mounted in a single location (")
                      .append(getEntity().getLocationName(loc))
                      .append(")\n");
                illegal = true;
            }
        }
        ...
        if (artemisIV + artemisV + artemisP > 0) {
            if (((artemisIV > 0) && (artemisV + artemisP > 0)) || ((artemisV > 0) && (artemisP > 0))) {
                buff.append("All Artemis systems must be of the same type.\n");
                illegal = true;
            }
            illegal |= checkIllegalArtemisApolloLinks(buff,
                  artemisIV + artemisV + artemisP,
                  "Artemis",
                  w -> w.hasFlag(WeaponType.F_ARTEMIS_COMPATIBLE));
        }
        if (apollo > 0) {
            illegal |= checkIllegalArtemisApolloLinks(buff, apollo, "Apollo", w -> w.getAmmoType() == AmmoType.T_MRM);
        }

        return illegal;
    }

    private boolean checkIllegalArtemisApolloLinks(StringBuffer buffer, int expected, String testingEquipment,
        ...
        for (Mounted<?> mount : getEntity().getTotalWeaponList()) {
            if (!mount.isWeaponGroup() && compatibility.test((WeaponType) mount.getType())) {
                linkedCount++;
    ...
    public double getWeightCarryingSpace() {
        ...
        for (Bay bay : getEntity().getTransportBays()) {
            if (!bay.isQuarters()) {
                weight = TestEntity.ceil(weight + bay.getWeight(), Ceil.KILO);
            }
        }
    ...
    public String printWeightCarryingSpace() {
        ...
        if (getEntity().getTroopCarryingSpace() != 0) {
            carryingSpace = StringUtil.makeLength("Carrying Capacity:", getPrintSize() - 5) +
                                  TestEntity.makeWeightString(getEntity().getTroopCarryingSpace(), usesKgStandard()) +
                                  "\n";
        }
        ...
        if (cargoWeight > 0) {
            cargoWeightString = StringUtil.makeLength("Cargo Weight:", getPrintSize() - 5) +
