...
public class RulerCommand extends ServerCommand {
    ...
    public void run(int connId, String[] args) {
        try {
            ...
            thd = LosEffects.calculateLos(server.getGame(), buildAttackInfo(start, end, elev1, elev2)).losModifiers(server.getGame());
            if (thd.getValue() != TargetRoll.IMPOSSIBLE) {
                toHit1 = thd.getValue() + " because "; //$NON-NLS-1$
            }
            toHit1 += thd.getDesc();
            
            thd = LosEffects.calculateLos(server.getGame(), buildAttackInfo(end, start, elev2, elev1)).losModifiers(server.getGame());
            if (thd.getValue() != TargetRoll.IMPOSSIBLE) {
                toHit2 = thd.getValue() + " because  "; //$NON-NLS-1$
            }
            toHit2 += thd.getDesc();
            
            server.sendServerChat(connId, "The ToHit from hex (" + (start.x+1) + ", " + (start.y+1) + ") at elevation "
