...
class MovePathHandler extends AbstractTWRuleHandler {
     ...
     */
    MovePathHandler(TWGameManager gameManager,
                    Entity entity,
                    MovePath md,
     ...
     */
    private void moveElevatorPlatformWithEntity(MoveStep step,
                                                Coords curPos,
                                                int curBoardId,
                                                int curElevation) {
     ...
     */
    private boolean entityFallsDownElevatorShaft(MoveStep step,
                                                 Hex curHex,
                                                 Coords curPos,
                                                 int curBoardId,
                                                 int curElevation,
                                                 EntityMovementType stepMoveType) {
        boolean isElevatorStep = (step.getType() == MoveStepType.ELEVATOR_ASCEND) || (step.getType() == MoveStepType.ELEVATOR_DESCEND);
        if (!curHex.containsTerrain(Terrains.INDUSTRIAL_ELEVATOR) || (stepMoveType == EntityMovementType.MOVE_JUMP) || isElevatorStep) {
            return false;
        }
        IndustrialElevator elevator = getGame().getIndustrialElevator(BoardLocation.of(curPos, curBoardId));
     ...
     * @param curElevation the unit's elevation after the step
     */
    private void reportElevatorUse(MoveStep step,
                                   int curElevation) {
        if ((step.getType() != MoveStepType.ELEVATOR_ASCEND) && (step.getType() != MoveStepType.ELEVATOR_DESCEND)) {
            return;
        }
        String direction = (step.getType() == MoveStepType.ELEVATOR_ASCEND) ? Messages.getString(
                "IndustrialElevator.directionUp") : Messages.getString("IndustrialElevator.directionDown");
     ...
     * records 1 MP spent, so an airborne beast-mounted platoon (e.g. Branth) that holds its hex still counts as having
     * walked.
     *
    ...
    static boolean mustSpendStationaryFlightMP(Entity entity) {
        return (entity instanceof ConvInfantry beastInfantry) && (beastInfantry.getMount() != null) && beastInfantry.getMount()
                                                                                                                    .movementMode()
                                                                                                                    .isVTOL() && entity.isAirborneVTOLorWIGE() && (entity.moved == EntityMovementType.MOVE_NONE);
    }
    ...
    void processMovement() {
        ...
        if (entity.isClimbing() && (entity instanceof Mek climbingMek)) {
            ...
            boolean cannotHoldOn = (climbableArms == 0) || (entity.isDangling() && (climbableArms < 2));
            if (cannotHoldOn) {
                logger.debug("[FALL-TRACE] Climbing/dangling Mek {} lost required arms, auto-falling " + "from elevation {} in hex {}",
                ...
                gameManager.sendServerChat(Messages.getString("MovementDisplay.ClimbingDialog.armsLostChat",
                                                              entity.getDisplayName()));
                ...
                PilotingRollData autoFallRoll = new PilotingRollData(entity.getId(),
                                                                     ...
                                                                     "climbing arms destroyed");
                addReport(gameManager.doEntityFallsInto(entity,
                                                        entity.getElevation(),
                                                        entity.getPosition(),
                                                        entity.getPosition(),
                                                        autoFallRoll,
        ...
        boolean isJumpPath = md.contains(MoveStepType.START_JUMP) || (md.getLastStepMovementType() == EntityMovementType.MOVE_JUMP);
        ...
        if (!entity.isClimbing() && !entity.isDangling() && (entity instanceof Mek) && ClimbingHelper.canClimb(entity) && getGame().getOptions()
                                                                                                                                   .booleanOption(
                                                                                                                                           OptionsConstants.ADVANCED_GROUND_MOVEMENT_TAC_OPS_CLIMBING) && md.getFinalClimbMode() && md.contains(
                MoveStepType.FORWARDS) && !isJumpPath) {
        ...
        boolean canProcessDangle = tacOpsClimbingEnabled && (entity.isClimbing() || entity.isDangling()) && hasDownStep;
        logger.debug("[DANGLE-TRACE] processMovement check: canProcessDangle={}, isEdgeDangle={}, " + "hasDownStep={}, isClimbing={}, isDangling={}, elevation={}, climbMode={}, stepCount={}",
                     canProcessDangle,
                     isEdgeDangle,
                     hasDownStep,
                     entity.isClimbing(),
        ...
        if (isEdgeDangle && (edgeDangleTargetPos != null)) {
            ...
            if ((chosenEdgeDescent > 0) && (entity instanceof Mek edgeMek) && ClimbingHelper.canClimb(entity)) {
                ...
                logger.debug("[CLIMB-TRACE] Server processing EDGE climb-down: entity={}, " + "from={} (alt {}), to={} (level {}), levelsToDescend={}, costPerLevel={}",
                             entity.getDisplayName(),
                             originalPos,
                             cliffTopAlt,
                             edgeDangleTargetPos,
                ...
                for (int i = 1; i <= levelsToDescend; i++) {
                    ...
                    if (gameManager.doSkillCheckWhileMoving(entity,
                                                            psrElevation,
                                                            edgeDangleTargetPos,
                                                            edgeDangleTargetPos,
                                                            psr,
                ...
                if (!fellWhileDescending) {
                    ...
                    int finalElevation = Math.max(floorRelative, (cliffTopAlt - destHex.getLevel()) - levelsDescended);
            ...
            if (!ClimbingHelper.canDangle(entity)) {
                logger.warn("[DANGLE-TRACE] Server rejecting EDGE dangle: entity={} lacks two " + "functional climbing arms; chosenLevels was 0 (dangle path).",
                            entity.getDisplayName());
                entity.setDone(true);
            ...
            int dangleElevation = cliffTopAlt - ClimbingHelper.DANGLE_LEVELS_PER_TURN - destHex.getLevel();
            dangleElevation = Math.max(0, dangleElevation);
            logger.debug("[DANGLE-TRACE] Server processing EDGE dangle: entity={}, " + "from={} (alt {}), to={} (level {}), dangleElevation={}",
                         entity.getDisplayName(),
                         originalPos,
                         cliffTopAlt,
                         edgeDangleTargetPos,
        ...
        if (canProcessDangle) {
            int downStepCount = (int) md.getStepVector().stream().filter(s -> s.getType() == MoveStepType.DOWN).count();
            ...
            if (hasClimbModeOnMarker && (downStepCount > 0) && (entity instanceof Mek descendingMek) && ClimbingHelper.canClimb(
                    entity)) {
                ...
                if (levelsToDescend < requestedLevels) {
                    logger.warn("[CLIMB-TRACE] Server capping CLIMB_DOWN: entity={} requested {} " + "levels but {} walk MP at {} MP/level only allows {} (clinging at " + "intermediate elevation).",
                                entity.getDisplayName(),
                                requestedLevels,
                                availableMP,
                                costPerLevel,
                ...
                logger.debug("[CLIMB-TRACE] Server processing CLIMB_DOWN: entity={}, " + "currentElevation={}, levelsToDescend={}, costPerLevel={}",
                             entity.getDisplayName(),
                             currentElevation,
                             levelsToDescend,
                             costPerLevel);
                ...
                if (!fellWhileDescending) {
                    int newElevation = Math.max(floorRelativeForDescent, currentElevation - levelsDescended);
            ...
            if (isDrop) {
                ...
                if (availableMP < ClimbingHelper.DROP_MP_COST) {
                    logger.warn("[DANGLE-TRACE] Server rejecting DROP: entity={} has {} walk MP, " + "needs {} for drop. Treating as cling.",
                                entity.getDisplayName(),
                                availableMP,
                ...
                logger.debug("[DANGLE-TRACE] Server processing DROP: entity={}, " + "dropDistance={}, effectiveDistance={}, isDangling={}, modifierReduction={}",
                             entity.getDisplayName(),
                             dropDistance,
                             effectiveDistance,
                             entity.isDangling(),
                ...
                String legDamageDesc = entity.isDangling() ? String.format(
                        "hanging from level %d, effective height for leg damage roll %d",
                        dropDistance,
                        effectiveDistance) : String.format("dropping from level %d, leg damage roll", dropDistance);
                String fallDesc = entity.isDangling() ? String.format(
                        "hanging from level %d, effective height for fall roll %d",
                        dropDistance,
                        effectiveDistance) : String.format("dropping from level %d, fall roll", dropDistance);
                if (effectiveDistance > 0) {
                    rollTarget = entity.getBasePilotingRoll(EntityMovementType.MOVE_WALK);
                    entity.addPilotingModifierForTerrain(rollTarget, entity.getPosition(), entity.getBoardId());
                    rollTarget.append(new PilotingRollData(entity.getId(), 2 * effectiveDistance, legDamageDesc));
                    if (0 < gameManager.doSkillCheckWhileMoving(entity,
                                                                entity.getElevation(),
                                                                entity.getPosition(),
                                                                entity.getPosition(),
                                                                rollTarget,
                        ...
                        addReport(gameManager.damageEntity(entity, new HitData(Mek.LOC_LEFT_LEG), effectiveDistance));
                        addReport(gameManager.damageEntity(entity, new HitData(Mek.LOC_RIGHT_LEG), effectiveDistance));
                        addNewLines();
                    ...
                    rollTarget = entity.getBasePilotingRoll(EntityMovementType.MOVE_WALK);
                    entity.addPilotingModifierForTerrain(rollTarget, entity.getPosition(), entity.getBoardId());
                    rollTarget.append(new PilotingRollData(entity.getId(), effectiveDistance, fallDesc));
                    if (0 < gameManager.doSkillCheckWhileMoving(entity,
                                                                entity.getElevation(),
                                                                entity.getPosition(),
                                                                entity.getPosition(),
                ...
                int landingElevation = (landingHex != null) ? landingHex.floor() - landingHex.getLevel() : 0;
                ...
                int dangleLevels = Math.min(ClimbingHelper.DANGLE_LEVELS_PER_TURN, dangleableLevels);
                int newElevation = Math.max(dangleFloor, entity.getElevation() - dangleLevels);
                logger.debug("[DANGLE-TRACE] Server processing dangle: entity={}, " + "currentElevation={}, dangleLevels={}, newElevation={}",
                             entity.getDisplayName(),
                             entity.getElevation(),
                             dangleLevels,
                             newElevation);
        ...
        if (md.contains(MoveStepType.EJECT)) {
            if (entity.isLargeCraft() && !entity.isCarcass()) {
                ...
                addReport(gameManager.ejectSpacecraft(ship,
                                                      ship.isSpaceborne(),
                                                      (ship.isAirborne() && !ship.isSpaceborne()),
                                                      legalPos));
        ...
        if (md.contains(MoveStepType.TAKEOFF) && entity.isAero()) {
            if (!usingAeroOnGroundMovement() && !MovementDisplay.hasAtmosphericMapForLiftOff(getGame(), entity)) {
                ...
                if (entity instanceof Dropship) {
                    gameManager.applyDropShipProximityDamage(md.getFinalCoords(),
                                                             boardId,
                                                             true,
                                                             md.getFinalFacing(),
                                                             entity);
        ...
        if ((md.contains(MoveStepType.LAND) || md.contains(MoveStepType.VERTICAL_LAND)) && entity.isAero()) {
            ...
            Coords finalPosition = isVertical ? md.getFinalCoords() : md.getFinalCoords()
                                                                        .translated(md.getFinalFacing(),
                                                                                    aero.getLandingLength());
            gameManager.checkLandingTerrainEffects(aero,
                                                   isVertical,
                                                   md.getFinalCoords(),
                                                   finalPosition,
                                                   md.getFinalBoardId(),
        ...
        if (md.contains(MoveStepType.DEPLOY) && Game.rulesManager.getRulesGame().isWalkOnDeployment()) {
            ...
            if (step != null) {
                ...
                entityElevation = step.getElevation();
            } else {
        ...
        if (startingHex == null) {
            logger.warn(String.format("Attempted to skip turn for %s (%s) which is not on the board!",
                                      entity,
        ...
        if (tookMagmaDamageAtStart && prevHex.terrainLevel(Terrains.MAGMA) == 2 && !(entity.getElevation() > 0 || entity.getMovementMode() == EntityMovementMode.HOVER)) {
        ...
        if ((entity.isClimbing() || entity.isDangling()) && (entity.getElevation() == 0)) {
            ...
            boolean onBuildingRoof = (finalHex != null) && finalHex.containsTerrain(Terrains.BUILDING) && (entity.getElevation() >= finalHex.terrainLevel(
                    Terrains.BLDG_ELEV));
            boolean clingingToAdjacentClimbable = entity.isClimbing() && isClingingToAdjacentClimbable(entity,
                                                                                                       curPos,
                                                                                                       finalHex);
            if (!onBuildingRoof && !clingingToAdjacentClimbable) {
                logger.debug("[CLIMB-TRACE] Clearing stale climbing flags at end of move: entity={}, " + "climbing={}, dangling={}, elevation=0 in hex {}",
                             entity.getDisplayName(),
                             entity.isClimbing(),
                             entity.isDangling(),
                             curPos);
        ...
        if (md.isAllUnderwater(getGame())) {
            ...
            if ((entity instanceof ConvInfantry infantry) && (infantry.getMount() != null) && entity.getMovementMode()
                                                                                                    .isSubmarine() && entity.underwaterRounds > infantry.getMount()
                                                                                                                                                        .getUWEndurance()) {
        ...
        rollTarget = Game.rulesManager.getRulesPSR()
                                      .checkWalkWithLegDestroyed(entity, overallMoveType, md.getHexesMoved());
        if (rollTarget.getValue() != TargetRoll.CHECK_FALSE && entity.canFall()) {
            gameManager.doSkillCheckInPlace(entity, rollTarget);
        }

        // if we sprinted with MASC or a supercharger, then we need a PSR
        rollTarget = entity.checkSprintingWithMASCXorSupercharger(overallMoveType, entity.mpUsed);
        if (rollTarget.getValue() != TargetRoll.CHECK_FALSE && entity.canFall()) {
            gameManager.doSkillCheckInPlace(entity, rollTarget);
        }
        ...
        // pilot damage on a 2
        if ((entity instanceof ProtoMek) && ((ProtoMek) entity).hasMyomerBooster() && (md.getMpUsed() > entity.getRunMP(
                MPCalculationSetting.NO_MYOMER_BOOSTER))) {
        ...
        if ((md.getLastStepMovementType() == EntityMovementType.MOVE_SPRINT) && (md.hasActiveMASC() || md.hasActiveSupercharger()) && entity.canFall()) {
        ...
        if (entity.isAirborne() && entity.isAero()) {
            ...
            // consume fuel
            if (((entity.isAero()) && getGame().getOptions()
                                               .booleanOption(OptionsConstants.ADVANCED_AERO_RULES_FUEL_CONSUMPTION)) || (entity instanceof TeleMissile)) {
                int fuelUsed = ((IAero) entity).getFuelUsed(thrust);

                // if we're a gas hog, aerospace fighter and going faster than walking, then use
                // 2x fuel
                if (((overallMoveType == EntityMovementType.MOVE_RUN) || (overallMoveType == EntityMovementType.MOVE_SPRINT) || (overallMoveType == EntityMovementType.MOVE_OVER_THRUST)) && entity.hasQuirk(
                        OptionsConstants.QUIRK_NEG_GAS_HOG)) {
                    fuelUsed *= 2;
                }
            ...
            if (!getGame().getBoard(entity.getBoardId()).isSpace()) {
                ...
                if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                    getGame().addControlRoll(new PilotingRollData(entity.getId(), 0, "hovering"));
                }
                ...
                if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                    ...
                    report.addDesc(entity);
                    addReport(report);
                    getGame().addControlRoll(new PilotingRollData(entity.getId(), 0, "stalled out"));
                ...
                if (a.isSpheroid() && !a.isSpaceborne() && a.isAirborne() && (md.getFinalNDown() == 0) && (md.getMpUsed() == 0)) {
        ...
        if (entity.isHullDown() && !md.getFinalHullDown() && (entity instanceof Tank || (entity instanceof QuadVee && entity.getConversionMode() == QuadVee.CONV_MODE_VEHICLE))) {
        ...
        if ((overallMoveType == EntityMovementType.MOVE_JUMP) && !entity.isMakingDfa()) {
            ...
            if (md.shouldMechanicalJumpCauseFallDamage()) {
                gameManager.getMainPhaseReport()
                           .addAll(gameManager.doEntityFallsInto(entity,
                                                                 entity.getElevation(),
                                                                 md.getJumpPathHighestPoint(),
                                                                 curPos,
                                                                 entity.getBasePilotingRoll(overallMoveType),
                                                                 false,
                                                                 entity.getMechanicalJumpBoosterMP()));
            }
            ...
            } else if (!(prevStep.climbMode() && curHex.containsTerrain(Terrains.BRIDGE)) && !(entity.getMovementMode()
                                                                                                     .isHoverOrWiGE())) {
            ...
            if ((useBlackIce && goodTemp) || goodWeather) {
                if (ServerHelper.checkEnteringBlackIce(gameManager,
                                                       curPos,
                                                       curHex,
                                                       useBlackIce,
                                                       goodTemp,
            ...
            if (bldg != null) {
                gameManager.checkForCollapse(bldg, curPos, true, gameManager.getMainPhaseReport());
            }

            // Don't interact with terrain when jumping onto a building or a bridge
            if (entity.getElevation() == 0) {
                ServerHelper.checkAndApplyMagmaCrust(curHex,
                                                     entity.getElevation(),
                                                     entity,
                                                     curPos,
                                                     true,
                ...
                if (curHex.getBogDownModifier(entity.getMovementMode(),
                    ...
                    if (entity instanceof Mek) {
                        ...
                        rollTarget = new PilotingRollData(entity.getId(), 5, "entering boggy terrain");
            ...
            if (entity instanceof Tank) {
                int modifier = 0;
                if (curHex.containsTerrain(Terrains.ROUGH) || curHex.containsTerrain(Terrains.WOODS) || curHex.containsTerrain(
                        Terrains.JUNGLE)) {
                    modifier = 1;
                }
        ...
        if (entity.isConvertingNow()) {
            ...
            report.subject = entity.getId();
            report.addDesc(entity);
            if (entity instanceof QuadVee && entity.isProne() && entity.getConversionMode() == QuadVee.CONV_MODE_MEK) {
                ...
                if (entity.isAero()) {
                    if (md.getFinalConversionMode() == EntityMovementMode.WIGE && entity.getAltitude() > 0 && entity.getAltitude() <= 3) {
                ...
                } else if (entity.getMovementMode() == EntityMovementMode.TRACKED || entity.getMovementMode() == EntityMovementMode.WHEELED) {
        ...
        boolean continueTurnFromFall = !(getGame().getOptions()
                                                  .booleanOption(OptionsConstants.ADVANCED_GROUND_MOVEMENT_FALLS_END_MOVEMENT) && (entity instanceof Mek) && !wasProne && entity.isProne()) && (fellDuringMovement && !entity.isCarefulStand()) // Careful standing takes up the whole turn
                                       && !turnOver && (entity.mpUsed < entity.getRunMP()) && (overallMoveType != EntityMovementType.MOVE_JUMP);
        if ((continueTurnFromFall || continueTurnFromPBS || continueTurnFromFishtail || continueTurnFromLevelDrop || continueTurnFromCliffAscent || detectedHiddenHazard) && entity.isSelectableThisTurn() && !entity.isDoomed()) {
            entity.applyDamage();
            ...
            if (entity.getMovementMode() == EntityMovementMode.WIGE) {
                ...
                if (md.automaticWiGELanding(false)) {
                    ...
                    if (hex.containsTerrain(Terrains.BLDG_ELEV)) {
                        ...
                        gameManager.addAffectedBldg(bldg,
                                                    gameManager.checkBuildingCollapseWhileMoving(bldg,
                                                                                                 entity,
                                                                                                 entity.getPosition()));
                ...
                } else if (!entity.hasETypeFlag(Entity.ETYPE_LAND_AIR_MEK) && !entity.hasETypeFlag(Entity.ETYPE_PROTOMEK)) {
                    ...
                    entity.setElevation(Math.min(entity.getElevation(),
                                                 1 + hex.maxTerrainFeatureElevation(getGame().getBoard(curBoardId)
                                                                                             .isLowAltitude())));
                }
            }
            ...
            if (entity instanceof LandAirMek && (entity.isLocationBad(Mek.LOC_RIGHT_TORSO) || entity.isLocationBad(Mek.LOC_LEFT_TORSO))) {
                report = new Report(9710);
                report.subject = entity.getId();
                report.addDesc(entity);
                if (entity.isAirborneVTOLorWIGE()) {
                    addReport(report);
                    gameManager.crashAirMek(entity,
                    ...
                    addReport(report);
                    addReport(gameManager.processCrash(entity,
        ...
        if (!getGame().isOutOfGame(entity)) {
            ...
            if (entity.isDoomed()) {
                gameManager.send(gameManager.createRemoveEntityPacket(entity.getId(), entity.getRemovalCondition()));
            }
        }
        ...
        if ((entity instanceof Mek) && entity.hasEngine() && ((Mek) entity).isIndustrial() && !entity.hasEnvironmentalSealing() && (entity.getEngine()
                                                                                                                                          .getEngineType() == Engine.COMBUSTION_ENGINE)) {
            if ((!entity.isProne() && (getGame().getBoard(curBoardId)
                                                .getHex(entity.getPosition())
                                                .terrainLevel(Terrains.WATER) >= 2)) || (entity.isProne() && (getGame().getBoard(
                    curBoardId).getHex(entity.getPosition()).terrainLevel(Terrains.WATER) == 1))) {
     ...
     */
    private boolean isClingingToAdjacentClimbable(Entity entity,
                                                  Coords curPos,
                                                  @Nullable Hex curHex) {
        if (curHex == null) {
            return false;
        ...
        if (adjacentHex.containsTerrain(Terrains.BRIDGE) && (adjacentBase + adjacentHex.terrainLevel(Terrains.BRIDGE_ELEV)) > entityAbsAlt) {
            return true;
        }
        if (adjacentHex.containsTerrain(Terrains.BUILDING) && (adjacentBase + adjacentHex.terrainLevel(Terrains.BLDG_ELEV)) > entityAbsAlt) {
            return true;
        }
        ...
        for (final ListIterator<MoveStep> i = md.getSteps(); i.hasNext(); ) {
            ...
            if (getGame().getOptions().booleanOption(OptionsConstants.ADVANCED_HIDDEN_UNITS)) {
                for (Entity hiddenEntity : hiddenEnemies) {
                    ...
                    // next clause).
                    if ((dist == 0) && !continueTurnFromPBS && (Compute.stackingViolation(getGame(),
                                                                                          this.entity,
                                                                                          step.getPosition(),
                                                                                          null,
                                                                                          this.entity.climbMode(),
                                                                                          false) != null)) {
                        ...
                        if (jumping) {
                            ...
                            addReport(gameManager.doEntityDisplacement(this.entity,
                                                                       previousStep.getPosition(),
                                                                       step.getPosition(),
                                                                       new PilotingRollData(this.entity.getId(),
                                                                                            0,
                    ...
                    } else if ((dist <= 1) && !hiddenEntity.madePointblankShot() && ((!this.entity.isAirborne() && md.isEndStep(
                            step)) || (this.entity.isAirborne() && (dist == ((this.entity.getBAPRange() > 0) ? 1 : 0))))) {
            ...
            if (firstStep) {
                if (entity instanceof VTOL) {
                    ...
                    // with
                    // likely reduced MP.
                    fellDuringMovement = gameManager.checkMASCFailure(entity,
                                                                      md) || gameManager.checkSuperchargerFailure(entity,
                                                                                                                  md);
                }
            }
            ...
            if (firstStep) {
                ...
                if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                    int mof = gameManager.doSkillCheckWhileMoving(entity,
                                                                  lastElevation,
                                                                  lastPos,
                                                                  curPos,
                                                                  rollTarget,
            ...
            if (step.getType() == MoveStepType.CONVERT_MODE) {
                ...
                if (entity instanceof QuadVee && entity.getConversionMode() == QuadVee.CONV_MODE_MEK && !entity.isOmni()) {
                    for (Entity rider : entity.getExternalUnits()) {
                        addReport(gameManager.checkDropBAFromConverting(entity,
                                                                        rider,
                                                                        curPos,
                                                                        curFacing,
                                                                        false,
            ...
            if (entity.isAirborne() && entity.isAero()) {
                ...
                if (!didMove && (md.length() != j)) {
                    ...
                    if ((rollTarget.getValue() != TargetRoll.CHECK_FALSE) && !(entity instanceof FighterSquadron) && !getGame().useVectorMove()) {
                        if (!gameManager.doSkillCheckInSpace(entity, rollTarget)) {
                            ...
                            if (a.getSI() == 0) {
                                ...
                                if (a instanceof LandAirMek lam) {
                                    ...
                                    if (lam.isAutoEject() && (!getGame().getOptions()
                                                                        .booleanOption(OptionsConstants.RPG_CONDITIONAL_EJECTION) || (getGame().getOptions()
                                                                                                                                               .booleanOption(
                                                                                                                                                       OptionsConstants.RPG_CONDITIONAL_EJECTION) && lam.isCondEjectCTDest()))) {
                                        addReport(gameManager.ejectEntity(entity, true, false));
                                    }
                                } else {
                                    // Aeros eject if the SI Destroyed switch is on
                                    Aero aero = (Aero) a;
                                    if (aero.isAutoEject() && (!getGame().getOptions()
                                                                         .booleanOption(OptionsConstants.RPG_CONDITIONAL_EJECTION) || (getGame().getOptions()
                                                                                                                                                .booleanOption(
                                                                                                                                                        OptionsConstants.RPG_CONDITIONAL_EJECTION) && aero.isCondEjectSIDest()))) {
                                        addReport(gameManager.ejectEntity(entity, true, false));
                                    }
                                }
                                addReport(gameManager.destroyEntity(entity, "Structural Integrity Collapse", false));
                            }
                        }
                    }
                    ...
                    if ((thrustUsed > (2 * health)) && !getGame().useVectorMove() && !(entity instanceof TeleMissile)) {
                ...
                if (step.getType() == MoveStepType.LAUNCH) {
                    ...
                    while (bayIter.hasNext()) {
                        ...
                        for (int fighterId : launches) {
                            ...
                            if (!gameManager.launchUnit(entity,
                                                        fighter,
                                                        curPos,
                                                        curFacing,
                                                        step.getVelocity(),
            ...
            if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                ...
                if ((entity instanceof Mek) && entity.hasEngine() && !(((Mek) entity).isIndustrial() && ((entity.getEngine()
                                                                                                                .getEngineType() == Engine.COMBUSTION_ENGINE) || (entity.getEngine()
            ...
            if ((step.getType() == MoveStepType.SEARCHLIGHT) && entity.hasSearchlight()) {
                ...
                if (gameManager.doBlind()) { // if double-blind, we may need to filter the
                    ...
                    gameManager.sendServerChat(entity.getDisplayName() + " switched searchlight " + (SearchOn ? "on" : "off") + '.');
                }
            }
            ...
            if (cachedGravityLimit < 0) {
                cachedGravityLimit = EntityMovementType.MOVE_JUMP == moveType ? (step.isUsingMekJumpBooster() ? entity.getMechanicalJumpBoosterMP(
                        MPCalculationSetting.NO_GRAVITY) : entity.getJumpMP(MPCalculationSetting.NO_GRAVITY)) : entity.getRunningGravityLimit();
            }
            // check for charge
            if (step.getType() == MoveStepType.CHARGE) {
                if (entity.canCharge()) {
                    ...
                    Targetable target = step.getTarget(getGame());
                    if (target != null) {
                        AirMekRamAttackAction raa = new AirMekRamAttackAction(entity.getId(),
                                                                              target.getTargetType(),
                                                                              target.getId(),
                                                                              target.getPosition());
                    ...
                    gameManager.sendServerChat("Illegal charge!! I don't think " + entity.getDisplayName() + " should be allowed to charge," + " but the client of " + entity.getOwner()
                                                                                                                                                                             .getName() + " disagrees.");
                    gameManager.sendServerChat("Please make sure " + entity.getOwner()
                                                                           .getName() + " is running MegaMek " + MMConstants.VERSION + ", or if that is already the case, submit a bug report at https://github.com/MegaMek/megamek/issues");
                    return;
                }
                break;
            }
            ...
            if (step.getType() == MoveStepType.DFA) {
                if (entity.canDFA()) {
                    ...
                    if (target != null) {
                        ...
                        String errorMessage = "Illegal DFA by " + entity.getDisplayName() + " against non-existent entity at " + step.getPosition();
                    ...
                    DfaAttackAction daa = new DfaAttackAction(entity.getId(), targetType, targetID, step.getPosition());
                    entity.setDisplacementAttack(daa);
                    entity.setElevation(Game.rulesManager.getRulesMovement()
                                                         .getDFAElevation(entity.getGame(),
                                                                          entity.getId(),
                                                                          targetID,
                                                                          step));
                    ...
                    gameManager.sendServerChat("Illegal DFA!! I don't think " + entity.getDisplayName() + " should be allowed to DFA," + " but the client of " + entity.getOwner()
                                                                                                                                                                       .getName() + " disagrees.");
                    gameManager.sendServerChat("Please make sure " + entity.getOwner()
                                                                           .getName() + " is running MegaMek " + MMConstants.VERSION + ", or if that is already the case, submit a bug report at https://github.com/MegaMek/megamek/issues");
                    return;
            ...
            if (step.getType() == MoveStepType.RAM) {
                if (entity.canRam()) {
                    ...
                    gameManager.sendServerChat("Illegal ram!! I don't think " + entity.getDisplayName() + " should be allowed to charge," + " but the client of " + entity.getOwner()
                                                                                                                                                                          .getName() + " disagrees.");
                    gameManager.sendServerChat("Please make sure " + entity.getOwner()
                                                                           .getName() + " is running MegaMek " + MMConstants.VERSION + ", or if that is already the case, submit a bug report at https://github.com/MegaMek/megamek/issues");
                    return;
            ...
            if (entity instanceof Infantry inf) {
                if (step.getType() == MoveStepType.DIG_IN) {
                    ...
                    if (!inf.hasWorkingMisc(MiscType.F_TRENCH_CAPABLE)) {
                        logger.debug("[Fortify] {}: fortify rejected - no fieldworks-capable equipment " + "(F_TRENCH_CAPABLE)",
                                     entity.getDisplayName());
                        continue;
                    }
                    inf.beginFortify();
                ...
                if (step.getType() == MoveStepType.TAKE_COVER) {
                    if (Infantry.hasValidCover(getGame(), step.getPosition(), step.getElevation())) {
                        inf.setTakingCover(true);
                    } else {
                        gameManager.sendServerChat(entity.getDisplayName() + " failed to take cover: " + "no valid unit found in " + step.getPosition());
                    }
                }
            }
            ...
            // requirements.
            if ((step.getType() == MoveStepType.TURN_LEFT || step.getType() == MoveStepType.TURN_RIGHT) && entity.usesTurnMode()) {
                int straight = 0;
                if (prevStep != null) {
            ...
            if (step.getType() == MoveStepType.BOOTLEGGER) {
                ...
                int mof = gameManager.doSkillCheckWhileMoving(entity, lastElevation, curPos, curPos, rollTarget, false);
                if (mof > 0) {
            ...
            // check for leap
            if (!lastPos.equals(curPos) && (stepMoveType != EntityMovementType.MOVE_JUMP) && (entity instanceof Mek) && !entity.isAirborne() && (step.getClearance() <= 0) // Don't check airborne LAMs
                && getGame().getOptions().booleanOption(OptionsConstants.ADVANCED_GROUND_MOVEMENT_TAC_OPS_LEAPING)) {
                int leapDistance = (lastElevation + getGame().getBoard(curBoardId)
                                                             .getHex(lastPos)
                                                             .getLevel()) - (curElevation + curHex.getLevel());
                logger.debug("[LEAP-TRACE] Leap check: lastPos={}, curPos={}, lastElevation={}, " + "curElevation={}, leapDistance={}, isClimbing={}, isDangling={}",
                             lastPos,
                             curPos,
                             lastElevation,
                             curElevation,
                             ...
                             entity.isDangling());
                if (leapDistance > 2) {
                    logger.debug("[LEAP-TRACE] Leaping {} levels from {} to {}", leapDistance, lastPos, curPos);
            ...
            if (!lastPos.equals(curPos) && (entity instanceof ConvInfantry infantry) && infantry.hasAbility(
                    OptionsConstants.MD_PL_GLIDER) && infantry.canUseGliderWings()) {
                int glideDistance = (lastElevation + getGame().getBoard(curBoardId)
                                                              .getHex(lastPos)
                                                              .getLevel()) - (curElevation + curHex.getLevel());
            ...
            // Check for skid.
            rollTarget = entity.checkSkid(moveType,
                                          prevHex,
                                          overallMoveType,
                                          prevStep,
                                          step,
            ...
            // check sideslip
            if ((entity instanceof VTOL) || (entity.getMovementMode() == EntityMovementMode.HOVER) || (entity.getMovementMode() == EntityMovementMode.WIGE && step.getClearance() > 0)) {
                rollTarget = entity.checkSideSlip(moveType,
                                                  prevHex,
                                                  overallMoveType,
                                                  prevStep,
                                                  prevFacing,
                ...
                if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                    ...
                    if (moF > 0) {
                        ...
                        int skidDirection;
                        Coords start;
                        if (step.getType() == MoveStepType.LATERAL_LEFT || step.getType() == MoveStepType.LATERAL_RIGHT || step.getType() == MoveStepType.LATERAL_LEFT_BACKWARDS || step.getType() == MoveStepType.LATERAL_RIGHT_BACKWARDS) {
                        ...
                        if (sideslipDistance > 0) {
                            ...
                            if ((entity.getElevation() == 0) && ((entity.getMovementMode() == EntityMovementMode.VTOL) || (entity.getMovementMode() == EntityMovementMode.WIGE))) {
                                turnOver = true;
                            }
                            // set entity parameters
                            curFacing = step.getFacing();
            ...
            rollTarget = entity.checkRubbleMove(step,
                                                overallMoveType,
                                                curHex,
                                                lastPos,
                                                curPos,
            ...
            if (rollTarget.getValue() != TargetRoll.CHECK_FALSE) {
                if (entity instanceof Mek) {
                    ...
                    if (0 < gameManager.doSkillCheckWhileMoving(entity,
                        ...
                        if ((0 == curHex.terrainsPresent()) || (step.getClearance() > 0)) {
                            if (entity instanceof VTOL) {
                        ...
                        addReport(gameManager.damageEntity(entity,
                                                           hit,
                                                           ChargeAttackAction.getDamageTakenBy(entity, entity)));
                        turnOver = true;
                        break;
                    }
                }
            ...
            if (curHex.terrainLevel(Terrains.MAGMA) != 2 || jumpedIntoMagma) {
                ...
                rollTarget = entity.checkBogDown(step,
                                                 lastStepMoveType,
                                                 curHex,
                                                 lastPos,
                                                 curPos,
            ...
            if (entity.tracksHeat() && !entity.isAirborne()) {
                if (!lastPos.equals(curPos) && (prevStep != null) && ((lastHex.containsTerrain(Terrains.FIRE) && (prevStep.getElevation() <= 1)) || (lastHex.containsTerrain(
                        Terrains.MAGMA) && (prevStep.getElevation() == 0))) && ((stepMoveType != EntityMovementType.MOVE_JUMP)
            ...
            if (!(entity instanceof Mek)) {
                boolean underwater = getGame().getBoard(curBoardId)
                                              .getHex(curPos)
                                              .containsTerrain(Terrains.WATER) && (getGame().getBoard(curBoardId)
                                                                                            .getHex(curPos)
                                                                                            .depth() > 0) && (step.getElevation() < getGame().getBoard(
                        curBoardId).getHex(curPos).getLevel());
                if (getGame().getBoard(curBoardId).getHex(curPos).containsTerrain(Terrains.FIRE) && !lastPos.equals(
                        curPos) && (stepMoveType != EntityMovementType.MOVE_JUMP) && (step.getElevation() <= 1) && !underwater) {
                    gameManager.doFlamingDamage(entity, curPos);
                }
            }

            if ((getGame().getBoard(curBoardId)
                          .getHex(curPos)
                          .terrainLevel(Terrains.SMOKE) == SmokeCloud.SMOKE_GREEN) && !stepMoveType.equals(
                    EntityMovementType.MOVE_JUMP) && entity.antiTSMVulnerable()) {
            ...
            if ((getGame().getOptions()
                          .booleanOption(OptionsConstants.ADVANCED_TAC_OPS_BAP) || entity.isAerospace()) && !lastPos.equals(
                    curPos)) {
                if (ServerHelper.detectMinefields(getGame(),
                                                  entity,
                                                  curPos,
                                                  gameManager.getMainPhaseReport(),
                                                  gameManager) || ServerHelper.detectHiddenUnits(getGame(),
                                                                                                 entity,
                                                                                                 curPos,
                                                                                                 gameManager.getMainPhaseReport(),
                                                                                                 gameManager)) {
            ...
            // end of movement
            if (!lastPos.equals(curPos) || (lastElevation != curElevation) || ((stepMoveType == EntityMovementType.MOVE_JUMP) && !i.hasNext())) {
                boolean boom = false;
                if (isOnGround) {
                    ...
                    if (empBoom && !empReports.isEmpty()) {
                        gameManager.send(entity.getOwner().getId(), gameManager.createSpecialReportPacket(empReports));
                ...
                if (getGame().containsMinefield(curPos)) {
                    ...
                    entity.setPosition(curPos);
                    if (gameManager.enterMinefield(entity,
                                                   curPos,
                                                   step.getElevation(),
                                                   isOnGround,
                                                   gameManager.getMainPhaseReport())) {
            ...
            if ((lastElevation < 0) && (step.getElevation() == 0) && lastHex.containsTerrain(Terrains.ICE) && lastHex.containsTerrain(
                    Terrains.WATER) && (stepMoveType != EntityMovementType.MOVE_JUMP) && !lastPos.equals(curPos)) {
            ...
            if (curHex.containsTerrain(Terrains.ICE) && curHex.containsTerrain(Terrains.WATER) && (stepMoveType != EntityMovementType.MOVE_JUMP) && !lastPos.equals(
                    curPos) && !(entity instanceof Infantry) && !(isPavementStep && curHex.containsTerrain(Terrains.BRIDGE))) {
            ...
            if (step.getType() == MoveStepType.LOAD) {
                ...
                if (target != null) {
                    ...
                    Iterator<Entity> entities = getGame().getEntities(Compute.getLoadableCoords(entity,
                                                                                                curPos,
                                                                                                curBoardId));
            ...
            if (step.getType() == MoveStepType.MOUNT) {
                ...
                if (targetToMountInto instanceof Entity entityToMountInto) {
                    if (!entityToMountInto.canLoad(entity)) {
                        // Something is fishy in Denmark.
                        logger.error("(Mounted) {} can not load {}",
                                     entityToMountInto.getShortName(),
                                     entity.getShortName());
                    } else {
            ...
            if (step.getType() == MoveStepType.PICKUP_CARGO) {
                ...
                if ((!carryableObjects.isEmpty()) && (cargoPickupIndex != null) && (cargoPickupIndex >= 0) && (cargoPickupIndex < carryableObjects.size())) {
                    ...
                    if (entity.maxGroundObjectTonnage() >= pickupTarget.getTonnage() || ((entity.getTransports()
                                                                                                .size() > (Integer.MAX_VALUE - cargoPickupLocation)) && (entity.getTransports()
                                                                                                                                                               .get(Integer.MAX_VALUE - cargoPickupLocation) instanceof ExternalCargo externalCargo && externalCargo.canLoadCarryable(
                            pickupTarget)))) {
                        pickupTarget.processPickupStep(step, cargoPickupLocation, gameManager, entity, overallMoveType);
                    } else {
                        logger.warn(
            ...
            if (step.getType() == MoveStepType.UNLOAD) {
                ...
                if ((entity instanceof SmallCraft) && (unloaded instanceof Entity)) {
                    ...
                    if (!(unloaded instanceof Infantry)) {
                        GameTurn newTurn = new SpecificEntityTurn(((Entity) unloaded).getOwner().getId(),
                                                                  unloaded.getId());
                        // Need to set the new turn's multiTurn state
                        newTurn.setMultiTurn(true);
                        getGame().insertNextTurn(newTurn);
            ...
            if (((step.getType() == MoveStepType.BACKWARDS) || (step.getType() == MoveStepType.LATERAL_LEFT_BACKWARDS) || (step.getType() == MoveStepType.LATERAL_RIGHT_BACKWARDS)) && !(md.isJumping() && step.isUsingMekJumpBooster()) && (lastHex.getLevel() + lastElevation != curHex.getLevel() + step.getElevation()) && !(entity instanceof VTOL) && !(curClimbMode && curHex.containsTerrain(
                    Terrains.BRIDGE) && ((curHex.terrainLevel(Terrains.BRIDGE_ELEV) + curHex.getLevel()) == (prevHex.getLevel() + (prevHex.containsTerrain(
                    Terrains.BRIDGE) ? prevHex.terrainLevel(Terrains.BRIDGE_ELEV) : 0))))) {
                ...
                if ((entity instanceof Mek) && (curHex.getLevel() < getGame().getBoard(curBoardId)
                                                                             .getHex(lastPos)
                                                                             .getLevel()) && !entity.hasFallen()) {
                    rollTarget = entity.getBasePilotingRoll(overallMoveType);
                    rollTarget.addModifier(0, "moving backwards over an elevation change");
                    gameManager.doSkillCheckWhileMoving(entity,
                                                        entity.getElevation(),
                                                        curPos,
                                                        curPos,
                                                        rollTarget,
            ...
            if (stepMoveType != EntityMovementType.MOVE_JUMP && (step.getClearance() == 0 || (entity.getMovementMode()
                                                                                                    .isWiGE() && (step.getClearance() == 1)) || curElevation == curHex.terrainLevel(
                    Terrains.BLDG_ELEV) || curElevation == curHex.terrainLevel(Terrains.BRIDGE_ELEV))) {
                IBuilding bldg = getGame().getBoard(curBoardId).getBuildingAt(curPos);
                if ((bldg != null) && (entity.getElevation() >= 0)) {
                    boolean wigeFlyingOver = entity.getMovementMode() == EntityMovementMode.WIGE && ((curHex.containsTerrain(
                            Terrains.BLDG_ELEV) && curElevation > curHex.terrainLevel(Terrains.BLDG_ELEV)) || (curHex.containsTerrain(
            ...
            boolean mekAffectedByCliff = (entity instanceof Mek || entity instanceof ProtoMek) && moveType != EntityMovementType.MOVE_JUMP && !entity.isAero();
            ...
            boolean isUpCliff = !lastPos.equals(curPos) && curHex.hasCliffTopTowards(prevHex) && (stepHeight == 1 || stepHeight == 2);
            boolean isDownCliff = !lastPos.equals(curPos) && prevHex.hasCliffTopTowards(curHex) && (stepHeight == -1 || stepHeight == -2);
            ...
            if (mekAffectedByCliff && !quadveeVehMode && isDownCliff && !isPavementStep) {
                ...
                if (gameManager.doSkillCheckWhileMoving(entity, lastElevation, lastPos, curPos, rollTarget, true) > 0) {
                    addNewLines();
                    turnOver = true;
                    break;
                }
            ...
            if (step.isClimbing() && (entity instanceof Mek climbingMek) && (stepHeight > 0)) {
                ...
                logger.debug("Climbing: totalLevels={}, affordableLevels={}, levelsThisTurn={}, " + "walkMP={}, availableMP={}, costPerLevel={}, nonClimbMpUsed={}, " + "levelsAlreadyClimbed={}, overallClimbHeight={}",
                             totalLevelsToClimb,
                             affordableLevels,
                             levelsThisTurn,
                             walkMP,
                ...
                for (int levelClimbed = 1; levelClimbed <= levelsThisTurn; levelClimbed++) {
                    ...
                    if (gameManager.doSkillCheckWhileMoving(entity,
                        ...
                        if (!canFallFromHere) {
                            ...
                            logger.debug("[CLIMB-TRACE] Failed first climbing PSR from elevation 0; " + "no fall (entity stays on ground). entity={}, lastPos={}",
                                         entity.getDisplayName(),
                                         lastPos);
                        ...
                        curPos = entity.getPosition();
                        curVTOLElevation = entity.getElevation();
                        logger.debug("[FALL-TRACE] After doSkillCheckWhileMoving fall: " + "entity.position={}, entity.elevation={}, entity.isProne={}, " + "climbingElevation={}, lastPos={}, curPos={}, curVTOLElevation={}",
                                     entity.getPosition(),
                                     entity.getElevation(),
                                     entity.isProne(),
                                     climbingElevation,
                        ...
                        if (fallHex != null) {
                            logger.debug("[FALL-TRACE] Fall hex: level={}, ceiling={}, floor={}, depth={}, " + "containsWater={}, isElevationValid={}",
                                         fallHex.getLevel(),
                                         fallHex.ceiling(),
                                         fallHex.floor(),
                                         fallHex.depth(),
                    ...
                    logger.debug("[FALL-TRACE] Building check: curPos={}, lastPos={}, " + "containsBuilding={}",
                                 curPos,
                                 lastPos,
                    ...
                    if (climbHex.containsTerrain(Terrains.BUILDING)) {
                        ...
                        if (climbBldg != null) {
                            ...
                            if (entity.getWeight() > currentCF) {
                                logger.debug("[FALL-TRACE] Building too weak for climbing entity: " + "weight={}, CF={}, climbingElevation={}, curPos={}, lastPos={}",
                                             entity.getWeight(),
                                             currentCF,
                                             climbingElevation,
                                             curPos,
                ...
                if (levelsThisTurn < totalLevelsToClimb) {
                    ...
                    mpUsed = walkMP;
                    logger.debug("Climbing: partial climb, {} of {} levels. " + "Clinging at elevation {} in hex {}",
                                 levelsThisTurn,
                                 totalLevelsToClimb,
                                 climbingElevation,
                                 lastPos);
            ...
            movePath.addElement(new UnitLocation(entity.getId(),
                                                 ...
                                                 curFacing,
                                                 step.getElevation(),
                                                 entity.getBoardLocation().boardId()));
     ...
     */
    protected Vector<Report> processPossiblePBS(MoveStep step,
                                                Entity hiddenEntity) {
        ...
        gameManager.send(hiddenEntity.getOwnerId(), gameManager.createEntityPacket(this.entity.getId(), null));
    ...
    private String getReason(IBuilding bldgExited,
                             IBuilding bldgEntered) {
        String reason;
        if (bldgExited == null) {
            // If we're not leaving a building, just handle the "entered".
            reason = "entering";
        } else if (bldgExited.equals(bldgEntered) && !(entity instanceof ProtoMek) && !(entity instanceof Infantry)) {
     ...
     */
    private Coords nudgeOntoBoard(Coords position,
                                  int facing) {
     ...
     */
    private void processBuildBridgeStep(Infantry infantry,
                                        MoveStep step) {
        ...
        if (ConvInfantry.isBridgeTargetClaimed(getGame(),
                                               convInfantry.getBoardId(),
                                               step.getBridgeTargetCoords(),
                                               convInfantry)) {
            logger.warn("[BuildBridge] BUILD_BRIDGE step ignored for {}: another platoon already has a bridge in " + "progress at {}",
                        convInfantry.getShortName(),
                        step.getBridgeTargetCoords());
            return;
        }
        ...
        boolean isRepair = repairAllowed && BridgeConstruction.isBridgeRepairSite(getGame().getBoard(convInfantry.getBoardId()),
                                                                                  step.getBridgeTargetCoords(),
                                                                                  step.getBridgeExits());
    ...
    private void processCancelBridgeStep(Infantry infantry) {
        ...
        logger.info("[BuildBridge] {} cancels its bridge build at {}: dismantling will take {} turn(s)",
                    convInfantry.getShortName(),
                    target,
                    convInfantry.getBridgeDismantleRequiredTurns());
    ...
    private void processResumeBridgeStep(Infantry infantry) {
        ...
        logger.info("[BuildBridge] {} resumes its bridge build at {}: {} of {} turns built",
                    convInfantry.getShortName(),
                    target,
                    convInfantry.getBridgeBuildTurns(),
     ...
     */
    private void beginRubbleClearing(Entity entity,
                                     Coords position) {
        ...
        gameManager.sendToast(GameToastEvent.Level.INFO,
                              Messages.getString("Bulldozer.beginClearToast", entity.getShortName(), requiredTurns),
                              entity);
