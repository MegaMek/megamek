...
public class DamageEntityHandler extends AbstractTWRuleHandler {
    
    DamageEntityHandler(TWGameManager gameManager) {
        super(gameManager);
    }
    ...
    public Vector<Report> damageEntity(Entity te, HitData hit, int damage, boolean ammoExplosion, DamageType damageType,
        ...
        if ((hit.getSingleAV() > -1) && !getGame().getOptions().booleanOption(OptionsConstants.ADVAERORULES_AERO_SANITY)) {
        ...
        if (isCapital &&
                  (!te.isCapitalScale() ||
                         getGame().getOptions().booleanOption(OptionsConstants.ADVAERORULES_AERO_SANITY))) {
            damage = 10 * damage;
                ...
                if (mek.isAutoEject() &&
                          (!getGame().getOptions().booleanOption(OptionsConstants.RPG_CONDITIONAL_EJECTION) ||
                                 (getGame().getOptions().booleanOption(OptionsConstants.RPG_CONDITIONAL_EJECTION) &&
                                        mek.isCondEjectAmmo()))) {
                    autoEject = true;
                    vDesc.addAll(gameManager.ejectEntity(te, true));
                }
        ...
        int critBonus = 0;
        if (getGame().getOptions().booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_CRIT_ROLL) &&
                  (damage_orig > 0) &&
        ...
        Entity ae = getGame().getEntity(hit.getAttackerId());
        ...
        if (platoonOrBattleArmor &&
                  !te.isDestroyed() &&
                  !te.isDoomed() &&
                  getGame().getPlanetaryConditions().getAtmosphere().isLighterThan(Atmosphere.THIN)) {
            // PBI. Double damage.
            damage *= 2;
            r = new Report(6041);
            r.subject = te_n;
        ...
        if ((te instanceof VTOL) &&
                  ...
                  !getGame().getOptions().booleanOption(OptionsConstants.ADVCOMBAT_FULL_ROTOR_HITS)) {
            damage = (damage + 9) / 10;
        }
        ...
        while (damage > 0) {
            ...
            if ((te instanceof Aero) && (hit.getLocation() == Aero.LOC_AFT) && !damageIS) {
                for (Mounted<?> mAmmo : te.getAmmo()) {
                    if (mAmmo.isDumping() &&
                        ...
                        // doh. explode it
                        vDesc.addAll(gameManager.explodeEquipment(te, mAmmo.getLocation(), mAmmo));
                        mAmmo.setHit(true);
                    }
                }
            }
                ...
                if (a.getCapArmor() <= 0) {
                    ...
                    if (a instanceof LandAirMek lam) {
                        ...
                        if (lam.isAutoEject() &&
                            ...
                            vDesc.addAll(gameManager.ejectEntity(te, true, false));
                        }
                    } else {
                    ...
                    vDesc.addAll(gameManager.destroyEntity(te, "Structural Integrity Collapse"));
                    a.doDisbandDamage();
                    a.setCapArmor(0);
                    if (hit.getAttackerId() != Entity.NONE) {
                        gameManager.creditKill(te, getGame().getEntity(hit.getAttackerId()));
                    }
                }
                // check for aero crits from natural 12 or threshold; LAMs take damage as meks
                if (te instanceof Aero) {
                    gameManager.checkAeroCrits(vDesc, (Aero) te, hit, damage_orig, critThresh, critSI, ammoExplosion, nukeS2S);
                }
                return vDesc;
            }
            ...
            if (!damageIS) {
                ...
                if (!ammoExplosion &&
                    ...
                    damage = gameManager.damageExternalPassenger(te, hit, damage, vDesc, passenger);
                }
                ...
                if ((!(te instanceof Mek) || bTorso) &&
                          ...
                          (damageType != DamageType.IGNORE_PASSENGER) &&
                          !ammoExplosion) {
                    Entity swarm = getGame().getEntity(swarmer);
                    ...
                    Vector<Report> newReports = gameManager.damageEntity(swarm, passHit, absorbedDamage);
            ...
            if (!ammoExplosion && (te.getArmor(hit) > 0) && !damageIS) {
                ...
                if ((te instanceof Tank) &&
                          getGame().getOptions().booleanOption(OptionsConstants.ADVCOMBAT_VEHICLES_THRESHOLD) &&
                          !((te instanceof VTOL) || (te instanceof GunEmplacement))) {
                    int thresh = (int) Math.ceil((getGame().getOptions()
                                                        .booleanOption(OptionsConstants.ADVCOMBAT_VEHICLES_THRESHOLD_VARIABLE) ?
                                                        te.getArmor(hit) :
                                                        te.getOArmor(hit)) /
                                                       (double) getGame().getOptions()
                ...
                if (armorThreshold >= damage) {
                    ...
                    if ((te instanceof TeleMissile) && (te.getArmor(hit) == damage)) {
                        vDesc.addAll(gameManager.destroyEntity(te, "damage", false));
                    }
                ...
                if (te.hasBARArmor(hit.getLocation())) {
                    if (origDamage > te.getBARRating(hit.getLocation())) {
                        if (te.hasArmoredChassis()) {
                            ...
                            vDesc.addAll(gameManager.criticalEntity(te,
                                  hit.getLocation(),
                                  hit.isRear(),
                                  -1 + critBonus,
                                  damage_orig));
                        } else {
                            vDesc.addAll(gameManager.criticalEntity(te, hit.getLocation(), hit.isRear(), critBonus, damage_orig));
                        }
                    }
                }
            ...
            if (damage > 0) {
                ...
                if (te instanceof Aero a) {
                    ...
                    if (getGame().getOptions().booleanOption(OptionsConstants.ADVAERORULES_STRATOPS_OVER_PENETRATE)) {
                    ...
                    if (a.getSI() <= 0) {
                        ...
                        if (a.isAutoEject() &&
                            ...
                            vDesc.addAll(gameManager.destroyEntity(te,
                                  "Structural Integrity Collapse",
                                  damageType != DamageType.CRASH));
                        }
                        a.setSI(0);
                        if (hit.getAttackerId() != Entity.NONE) {
                            gameManager.creditKill(a, getGame().getEntity(hit.getAttackerId()));
                        }
                    }
                    gameManager.checkAeroCrits(vDesc, a, hit, damage_orig, critThresh, critSI, ammoExplosion, nukeS2S);
                    return vDesc;
                }
                ...
                if (ammoExplosion && (te instanceof Tank) && te.locationHasCase(Tank.LOC_BODY)) {
                    ...
                    vDesc.addAll(gameManager.applyCriticalHit(te, Entity.NONE, new CriticalSlot(0, critIndex), true, 0, false));
                }
                ...
                if (te.getInternal(hit) > 0) {
                    ...
                    if ((te.getInternal(hit) > damage) && (damage > 0)) {
                        ...
                        if (te instanceof ProtoMek) {
                            ...
                            if (hits > 0) {
                                vDesc.addAll(gameManager.damageCrew(te, hits));
                        ...
                        if ((te instanceof Mek) &&
                            ...
                            vDesc.addElement(r);
                            Hex h = getGame().getBoard().getHex(te.getPosition());
                            if (null != h) {
                                ...
                                gameManager.sendChangedHex(te.getPosition());
                            }
                        }
                        ...
                        if ((te instanceof Mek) || (te instanceof Tank)) {
                            ...
                            if ((null != passenger) && !passenger.isDoomed()) {
                                ...
                                if (passenger.getInternal(passHit) > 0) {
                                    vDesc.addAll(gameManager.damageEntity(passenger, passHit, damage));
                                }
                        ...
                        if (te instanceof BattleArmor) {
                            ...
                            if (infernos > 0) {
                                ...
                                if (diceRoll.getIntValue() >= 8) {
                                    ...
                                    if (c == null) {
                                        Entity transport = getGame().getEntity(te.getTransportId());
                                        if (transport != null) {
                                            c = transport.getPosition();
                                        }
                                        gameManager.getMainPhaseReport().addAll(gameManager.deliverInfernoMissiles(te, te
                                              , infernos));
                                    }
                                    if (c != null) {
                                        gameManager.getMainPhaseReport().addAll(gameManager.deliverInfernoMissiles(te,
                ...
                if (te.getInternal(hit) <= 0) {
                    ...
                    if (nextHit.getLocation() == Entity.LOC_DESTROYED) {
                        ...
                        boolean engineExploded = gameManager.checkEngineExplosion(te, vDesc, te.engineHitsThisPhase);
                        ...
                        if (!engineExploded) {
                            ...
                            if ((te instanceof Mek mek) &&
                                ...
                                if (mek.isAutoEject() &&
                                    ...
                                    autoEject = true;
                                    vDesc.addAll(gameManager.ejectEntity(te, true, true));
                                }
                            }
                            ...
                            if (getGame().getOptions().booleanOption(OptionsConstants.ADVGRNDMOV_AUTO_ABANDON_UNIT)) {
                                vDesc.addAll(gameManager.abandonEntity(te));
                            }
                        }
            ...
            if ((te.getInternal(hit) != IArmorState.ARMOR_DESTROYED) &&
                ...
                for (int i = 0; i < specCrits; i++) {
                    ...
                    vDesc.addAll(gameManager.criticalEntity(te, hit.getLocation(), hit.isRear(), critMod + critBonus, damage_orig));
                }
                specCrits = 0;
            }
            ...
            if (isHeadHit && !te.hasAbility(OptionsConstants.MD_DERMAL_ARMOR)) {
                Report.addNewline(vDesc);
                vDesc.addAll(gameManager.damageCrew(te, 1));
            }
            ...
            if (!(te instanceof Aero) && (te.getInternal(hit) <= 0)) {
                ...
                if ((te instanceof Mek) && ((hit.getLocation() == Mek.LOC_RT) || (hit.getLocation() == Mek.LOC_LT))) {
                    ...
                    boolean engineExploded = gameManager.checkEngineExplosion(te, vDesc, numEngineHits);
                    ...
                    if (!engineExploded && (numEngineHits >= hitsToDestroy)) {
                        // third engine hit
                        vDesc.addAll(gameManager.destroyEntity(te, "engine destruction"));
                        if (getGame().getOptions().booleanOption(OptionsConstants.ADVGRNDMOV_AUTO_ABANDON_UNIT)) {
                            vDesc.addAll(gameManager.abandonEntity(te));
                        }
                    ...
                    if ((te instanceof LandAirMek) && !te.isDestroyed() && !te.isDoomed()) {
                        ...
                        if (te.isAirborneVTOLorWIGE()) {
                            vDesc.add(r);
                            gameManager.crashAirMek(te,
                                  new PilotingRollData(te.getId(), TargetRoll.AUTOMATIC_FAIL, "side torso destroyed"),
                                  vDesc);
                        } else if (te.isAirborne() && te.isAero()) {
                            vDesc.add(r);
                            vDesc.addAll(gameManager.processCrash(te, ((IAero) te).getCurrentVelocity(), te.getPosition()));
                        }
                    }
                }
        ...
        // destroyed the unit
        if (ammoExplosion &&
                  getGame().getOptions().booleanOption(OptionsConstants.ADVCOMBAT_TACOPS_AMMUNITION)
