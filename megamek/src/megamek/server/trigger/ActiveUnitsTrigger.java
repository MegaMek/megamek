 ...
 * of all players are counted, otherwise only those of the given player. When a list of unit IDs is given, only those
 * units will be considered. Note that immobile units or those with stunned crews or the like are still active. Only
 * units that have fled or are destroyed are no longer active. Note that destroyed units can spawn pilots and some other
 * objects are counted as in-game, so providing an ID list can make sense. Note that this trigger will react multiple
 * times. Use {@link OnceTrigger} to limit it to one-time-only.
 */
