...
public class KilledUnitsTrigger implements Trigger {
     ...
     * When the playerName is blank, units of all players are counted, otherwise only those of the given player. When a
     * list of unit IDs is given, only those units will be considered. Note that destroyed units can spawn pilots, so
     * providing an ID list to prevent counting the wrong units usually makes sense. Note that this trigger will react
    ...
    /**
     * Creates a Trigger that reacts when the given unit is killed. Note that this trigger will react multiple times.
     * Use {@link OnceTrigger} to limit it to one-time-only.
     *
    ...
    /**
     * Creates a Trigger that reacts when the count of units that were killed is equal to the given count. When a list
     * of unit IDs is given, only those units will be considered. Note that destroyed units can spawn pilots, so
     * providing an ID list to prevent counting the wrong units usually makes sense. Note that this trigger will react
     * multiple times. Use {@link OnceTrigger} to limit it to one-time-only.
     *
     * @param unitIds         A list of Ids to limit the checked units to; when empty, all units are considered
