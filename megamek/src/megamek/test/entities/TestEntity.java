...
public abstract class TestEntity implements TestEntityOption
{
    ...
    // Copied from megamek/server/UnitStatusFormatter.java
    private static final String SPACES = 
        "                                                                     ";
    protected static String makeLength(String s, int n)
    {
        return makeLength(s, n, false);
    }

    protected static String makeLength(String s, int n, boolean bRightJustify)
    {
        int l = s.length();
        if (l == n)
            return s;
    ...
    protected static String makeLength(int s, int n)
    {
    ...
    {
        for (Enumeration e = getEntity().getEquipment(); e.hasMoreElements(); )
        {
    ...
    {
        return makeLength("Engine: "+engine.getShortEngineName(), getPrintSize()-5)+
            ...
            int posWeight)
    {
        for (Enumeration e = getEntity().getMisc(); e.hasMoreElements(); )
        {
            ...
            buff.append(makeLength(mt.getName(),20));
            buff.append(makeLength(getLocationAbbr(m.getLocation()),
                        getPrintSize()-5-20))
    ...
    public StringBuffer printLocations()
    {
    ...
    public String printWeightCarryingSpace()
    {
        if (getEntity().getTroopCarryingSpace()!=0)
            return makeLength("Carrying Capacity:", getPrintSize()-5)+
    ...
    public String printArmorLocation(int loc)
    {
        if (getEntity().hasRearArmor(loc))
            return makeLength(getEntity().getLocationAbbr(loc)+":", 5)+
                makeLength(getEntity().getOInternal(loc), 4)+
                makeLength(getEntity().getOArmor(loc),3)+" / "+
                makeLength(getEntity().getOArmor(loc,true),2);
        else 
            return makeLength(getEntity().getLocationAbbr(loc)+":", 5)+
                makeLength(getEntity().getOInternal(loc), 4)+
                makeLength(getEntity().getOArmor(loc),6)+"  ";
    }
