...
public class RATGeneratorEditor extends JFrame {
    ...
    private static final String[] MOVEMENT_TYPE_NAMES = { "Leg", "Tracked", "Wheeled", "Hover", "WiGE", "VTOL", "Naval",
                                                          "Underwater", "Jump", "Motorized", "Atmospheric", "Aerospace",
                                                          "Space", "None" };
    ...
    public RATGeneratorEditor() {
        ...
        ratGenerator.getEraSet().forEach(e -> ratGenerator.loadYear(e));
        ERAS = ratGenerator.getEraSet().toArray(new Integer[0]);
    ...
    public RATGeneratorEditor(File dir) {
        ...
        if (dir.exists() && dir.isDirectory()) {
            lastDir = dir;
            ratGenerator.reloadFromDir(lastDir);
            ERAS = ratGenerator.getEraSet().toArray(new Integer[0]);
            ratGenerator.initRemainingUnits();
        }
    }
    ...
    private void initUI() {
        ...
        masterUnitListModel = new MasterUnitListTableModel(ratGenerator.getModelList());
        ...
        panButtons.add(button);
        button.addActionListener(ev -> RATDataCSVExporter.exportToCSV(ratGenerator));
    ...
    private void saveValues() {
        ...
        if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            lastDir = chooser.getSelectedFile();
            ratGenerator.exportRATGen(lastDir);
        }
    }

    private void fillFactionChoosers() {
        if (ratGenerator.getFactionList() == null) {
            return;
        }
    ...
    private void setCurrentUnitFactions() {
        ...
        if (tblMasterUnitList.getSelectedRow() >= 0) {
            ModelRecord model = masterUnitListModel.getUnitRecord(tblMasterUnitList.convertRowIndexToModel(
                  tblMasterUnitList.getSelectedRow()));
            for (int i = 0; i < ratGenerator.getEraSet().size(); i++) {
                Collection<AvailabilityRating> chassisRecs = ratGenerator.getChassisFactionRatings(ERAS[i],
                      model.getChassisKey());
                if (chassisRecs != null) {
    ...
    private JComponent createUnitTab() {
        ...
        tblMasterUnitList.getSelectionModel().addListSelectionListener(evt -> {
            ...
            if (tblMasterUnitList.getSelectedRow() >= 0) {
                ModelRecord rec = masterUnitListModel.getUnitRecord(tblMasterUnitList.convertRowIndexToModel(
                      tblMasterUnitList.getSelectedRow()));
    ...
    private JComponent createUnitModelEditor() {
        ...
        return getjComponent(unitModelEditor,
              tblUnitModelEditor,
    ...
    private JComponent getjComponent(Box unitModelEditor, JTable tblUnitModelEditor,
          UnitEditorTableModel unitModelEditorModel, JTable tblMasterUnitList, TableCellRenderer unitListRenderer,
        ...
        tblUnitModelEditor.getTableHeader()
              .setPreferredSize(new Dimension(tblMasterUnitList.getColumnModel().getTotalColumnWidth(), 32));
        ...
        JScrollPane scroll = new JScrollPane(tblUnitModelEditor,
    ...
    private JComponent createUnitChassisEditor() {
        ...
        return getjComponent(unitChassisEditor,
              tblUnitChassisEditor,
    ...
    private JPanel createFactionTab() {
        ...
        delButton.addActionListener(ev -> {
            if (tblMasterFactionList.getSelectedRow() >= 0) {
                FactionRecord rec = masterFactionListModel.getFactionRecord(tblMasterFactionList.convertRowIndexToModel(
                      tblMasterFactionList.getSelectedRow()));
                masterFactionListModel.delRecord(rec);
                ratGenerator.removeFaction(rec);
            }
        });
        ...
        masterFactionListModel = new FactionListTableModel(ratGenerator.getFactionList());
        ...
        JScrollPane editorScroll = new JScrollPane(tblFactionEditor,
              JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
              JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        ...
        JScrollPane salvageScroll = new JScrollPane(tblSalvageEditor,
              JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
              JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    private JButton getCopyButton() {
        ...
        copyButton.addActionListener(ev -> {
            if (!txtNewFaction.getText().isBlank() && (tblMasterFactionList.getSelectedRow() >= 0)) {
                FactionRecord from = masterFactionListModel.getFactionRecord(tblMasterFactionList.convertRowIndexToModel(
                      tblMasterFactionList.getSelectedRow()));
                FactionRecord rec = new FactionRecord(txtNewFaction.getText());
        ...
        RowFilter<MasterUnitListTableModel, Integer> rf = new RowFilter<>() {
            ...
            public boolean include(Entry<? extends MasterUnitListTableModel, ? extends Integer> entry) {
                ...
                if (cbMovementType.getSelectedIndex() > 0 &&
                          (rec.getMovementMode() !=
                                 EntityMovementMode.parseFromString((String) cbMovementType.getSelectedItem()))) {
                    return false;
                }
    ...
    private static class MasterUnitListTableModel extends DefaultTableModel {
        ...
        public static final String[] colNames = { "Chassis", "Model", "Unit Type", "Weight", "Year", "Extinct Years",
                                                  "MUL Role", "Role", "Deployed With", "Exclude Factions" };
    ...
    private static class UnitEditorTableModel extends DefaultTableModel {
        ...
        public void setData(AbstractUnitRecord unitRec, int mode) {
            ...
            for (int i = 0; i < ratGenerator.getEraSet().size(); i++) {
                Collection<AvailabilityRating> recs = (mode == MODE_MODEL) ?
                                                            ratGenerator.getModelFactionRatings(ERAS[i], getUnitKey()) :
                                                            ratGenerator.getChassisFactionRatings(ERAS[i],
                                                                  unitRec.getChassisKey());
                if (recs != null) {
                    for (AvailabilityRating rec : recs) {
                        ...
                        if (mode == MODE_SUMMARY) {
                            AvailabilityRating mar = ratGenerator.findModelAvailabilityRecord(ERAS[i],
                                  unitRec.getKey(),
                                  rec.getFaction());
                            if (mar != null) {
                                int weight = (int) ((rec.getWeight() * 10.0 * mar.getWeight()) /
                                                          ratGenerator.getChassisRecord(unitRec.getChassisKey())
        ...
        public void setValueAt(Object value, int row, int col) {
            ...
            if (ratGenerator.getChassisRecord(getUnitKey()) != null) {
                if (ar == null) {
                    ratGenerator.removeChassisFactionRating(era, getUnitKey(), factions.get(row));
                } else {
                    ratGenerator.setChassisFactionRating(era, getUnitKey(), ar);
                }
            } else {
                if (ar == null) {
                    ratGenerator.removeModelFactionRating(era, getUnitKey(), factions.get(row));
                } else {
                    ratGenerator.setModelFactionRating(era, getUnitKey(), ar);
                }
            }
        }
        ...
        public boolean addEntry(String faction, List<String> factionData) {
            ...
            if (mode == MODE_MODEL) {
                ...
                for (int era : ERAS) {
                    if (ratGenerator.getChassisFactionRatings(era, unitRecord.getChassisKey()) != null) {
            ...
            if (rowAdded) {
                for (int i = 0; i < ERAS.length; i++) {
                    if (!rowData.eraData.get(i).isBlank()) {
                        AvailabilityRating ar = new AvailabilityRating(getUnitKey(),
                              ERAS[i],
                              rowData.faction + ":" + rowData.eraData.get(i));
                        if (mode == MODE_MODEL) {
                            ratGenerator.setModelFactionRating(ERAS[i], getUnitKey(), ar);
                        } else {
                            ratGenerator.setChassisFactionRating(ERAS[i], getUnitKey(), ar);
                        }
                    }
                }
            }
    ...
    private class FactionListTableModel extends DefaultTableModel {
        ...
        public final String[] colNames = { "Code", "Name", "Years", "Minor", "Clan", "Periphery", "Ratings",
                                           "Use Alt" };
    ...
    private static class FactionEditorTableModel extends DefaultTableModel {
        ...
        private static final String[] CATEGORIES = { "Omni %", "Clan %", "SL %", "Omni % (Aero)", "Clan % (Aero)",
                                                     "SL % (Aero)", "Clan % (Vee)", "SL % (Vee)" };

        private static final int[] WEIGHT_DIST_UNIT_TYPES = { UnitType.MEK, UnitType.TANK, UnitType.AEROSPACEFIGHTER,
                                                              UnitType.AERO };
        ...
        public int getRowCount() {
            ...
            return 1 + CATEGORIES.length * factionRec.getRatingLevels().size() + WEIGHT_DIST_UNIT_TYPES.length;
        }
        ...
        public Object getValueAt(int row, int col) {
            ...
            if (col == 0) {
                if (row == 0) {
                    ...
                    return UnitType.getTypeName(WEIGHT_DIST_UNIT_TYPES[row -
                                                                             1 -
                                                                             factionRec.getRatingLevels().size() *
                                                                                   CATEGORIES.length]);
                } else {
            ...
            if (row == 0) {
                ...
                int unitType = WEIGHT_DIST_UNIT_TYPES[row -
                                                            1 -
