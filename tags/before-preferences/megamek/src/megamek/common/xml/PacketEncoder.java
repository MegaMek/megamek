...
/**
 * Objects of this class can encode a <code>Packet</code> object as XML
...
public class PacketEncoder {
     ...
     *
     * @param   packet - the <code>Packet</code> to be encoded.
     ...
     *          contain a valid <code>Packet</code>.
     ...
     * @throws  <code>IOException</code> if there's any error on write.
     */
    public static void encode( Packet packet, Writer out )
        throws IOException
    {
            ...
            if ( subNode.getName().equals("packetData") ) {
                ...
                if ( subNode.getAttribute("isGzipped").equals("true") ) {
                    ...
                    if ( null == cdata ) {
                        ...
                        while ( cdataEnum.hasMoreElements() &&
                                null == cdata ) {
                            final ParsedXML cdataNode =
                                (ParsedXML) cdataEnum.nextElement();
                    ...
                    try {
                        ...
                        iobuf.append( "Could not unzip data elements: " )
                            .append( ioErr.getMessage() );
                    ...
                    try {
                        ...
                        try {
                            ...
                            int inChar = 0;
                            while ( -1 != (inChar = parseStream.read()) ) {
