...
public class BoardView1 extends JPanel implements IBoardView, Scrollable, BoardListener,
    ...
    public synchronized void paintComponent(Graphics g) {
        ...
        rect.x = -getX();
        rect.y = -getY();
        rect.width = Math.min(scrollpane.getViewport().getViewRect().width, boardSize.width);
        rect.height = Math.min(scrollpane.getViewport().getViewRect().height, boardSize.height);
        for (int i = 0; i < displayables.size(); i++) {
            IDisplayable disp = displayables.get(i);
            disp.draw(g, rect);
        }
    ...
    /**
     * Draws the Entity for the given hex. This function is used by the isometric rendering process so
     ...
     */
    private synchronized void drawIsometricSpritesForHex(Coords c, Graphics g, ArrayList<IsometricSprite> spriteArrayList) {
     ...
     */
    private final void drawIsometricSprites(Graphics g, ArrayList<IsometricSprite> spriteArrayList) {
        Rectangle view = g.getClipBounds();
        for (IsometricSprite sprite : spriteArrayList) {
            if (view.intersects(sprite.getBounds()) && !sprite.hidden) {
                ...
                sprite.drawOnto(g, sprite.getBounds().x, sprite.getBounds().y, this, true);
            }
        }
    }
    ...
    private void drawHex(Coords c, Graphics boardGraph) {
        ...
        if (useIsometric() && (hex.terrainLevel(Terrains.BRIDGE_ELEV) > 0)) {
    ...
    /**
     * Draws a orthographic hex onto the board buffer. This assumes that drawRect is current,
     *  and does not check if the hex is visible.
     */
    private void drawOrthograph(Coords c, Graphics boardGraph) {
        ...
        int orthX = oHexLoc.x;
        int orthY = (oHexLoc.y - (int)(HEX_ELEV * scale * oHex.terrainLevel(Terrains.BRIDGE_ELEV)));
        if (!useIsometric()){
        	orthY = oHexLoc.y;
        	}
    ...
    private final void drawIsometricElevation(Coords c, Color color, Point p1,
        ...
        Polygon p = new Polygon(new int[] { p1.x, p2.x, p2.x, p1.x}, new int[] {
                p1.y+fudge, p2.y+fudge, p2.y + fudge + (int) (HEX_ELEV * scale * delta),
                p1.y + fudge + (int) (HEX_ELEV * scale * delta)}, 4);
    ...
    public void redrawMovingEntity(Entity entity, Coords position, int facing, int elevation) {
        ...
        if (sprite != null) {
            ...
            newSpriteIds = new HashMap<ArrayList<Integer>, EntitySprite>(entitySpriteIds);
        ...
        if(isoSprite != null) {
            ...
            newIsoSpriteIds = new HashMap<ArrayList<Integer>, IsometricSprite>(isometricSpriteIds);
        ...
        if (entity.getPosition() != null) {
            mSprite = new MovingEntitySprite(entity, position, facing, elevation);
    ...
    public void redrawEntity(Entity entity) {
        ...
        HashMap<ArrayList<Integer>, EntitySprite> newSpriteIds = new HashMap<ArrayList<Integer>, EntitySprite>(
                entitySpriteIds);
        ArrayList<IsometricSprite> isoSprites = new ArrayList<IsometricSprite>(isometricSprites);
    ...
    void redrawAllEntities() {
        ...
        ArrayList<IsometricSprite> newIsometricSprites = new ArrayList<IsometricSprite>(game.getNoOfEntities());
        HashMap<ArrayList<Integer>, EntitySprite> newSpriteIds = new HashMap<ArrayList<Integer>, EntitySprite>(game
                .getNoOfEntities());
        HashMap<ArrayList<Integer>, IsometricSprite> newIsoSpriteIds = new HashMap<ArrayList<Integer>, IsometricSprite>(game
                .getNoOfEntities());
    ...
    private class MovingEntitySprite extends Sprite {
        ...
        public MovingEntitySprite(final Entity entity, final Coords position, final int facing, final int elevation) {
            this.entity = entity;
            this.facing = facing;
        ...
        public void drawOnto(Graphics g, int x, int y, ImageObserver observer) {
            ...
            if (useIsometric() && (entity.isAirborne() || entity.isAirborneVTOLorWIGE())) {
                ...
                g.drawImage(shadow, x, y + (int) (elevation * HEX_ELEV * scale), observer);
            }
            //submerged?
            if (useIsometric() && ((elevation + entity.getHeight()) < 0)) {
            Graphics2D g2 = (Graphics2D) g;
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.35f));
            g2.drawImage(image, x, y - (int) (elevation * HEX_ELEV * scale), observer);
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0f));
            } else {
                //create final image
                drawOnto(g, x, y, observer, false);                      	  
        ...
        public void prepare() {
            ...
            graph.setColor(new Color(TRANSPARENT));
            graph.fillRect(0, 0, bounds.width, bounds.height);
            graph.drawImage(tileManager.imageFor(entity, facing, -1), 0, 0, this);
    ...
    private class EntitySprite extends Sprite {
        ...
        public void drawOnto(Graphics g, int x, int y, ImageObserver observer) {
            if ( (trackThisEntitiesVisibilityInfo(entity)
                 && !entity.isVisibleToEnemy() && GUIPreferences.getInstance()
        ...
        public String[] getTooltip() {
            ...
            if (ge == null) {
                buffer.append(Messages.getString("BoardView1.move")) //$NON-NLS-1$
                        ...
                        .append(")") //$NON-NLS-1$
                        .append(entity.isEvading()?Messages.getString("BoardView1.Evade"):"")//$NON-NLS-1$ //$NON-NLS-2$
                        .append(";") //$NON-NLS-1$
    ...
    private void moveBoardImage() {
        ...
        boardGraph.copyArea(0, 0, drawRect.width, drawRect.height,
                drawRect.x - viewRect.x, drawRect.y - viewRect.y);

        // what's left to paint?
        int midX = Math.max(viewRect.x, drawRect.x);
        int midWidth = viewRect.width - Math.abs(viewRect.x - drawRect.x);
        Rectangle unLeft = new Rectangle(viewRect.x, viewRect.y, drawRect.x - viewRect.x,
                viewRect.height);
        ...
        Rectangle unBottom = new Rectangle(midX,
                drawRect.y + drawRect.height, midWidth, viewRect.y - drawRect.y);
