...
public class TechConstants {
    ...
    public static final String[] T_NAMES = { "IS_level_1", "IS_level_2",
            "Clan_level_2", "IS_level_1_and_2", "All_level_2", "IS_level_3",
            "Clan_level_3", "All" };
    ...
    public static boolean isLegal(int entityTechlevel, int equipmentTechlevel,
        ...
        // If the entity is level 3, it can legally be mixed tech, so we pretty
        // much just smile and nod.
        if ((entityTechlevel == T_IS_LEVEL_3)
        ...
        if ((equipmentTechlevel == T_IS_LEVEL_1)
                && ((entityTechlevel == T_IS_LEVEL_2)
                        ...
                        || (entityTechlevel == T_IS_LEVEL_3) || (entityTechlevel == T_ALL)))
            return true;
        ...
        if ((equipmentTechlevel == T_CLAN_LEVEL_2)
                && ((entityTechlevel == T_CLAN_LEVEL_2)
                        || (entityTechlevel == T_CLAN_LEVEL_3) || (entityTechlevel == T_ALL)))
            return true;
    ...
    public static String getTechName(int level) {
        if (level == T_IS_LEVEL_1 || level == T_IS_LEVEL_2
                || level == T_IS_LEVEL_3)
            return "Inner Sphere";
