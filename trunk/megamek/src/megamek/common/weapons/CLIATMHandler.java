...
public class CLIATMHandler extends ATMHandler {
	...
	public CLIATMHandler(ToHitData t, WeaponAttackAction w, IGame g, Server s) {
		super(t, w, g, s);
		isAngelECMAffected = Compute.isAffectedByAngelECM(ae,
	...
	protected int calcHits(Vector<Report> vPhaseReport) {
		...
		if (atype.getMunitionType() == AmmoType.M_IATM_IIW || atype.getMunitionType() == AmmoType.M_IATM_IMP) 
		{
			return hits;
		}
	...
	protected int calcMissileHits(Vector<Report> vPhaseReport) {
		...
		// Only apply if not all shots hit. IATM IMP have HE ranges and thus suffer from spread too
		if ((atype.getMunitionType() == AmmoType.M_HIGH_EXPLOSIVE || atype.getMunitionType() == AmmoType.M_IATM_IMP)
	      ...
	      //Now I need to adjust this for attacks on aeros because they use attack values and different rules
	        ...
	        while (hits > 0) {
	            ...
	            if (entityTarget != null) {
	                ...
	                hits -= nCluster;
	                // do IMP stuff here!
	                if(entityTarget instanceof Mech || entityTarget instanceof Aero || entityTarget instanceof Tank )
	                {
	                	entityTarget.addIMPHits(Math.max(0, hits-bldgAbsorbs));
	                }
	            }
